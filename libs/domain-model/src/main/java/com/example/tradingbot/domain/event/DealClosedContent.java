...
public record DealClosedContent(String dealInternalId,
    ...
    /**
     * Содержимое терминала по сделке и идентичностям её радиуса.
     *
     * <p><b>Фабрика существует потому, что писателей у класса два —
     * терминалов два</b> (docs/architecture/contracts.md §События). Два
     * списка из восемнадцати компонентов разошлись бы <b>молча</b>: у
     * недоложенного операнда читатель получает не отказ, а правдоподобную
     * строку агрегата.
     *
     * <p>Перечни приводятся к именам своих значений, а пустой перечень
     * остаётся пустым: {@code String.valueOf} завёл бы на его месте литерал
     * {@code "null"}.
     *
     * @param graphComplete граф сделки предъявлен целиком — операнд
     *                      прохода, а не поле сделки
     */
    public static DealClosedContent of(Deal deal, String exchangeAccountInternalId,
                                       String instrumentInternalId, String strategyInternalId,
                                       Boolean graphComplete) {
        return new DealClosedContent(deal.getInternalId(),
                exchangeAccountInternalId,
                instrumentInternalId,
                strategyInternalId,
                name(deal.getStatus()),
                name(deal.getCloseReason()),
                deal.positionObserved(),
                graphComplete,
                deal.getResultProfit(),
                deal.getResultProfitCurrency(),
                deal.accumulatedFeeCost(),
                deal.accumulatedFundingCost(),
                deal.accumulatedLiquidationPenaltyCost(),
                deal.getPlannedRiskAmount(),
                name(deal.getCloseOutcome()),
                name(deal.getReconciliationStatus()),
                name(deal.getBreakdownIncomplete()),
                name(deal.getRiskBenchmarkAvailability()));
    }
}
