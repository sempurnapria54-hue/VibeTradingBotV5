...
import com.example.connector.okx.integration.model.okx.request.AttachAlgoOrdOkxRequest;
...
import com.example.connector.okx.integration.model.okx.response.AttachAlgoOrdOkxResponse;
...
public interface OrderMapper {
    ...
    @Mapping(target = "tag", expression = "java(OkxConstants.ORDER_TAG)")
    ...
    @Mapping(target = "slOrdPx", expression = "java(OkxConstants.MARKET_PRICE_FLAG)")
    ...
    default String resolveOrdType(Order order) {
        return nonNull(order.getPrice()) ? OkxConstants.ORD_TYPE_LIMIT : OkxConstants.ORD_TYPE_MARKET;
    }
}
