...
/**
 * Клетка {@code B4.2} — смещение ниже наименьшего доступного: момент
 * разрыва (.claude/tests/cases/audit.md).
 *
 * <p><b>Класс у неё свой, потому что вход её — СОСТОЯНИЕ ТЕМЫ на
 * брокере.</b> Записи темы удаляются безвозвратно, и соседняя клетка,
 * читающая ту же тему, наблюдала бы исход этой.
 *
 ...
 * поднимает наименьшее доступное над ним
 * ({@link Wire#deleteRecordsBefore}). В проде то же состояние производит
 * срок хранения — величина ОКРУЖЕНИЯ, ждать которой прогон не может;
 * наблюдаемое у обоих поводов одно, и клетка утверждает о нём.
 *
 * <p><b>Соседняя пара получает своё смещение ДО ребалансировки, и это
 * несущая часть клетки.</b> Без него её ветвь на том же назначении —
 * третья («смещения нет вовсе»), и момент наблюдения соседки уехал бы
 * вперёд, подняв нижнюю границу полноты: клетка о неподвижности границы
 * краснела бы по причине, которой не ставила.
 *
 * <p><b>Ложь предиката даёт РАЗРЫВ, а не остановка.</b> Флаг остановки у
 * пары лежит ложью и после назначения: приём по ней не ломали ни разу.
 */
class RetentionGapBoxTest extends AuditBox {
    ...
    /** Сколько строк журнала лежит к моменту ребалансировки. */
    ...
    @Test
    @DisplayName("B4.2 — Смещение ниже наименьшего доступного: момент разрыва")
    void anOffsetBelowTheEarliestAvailableYieldsTheGapMoment() {
        givenReceptionStateRows();
        publish(neighbour(), "E-N", momentsAgo(EVENT_AGE), Bodies.reference());
        awaitConsumed(neighbour());
        Wire.publishInTransaction(subject(), TENANT, envelope("E-1", momentsAgo(EVENT_AGE)),
                Bodies.reference());
        awaitRecordCount(PUBLISHED);
        ...
        assertThat(pair(neighbour()).get(GAP_COLUMN))
                .as("у соседней пары разрыва нет").isNull();
    ...
    /** Соседняя тема: ею наблюдается радиус разрыва и неподвижность границы. */
