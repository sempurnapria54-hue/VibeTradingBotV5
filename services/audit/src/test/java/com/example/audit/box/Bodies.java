...
final class Bodies {
    ...
    /**
     * Содержимое ровно с ОДНИМ именем радиуса верхнего уровня — сделкой.
     *
     * <p>Им ставится строка, которая одному отбору отвечает, а
     * конъюнкции с соседним — уже нет: без такой строки конъюнктивность
     * отборов не отличить от того, что лишний отбор просто не читается.
     *
     * @param deal сделка
     */
    static String withDeal(String deal) {
        return """
                {"dealInternalId": "%s", "outcome": "OPENED"}"""
                .formatted(deal);
    }
