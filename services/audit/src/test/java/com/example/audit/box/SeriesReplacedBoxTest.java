 ...
 * вынужденная замена оси.</b> Состав пар внутри одного контекста
 * неподвижен по построению: тик приводит строки к объявленной подписке
 * тем же тактом, которым считает ряды, а подписку контейнер читает при
 * подъёме. Значит набор рядов возраста в живом контексте сузиться не
 * может ни при каком входе, и клетка, стоящая на нём, мерила бы
 * недостижимое. Ряд порога сужается на достижимом входе: тема, снятая у
 * брокера, срока хранения больше не отдаёт.
 *
 * <p><b>Предмет от замены оси не меняется.</b> Утверждается ровно то, что
 * названо домом: ряд живёт один такт, и ряд прошлого такта не остаётся
 * ни прежним значением, ни нулём (docs/components/ReceptionStateJob.md
 * §«Ряды экспорта пишет тот же тик»). Ряд, который такт измерил, при этом
 * стои́т со своей прежней меткой.
 *
 * <p><b>Приём остаётся живым:</b> снимается ОДНА тема из двух, и
 * назначение по второй держит контейнер запущенным.
 *
 ...
 * уносит ряды ВСЕ — как всякий такт неживого приёма. Ожидание одного
 * лишь исчезновения ряда ушедшей темы кончилось бы именно там, и клетка
 * сошлась бы по причине соседней клетки {@code B3.6}. Поэтому условием
 * стои́т пара: ряда ушедшей нет, а ряд оставшейся ЕСТЬ, — то есть такт
 * измерил и заменил состав, а не промолчал.
 *
 * <p><b>Контекст закрывается вместе с классом</b>: одной темы его
 * подписки у брокера больше нет.
 */
@DirtiesContext
class SeriesReplacedBoxTest extends AuditBox {
