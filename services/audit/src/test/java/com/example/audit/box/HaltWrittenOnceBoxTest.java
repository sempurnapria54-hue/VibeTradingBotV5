...
/**
 * Клетка {@code B2.13} — флаг ставится один раз, а не на каждой попытке
 * (.claude/tests/cases/audit.md).
 *
 * <p><b>Повторов у группы журнала не ограничено ничем</b>, и запись на
 ...
 *
 * <p><b>Наблюдателей у «не писали» ДВА, и первого мало.</b> Момент
 * обновления строки ведёт только тик
 * (docs/rules/writer-named-for-every-value.md), и запись величин приёма его
 * не трогает — но ровно поэтому неподвижный момент не отличает «записали
 * один раз» от «записывали сто раз»: обе тропы оставляют его прежним.
 * Отличает их ВЕРСИЯ строки в базе ({@link Rows#rowVersion}) — номер
 * транзакции, положившей нынешнюю версию, меняется при каждой записи, что
 * бы та ни писала. Клетка утверждает оба наблюдения разом.
 *
 ...
 * {@link UnlimitedRetryBoxTest}: входом клетки является число попыток, а не
 * их частота.
 */
class HaltWrittenOnceBoxTest extends PoisonedReceptionBox {
    ...
    static void substrate(DynamicPropertyRegistry registry) {
        AuditSubstrate.registerOwn(registry, "b2-13",
                Map.of(AuditSubstrate.RETRY_INTERVAL_KEY, RETRY_INTERVAL));
    }

    @Test
    @DisplayName("B2.13 — Флаг ставится один раз, а не на каждой попытке")
    void theHaltFlagIsWrittenOnceRatherThanOnEveryAttempt() {
        ...
        String versionAfterHalt = pairVersion(poisonedTopic());
        Awaitility.await()
                .during(OBSERVATION)
    ...
    private Boolean stateRowUnchanged(String versionAfterHalt, Long pairsBeforeFailure) {
        return Objects.equals(versionAfterHalt, pairVersion(poisonedTopic()))
