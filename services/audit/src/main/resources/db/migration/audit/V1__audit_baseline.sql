...
--
-- ЦЕПОЧКА СВОЯ, С V1, И ОНА У СЕРВИСА ЕДИНСТВЕННАЯ: владелец данных у
-- процесса один. Она лежит в db/migration/audit и идёт под
-- ролью-владельцем журнала; соседняя, ведшая базу агрегатов, уехала со
-- своим сервисом (.claude/decisions/audit-statistics-split.md).
-- Обязательные колонки вводятся сразу целевой формой: существующих строк
-- нет (.claude/rules/pre-launch-schema-changes.md).
--
