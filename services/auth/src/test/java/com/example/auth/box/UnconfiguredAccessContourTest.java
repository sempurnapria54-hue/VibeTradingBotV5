...
/**
 * Ненастроенный контур доступа — клетка {@code B6.1} документа
...
class UnconfiguredAccessContourTest {
    ...
    @Test
    @DisplayName("B6.1 — ненастроенный контур доступа не поднимает поверхности")
    void b6_1_anUnconfiguredAccessContourRaisesNoSurface() {
        Map<String, Object> properties = new LinkedHashMap<>();
        properties.putAll(AuthSubstrate.defaults());
        properties.put("spring.security.oauth2.resourceserver.jwt.issuer-uri", "");
        properties.put("server.port", "0");

        assertThatThrownBy(() -> new SpringApplicationBuilder(AuthApplication.class)
