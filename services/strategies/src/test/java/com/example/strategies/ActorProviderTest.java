 ...
 * правдоподобная: событие активации получило бы класс контура вместо имени
 * человека, который её запустил, — и журнал ответил бы на «кто это
 * сделал» неверно, а не пустотой. Отличить такую строку постфактум нечем
 * (docs/concept.md П3).
 *
 * <p><b>Отдельно проверяется ПЕРЕНОС контекста в чужой тред.</b>
 * Различение по треду ломается на первом же асинхронном фасаде: ручной
 * триггер джобы порождён человеком, а работа идёт в треде пула. Наследуемый
 * тред-локал здесь не годится по построению — он отдаёт контекст в момент
 * создания треда, а треды пула переиспользуются.
 *
