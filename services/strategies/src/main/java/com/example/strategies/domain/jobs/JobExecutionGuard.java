 ...
 * одновременно. Для реле это не опасно и без замка: публикация повторяется
 * идемпотентно, а дубль закрыт отметкой обработанного у потребителя,
 * форму которой задаёт его следствие — замок экономит работу, а не
 * держит инвариант.
 */
@Slf4j
@Component
