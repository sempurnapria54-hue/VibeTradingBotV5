 ...
 * вовсе</b>: он приходит контекстом вызова, выведенным периметром из
 * членств предъявителя, и принятый из тела был бы ВТОРЫМ объявлением
 * вызывающего о самом себе
 * (docs/models/domain/aggregate/Strategy.md §«У каждого поля контекста
 * назван писатель и момент»). Полученное значение сервис-владелец не
 * сверяет — названное ограничение, дом которого
 * docs/architecture/contracts.md §«Контекст тенанта в вызове». Счёт и
 * инструмент, наоборот, — содержательный выбор автора, и их место здесь.
 *
