 ...
 * market-data сам ходит к соседу под служебной идентичностью кластера, и
 * точка выдачи того же стаба есть вход клеток {@code B8.12} и
 * {@code B8.13}. Токен выдачи — значение, известное до прогона: иначе
 ...
 * предмета.</b> На границе market-data вызывающий — сервис, а не человек:
 * членство не резолвится, тенант не принимается вовсе
 * (docs/architecture/contracts.md §«Контекст тенанта в вызове»). Поэтому
 * входами остаются ровно те оси, которые проверяет контур
 * ресурс-сервера, — ключ подписи, издатель и срок ({@code B8.2}).
 *
...
final class IdentityStub {
     ...
     * что клетка {@code B8.12} ищет его отсутствие, а искать случайную
     * строку нечем.
     */
    ...
    /** Точка, отказывающая в выдаче: вход клетки {@code B8.13}. */
