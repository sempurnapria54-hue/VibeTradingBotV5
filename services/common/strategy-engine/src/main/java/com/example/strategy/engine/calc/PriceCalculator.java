...
/**
 * Считает цены действия и возвращает {@link CalculatedPrice}
 * (docs/components/PriceCalculator.md). Размер не считает, риск не
 * проверяет и не решает, нужно ли выполнять действие; волатильность и
 * структуру по свечам не считает — берёт готовые значения контекста.
 *
 * <p><b>Источник, которого источник данных не отдаёт, — отказ, а не
 * подстановка.</b> Марк-цены и индексной цены в снапшоте рыночных цен нет
 * (docs/components/models/MarketPriceData.md); подстановка последней цены
 * на их место меняет ЦЕНОВОЙ ДОМЕН молча. До калькулятора такое
 ...
 * отказ здесь — второй рубеж на случай операнда, приехавшего иной тропой.
 *
...
public class PriceCalculator {
    ...
    private static final String PRICE_SOURCE_UNAVAILABLE = "PRICE_SOURCE_UNAVAILABLE";
    ...
    private CalculatedPrice orderPrice(StrategyOrderAction action, CalculationContext context) {
        ...
        if (Order.Type.ENTRY_ATTACHED_STOP_LOSS.equals(action.getOrderType())
                && nonNull(action.getAttachedProtection())) {
    ...
    private CalculatedPrice algoPrice(StrategyAlgoOrderAction action, CalculationContext context) {
        StrategyTradeDirection direction = context.getStrategyDirection();
        BigDecimal anchor = levelAnchor(context);
        CalculatedPrice.CalculatedPriceBuilder builder = CalculatedPrice.builder()
                .priceMode(PriceMode.NOT_REQUIRED)
                .sendPriceToExchange(Boolean.FALSE)
        ...
        switch (action.getConditionType()) {
            case STOP_LOSS, PARTIAL_STOP_LOSS -> builder
                    .purpose(stopPurpose(action.getStopLossSettings()))
                    .stopLossPrice(resolveStopLoss(action.getStopLossSettings(), anchor, direction, context));
            case TAKE_PROFIT, PARTIAL_TAKE_PROFIT -> builder
                    .purpose(StrategyPricePurpose.TAKE_PROFIT_TRIGGER_PRICE)
                    .takeProfitPrice(resolveTakeProfit(action, anchor, direction, context));
            ...
            case TRAILING_PERCENTS, TRAILING_VALUE -> builder
                    ...
                    .trailingPrice(resolveTrailing(action, anchor, direction, context));
        }
        return builder.build();
    }

    /**
     * Безубыток отличается от прочих уровней остановки убытка НАЗНАЧЕНИЕМ,
     * а не только числом: имя «безубыток» обещает ноль, и различимость
     * этого в данных — требование docs/concept.md П3.
     */
    private StrategyPricePurpose stopPurpose(StopLossSettings settings) {
        return nonNull(settings) && StopLossCalculationType.BREAKEVEN.equals(settings.getCalculationType())
                ? StrategyPricePurpose.BREAKEVEN_PRICE
                : StrategyPricePurpose.STOP_LOSS_TRIGGER_PRICE;
    }

    private ResolvedStopLossPrice resolveStopLoss(StopLossSettings settings, BigDecimal anchor,
        ...
        BigDecimal trigger = switch (settings.getCalculationType()) {
            case ENTRY_PRICE_PERCENT -> applyDistance(anchor, percentOf(anchor,
                    requireDistance(settings.getDistancePercents())), isLong, false);
            case ATR_PERCENT -> applyDistance(anchor, percentOf(atrValue(settings, context),
                    requireDistance(settings.getDistancePercents())), isLong, false);
            case MARKET_STRUCTURE_BUFFER_PERCENT -> structureStop(settings, isLong, context);
            case BREAKEVEN -> breakevenLevel(anchor, isLong, context);
        };
        return ResolvedStopLossPrice.builder()
                .triggerPrice(requirePositive(roundToTick(trigger, context, conservativeMode(trigger, anchor))))
     ...
     * нулевого итога следует S = P(1 + f)/(1 - f) для длинной позиции и
     * S = P(1 - f)/(1 + f) для короткой (docs/spec/stop-distance.json,
    ...
    private ResolvedTakeProfitPrice resolveTakeProfit(StrategyAlgoOrderAction action, BigDecimal anchor,
                                                      StrategyTradeDirection direction, CalculationContext context) {
        boolean isLong = StrategyTradeDirection.LONG.equals(direction);
        BigDecimal distance = percentOf(anchor, requireDistance(action.getTriggerProfitPercents()));
        BigDecimal trigger = applyDistance(anchor, distance, isLong, true);
    ...
    private ResolvedTrailingPrice resolveTrailing(StrategyAlgoOrderAction action, BigDecimal anchor,
                                                  StrategyTradeDirection direction, CalculationContext context) {
        TrailingSettings settings = action.getTrailingSettings();
        if (isNull(settings)) {
        ...
        if (nonNull(settings.getActivationProfitPercents())) {
            ...
            BigDecimal distance = percentOf(anchor, activationPercent);
            RoundingMode mode = isLong ? RoundingMode.UP : RoundingMode.DOWN;
            activationPrice = requirePositive(
                    roundToTick(applyDistance(anchor, distance, isLong, true), context, mode));
        }
     ...
     * наблюдена — расчёт ОТКАЗЫВАЕТ: откат к плановой был бы благоприятным
     * умолчанием на непредъявленном факте.
     */
    ...
    /**
     * <b>Марк- и индексная цена — отказ, а не подстановка последней.</b>
     * Их в снапшоте рыночных цен нет вовсе, а подстановка соседней цены
     * меняет ценовой домен молча: объявленный уровень «процент от
     * марк-цены» встал бы от последней и разошёлся бы с намерением автора
     * ровно на величину базиса.
     */
    private BigDecimal marketPriceBySource(StrategyPricePlacement placement, CalculationContext context) {
        ...
        BigDecimal resolved = switch (placement.getPriceSource()) {
            ...
            case LAST_PRICE -> priceData.getExternalLastPrice();
            case MARK_PRICE, INDEX_PRICE -> throw error(PRICE_SOURCE_UNAVAILABLE,
