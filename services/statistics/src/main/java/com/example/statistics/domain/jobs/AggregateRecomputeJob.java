...
/**
 * Пересчитывает агрегаты статистики из СВОИХ фактов скользящим окном последних
 * суток (docs/rules/statistics-aggregates.md §«Пересчёт — проекция, а не
 * накопитель»).
 *
 * <p><b>Строка агрегата — проекция, а не накопитель:</b> она
 * пересчитывается из фактов целиком и событием не двигается. Довод тот
 ...
 * сутки, а не один запрос на всё окно: окно, расширенное до всего ряда,
 * дало бы диапазонный скан по таблице, которая в {@code prod} не чистится
 * и растёт без предела (.claude/rules/codestyle.md §«Выборка данных: не тянем
