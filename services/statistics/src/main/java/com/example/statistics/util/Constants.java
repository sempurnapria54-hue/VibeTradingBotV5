...
public class Constants {
     ...
     * заполняются ключи сделочного зерна.
     *
     ...
     * идентичности, а не все» — дом механизма у владельца журнала).
     *
     * <p><b>Имён здесь ДВА, а не четыре, и это не усечение перечня:</b>
     * ключи зерна у статистики — тенант, биржевой счёт и определение
     * стратегии, причём тенант приезжает ключом записи, а не содержимым;
     * инструмента и сделки ни одно зерно ключом не держит
     * (docs/models/domain/other/StatisticsFact.md §«Факт происшествия»,
     * §«Сделочный факт»). Константа без читателя была бы формой раньше
     * предмета (.claude/rules/design-simplicity.md).
     */
    @UtilityClass
     ...
     * (deploy/base/services/statistics.yaml), и переименование
     ...
     * нет — оживитель назван секцией бэклога (.claude/work/backlog.md,
     * §«Слова провода, по которым ветвится потребитель, у четырёх
     * компонентов терминала не запинены»).
     */
    @UtilityClass
     ...
     * нет; оживитель — секция бэклога (.claude/work/backlog.md,
     * §«Слова провода, по которым ветвится потребитель, у четырёх
     * компонентов терминала не запинены»).
     */
    @UtilityClass
