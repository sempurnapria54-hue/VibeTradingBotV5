 ...
 * терминала.
 *
 * <p>Исполнимые формы: торговый исход —
 * docs/spec/position-close-outcome.json, сверка —
 * docs/spec/pnl-reconciliation.json, доступность знаменателя —
 * docs/spec/deal-lifecycle.json §benchmarkAvailabilityOnTerminal; полнота
 * разбивки — окно добычи против глубины доступности движений
...
public class DealTerminalFeaturesWriter {
    ...
    public DealTerminalFeatures apply(DealContext dealContext, Boolean resultFinalized) {
        ...
        verifyCloseRecordCurrency(dealContext);
        return features;
    }

    /**
     * Контроль валюты, в которой площадка посчитала итог эпизода: она
     * сверяется с расчётной валютой инструмента — единственным авторитетом
     * валюты результата — при чтении положений закрытия, до сложения
     * net'ов (docs/rules/pnl-reconciliation.md §«Проверка валюты чисел
     * записей закрытия»).
     *
     * <p><b>Исходов три, а не два.</b> Пустой правый операнд даёт свой
     * исход: контроль НЕ ПРОВЕДЁН. Без этой ветви «не проверяли»
     * становится неотличимо от «проверили, всё в порядке» — разрешающая
     * ошибка.
     *
     * <p><b>Терминал контролем не блокируется:</b> реакция отчётная.
     * Пересчитать итог по курсу нечем — механики курса у положения
     * закрытия нет, — а блокировка отправила бы возможно штатную популяцию
     * на тропу повторов, где повтор бессилен: факт стабилен, а не недобыт.
     *
     * <p><b>Строка одна на ребро, а не на эпизод.</b> Происшествие здесь —
     * само терминальное ребро: правый операнд у эпизодов один, разбор
     * начинается с него же, и вторая строка того же кода к разбору ничего
     * не добавляет.
     */
    private void verifyCloseRecordCurrency(DealContext dealContext) {
        List<Position> fetched = emptyIfNull(dealContext.getDeal().getPositions()).stream()
        ...
        if (isBlank(settleCurrency)) {
            journal(dealContext, Constants.Hold.RESULT_CURRENCY_UNVERIFIABLE);
            return;
        }
        boolean mismatched = fetched.stream()
                .anyMatch(episode -> isFalse(Objects.equals(settleCurrency,
                        episode.getExternalResultCurrency())));
        if (mismatched) {
            journal(dealContext, Constants.Hold.RESULT_CURRENCY_MISMATCH);
        }
    }
    ...
    /**
     * Накрыло ли окно добычи всю жизнь сделки: возраст нижней границы окна
     * на момент добычи против глубины доступности движений у источника.
     * Добыча не выполнялась — сравнивать нечего, и это единственный
     * триггер значения «не оценивалось».
     *
    ...
    private Deal.BreakdownCompleteness breakdownCompleteness(DealContext dealContext) {
        ...
        if (isNull(fetchedThrough) || isNull(lowerBound)) {
