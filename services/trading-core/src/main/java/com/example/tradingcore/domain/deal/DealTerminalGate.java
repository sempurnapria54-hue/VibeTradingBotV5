 ...
 * Исполнимая форма — docs/spec/deal-lifecycle.json §riskProvenAbsent; этот
 * класс её выражает, при расхождении верна спека.
 *
 * <p><b>Носитель один на обоих читателей</b> — терминальные звенья, что
 * ребро применяют, и машина сделки, что переход разрешает. Свой,
 * упрощённый предикат живого риска у звена был бы вторым носителем
 * инварианта и разошёлся бы с первым в разрешающую сторону: он не видит
 * ни сверки экспозиции, ни полноты графа.
 *
 * <p><b>Гейт вычисляется ВСЕГДА на состоянии «живого эпизода нет»</b> — он
 * и стои́т в конъюнкции с ним, — поэтому каждый его операнд обязан быть
 ...
 * либо не дал движения — самая частая тропа) либо строка закрыта и
 * доживает с НЕОБНУЛЁННЫМ размером (внешний размер пишется только из
 * снимка, при закрытии снимка нет, обнулителя поля в системе нет ни
 * одного).
 *
 * <p>Контракты результата (штатный и аварийный) сюда не входят: их
 * операнды — поля финализации, и читает их само терминальное звено,
 * которое их и пишет.
 */
@Slf4j
@Service
