...
/**
 * Строка исполнения СТРАТЕГИЙНОГО действия (таблица
 * {@code deal_strategy_action_states}).
 *
 * <p>Ссылка на узел стратегии обязательна — вид действия кодируется
 * таблицей, колонки рода нет. Цель — две скалярные колонки, а не навес:
 * тип цели входит в ключ уникальности. Последняя ошибка — навес JSONB;
 * скаляры повторов — обычные колонки
 * (docs/models/domain/other/DealActionState.md §Инварианты).
 *
 * <p><b>Идентичность обеих таблиц берётся из ОДНОЙ последовательности:</b>
 * анкер команды однозначен одним числом, и второго поля в команде не
...
public class DealStrategyActionStateEntity extends AuditableEntity {
    ...
    @SequenceGenerator(name = "dealActionStateSequence", sequenceName = "deal_action_state_seq",
            allocationSize = 1)
    private Long id;
