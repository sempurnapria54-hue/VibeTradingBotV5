...
/**
 * Шаг объявления (таблица strategy_steps).
 *
 * <p><b>Уровень объявления читается по родителю строки, а не по типу
 * шага.</b> Потраншевый шаг ссылается на объявление транша и несёт статус
 * транша ключом группировки; шаг узкой агрегатной поверхности ссылается
 * на деталь и несёт статус сделки. Ровно одна из двух пар непуста —
 * инвариант держит ограничение схемы, в домене обе карты пересобирает
 * маппер.
 *
 * <p>Условие шага и политика устаревания — навес JSONB на этой строке;
 * действия — дочерние строки.
 */
@Getter
@Setter
@Entity
