 ...
 * системные исполнения сделки (docs/components/SystemActionExecutor.md).
 *
 * <p><b>Точек входа две.</b> Первая отвечает про одно НАЗВАННОЕ действие;
 * вторая ревизует ВСЕ живые системные исполнения. Вторая нужна потому, что
 * закрытие неактуального исполнения по определению происходит там, где
 * действие больше никто не называет, — то есть первая до него не доходит.
 *
...
public class SystemActionExecutor {
    ...
    /**
     * Ревизия живых системных исполнений: исполнение, чья надобность снята
     * фактами сделки, закрывается как неактуальное. Иначе живая строка
     * держала бы частичный ключ и не давала завести новую, а её бюджет
     * тратился бы на надобность, которой больше нет.
     */
     ...
     * одном случае: повтор ещё ждёт отката.
     */
    private Optional<DealActionState> anchor(SystemActionType type, DealContext dealContext,
                                             DealTranche tranche) {
     ...
     */
    private DealActionState createPlanned(SystemActionType type, DealContext dealContext,
                                          DealTranche tranche) {
     ...
     * эмитится только после того, как отработало предшествующее: у штатной
     * тропы — расчёт числа перед терминальным ребром, у аварийной — вход в
     * ошибочное состояние перед аварийным терминалом.
     */
    private Optional<SystemActionLink> derivedLink(SystemActionType type, DealContext dealContext,
        ...
        return switch (type) {
            ...
            case REFRESH_DEAL_CONTEXT_ACTION -> fetchLink(deal);
        };
    }

    /**
     * Консолидация входа надобна, пока транш стои́т в отправленном входе:
     * статус подтверждённого входа пишет само звено, и по нему же
     ...
     * читает (docs/models/domain/aggregate/Deal.md §Структура).
     */
    private Optional<SystemActionLink> fetchLink(Deal deal) {
    ...
    private Order firstLiveOrder(Deal deal) {
        List<Order> live = emptyIfNull(deal.getTranches()).stream()
    ...
    private AlgoOrder firstLiveAlgoOrder(Deal deal) {
        List<AlgoOrder> live = emptyIfNull(deal.getTranches()).stream()
    ...
    private Boolean stale(DealActionState state, DealContext dealContext) {
        ...
        return isTrue(tranche.isTerminal())
                || isFalse(Objects.equals(tranche.getEpisodeSeq(), state.getTrancheEpisodeSeq()));
    }

    private DealTranche trancheOf(Deal deal, Long trancheId) {
        return emptyIfNull(deal.getTranches()).stream()
                .filter(candidate -> Objects.equals(trancheId, candidate.getId()))
                .findFirst()
                .orElse(null);
    }
