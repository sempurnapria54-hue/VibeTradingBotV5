...
public class StrategyDefinitionConsumer {
    ...
    private void apply(String eventId, StrategyEventType type, String payload) {
        switch (type) {
            case STRATEGY_ACTIVATED -> applier.applyActivated(eventId,
                    read(payload, StrategyActivatedMessage.class).definition());
            case STRATEGY_DEACTIVATED, STRATEGY_DELETED -> applier.applyLifecycle(eventId, type,
                    read(payload, StrategyLifecycleMessage.class).strategyInternalId());
        }
    }
