 ...
 *
 * <p><b>Предмет — направление ошибки.</b> ЭТОЙ командой база пишется
 * только из пустоты и только на строго положительном остатке; всякий иной
 * исход оставляет её пустой, а пустая база отвергает risk-creating
 * действие. Проверяется, что
...
class BalanceRefreshTest {
     ...
     * запись однократная и только из пустоты.
     *
     * <p><b>Предмет пробы — эта команда, а не база вообще.</b> База
     * следует за свободным остатком <b>в обе стороны</b>, и двигают её
     * терминальные исполнители после коммита терминала
     * (docs/rules/risk-policy.md §«Носитель базы и её писатели»);
     * приземление снимка среди её писателей — только первое наблюдение.
     */
    @Test
