 ...
 *
 * <p><b>Зачем она есть.</b> Клетка {@code B9.5} утверждает о ДВУХ тиках
 * подряд: на первом брокер отказывает и строки остаются непомеченными, на
 * втором — те же строки уходят. Адрес брокера приезжает конфигурацией, то
 * есть фиксируется подъёмом контекста; без перекрываемой тропы вторая
 * половина ожидания не измерялась бы вовсе, а клетка предъявляла бы
 * половину как целое.
 *
 * <p><b>Отказ ставится ЗАКРЫТИЕМ соединения, а не отсутствием слушателя.</b>
 * Порт держится занятым всю жизнь прогона: снимать и возвращать привязку
 * значило бы менять адрес, который сервис уже прочитал.
 *
 ...
 * ({@code max.block.ms}), величиной конфигурации сервиса он не объявлен, и
 * тик против закрытого гейта стои́т этого потолка целиком. Клетка поэтому
 * ждёт своего тика дольше штатного.
 */
