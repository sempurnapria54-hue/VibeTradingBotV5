...
`AG3.4`, `AG6.1`, `AG6.2`, `MG7.5`, `M17.5`) — из них семь дали факт
целиком, три частично (`AG1.7` — п. 14 да, п. 7 нет; `AG1.8` — событие не
наступило; `M17.5` — C17a да, C17b/C17c недостижимы). Негейтящие семь
...
### M17.5 Содержательный (шаг 7, A8 `DOCS_CHECK_24`) — судьба встроенной защиты при отмене родителя ◐ RUN 2026-08-30 — C17a наблюдён, C17b/C17c недостижимы — **ГЕЙТИТ `CODE`** (предусловие п. 17)
...
- **C17b и C17c — недостижимы под потолком риска.** Потолок 200 USDT на
  `ETH-USDT-SWAP` эквивалентен **0.81 контракта**, а объём лучшего уровня
  книги demo — десятки и тысячи контрактов (наблюдено 55.6, 14.82,
  3043.85). Любая заявка под потолком наливается **целиком**: две попытки
  C17b пропущены по правилу «объём лучшего аска больше половины потолка»,
  третья выродилась в `filled` (книга сменилась между чтением и
  постановкой); C17c недостижим по тому же счёту. Три попытки исчерпаны —
  кейс остаётся **`PENDING`**, предусловие п. 17 не закрыто.
...
### AG1.8 Содержательный (шаг 7, H27) — след автоделевериджа в bills ⏳ PENDING — прогнан 2026-08-30, событие не наступило — **ГЕЙТИТ `CODE`** (предусловие п. 10)
...
заказывается. По редакции §«Исходы содержательного кейса» от 2026-08-30
исход «не наступило» гейт **не закрывает**, поэтому предусловие п. 10
остаётся открытым.
...
подряд под общим `posId=3670806710316703744`. Адресуемая единица —
**пара (`posId`, `cTime`)**. Отсюда правятся оба носителя, названные ниже:
