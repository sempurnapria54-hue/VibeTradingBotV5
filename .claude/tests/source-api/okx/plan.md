...
которое биржа не обязана произвести (сегодня это п. 10 реестра — след
автоделевериджа). Снятие гейтящего статуса у условно наблюдаемых пунктов
— отдельная развилка держателя, этой редакцией она не решается.
...
### AG6.2 Содержательный (шаг 7, H10) — перечень типов вне экономики сделки ✅ RUN 2026-08-30 — **ГЕЙТИТ `CODE`** (предусловие п. 2)
