...
торгового bill'а и отдельной записи не порождает. Отсюда обе половины
находки: контроль **не** мёртв (движения маржи Σ|amount| не раздувают), а
список исключений непуст по построению — 177 пар справочника из 179 в
...
- **Статус:** ⏳ **PENDING** — прогон 2026-08-30 состоялся, но исход-**перечень** остался в логе: наблюдалась мощность, а слоту нужен состав. Кейс правлен — пишет перечень в `observations/AG6_2.md`; слот п. 2 закрывается перепрогоном. Провенанс — H10 `DOCS_CHECK_11`.
