...
- **Инвариант восстановления состояния кейса.** Каждый **stateful-кейс**
  ...
  **Verify.end** (последний шаг после restore/teardown: ассерт «конец ==
  ...
  (`leverage`/`posMode`/`acctLv`) на demo допущены; если demo не даёт
  переключить/вернуть — **находка** (реальное поведение + флаг
  остаточного состояния), не тихий мусор.
...
- **Объект:** любой приватный вызов сырого клиента (напр. `getBalance`)
  ...
  **Тип:** probe бага I3 (auth-негатив). **Не в коллекции и не через
  `/raw`** — требует изолированной конфигурации без кредов (поднятый app
  креды имеет); проверяется код-тестом / ручным бутом.
