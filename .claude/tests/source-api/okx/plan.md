...
- **Статус:** ⏳ **PENDING — до `CODE` шага 7** (гоняется вместе с §AG1.5).
  ...
  сделка); вторая половина — B11 `DOCS_CHECK_20`.
...
### MG9.5 Содержательный (шаг 7, C10 / N1 `DOCS_CHECK_21`) — базис `last` ↔ `mark` ⏳ PENDING — не гейтит
...
величина не калибруется, и безопасный выбор автора стратегии — `MARK`.
