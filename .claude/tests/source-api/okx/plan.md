...
- **(1) Горизонт `fundingFee` (предусловие п. 7) — НЕ наблюдён:** границу
  ...
  что нужно — удержание позиции через 00:00 / 08:00 / 16:00 UTC.
...
| Запрос | Проверки (сырой JSON OKX) | Ожидаемый результат | Факт + наблюдения (RUN) |
|---|---|---|---|
| `POST /raw {method:GET, path:/api/v5/account/subtypes, signed:true}` | HTTP 200; `b.code="0"`; `b.data` — массив непустой; `b.data[0]` несёт пары type/subType | Справочник bill types: непустой перечень. Включает funding-подтипы `173`(expense)/`174`(income). Точная форма элемента — наблюдение (сверка с офдоком при RUN). **Наблюдение записывается перечнем, а не мощностью** — см. клаузу ниже | RUN 2026-08-30 ✓ — **перечень сохранён построчно**: элемент — `{type, typeDesc, subTypeDetails:[{subType, subTypeDesc}]}`, **32 верхних типа и 179 пар** `type`/`subType`. Прежнее «data.size=32» (RUN 2026-06-20) было счётом **верхних типов**, а не пар — то есть измеряло не ту величину. Ожидание кейса подтверждено: funding-подтипы `173` (Funding fee expense) и `174` (Funding fee income) присутствуют под типом `8` (Funding rate) |
