...
`AG3.4`, `AG6.1`, `AG6.2`, `MG7.5`, `M17.5`) — из них восемь дали факт
целиком, два частично (`AG1.7` — п. 14 да, п. 7 нет; `AG1.8` — событие не
наступило, гейтящий статус пункта снят решением держателя 2026-08-30,
кейс закрывается заменителями). Негейтящие семь
(`M1.7`, `M15.7`, `AG3.5`, `AG3.6`, `AG12.4`, `AG12.5`, `MG9.5`) остаются
`PENDING`. Состояние слотов гейта — реестр
`.claude/tests/source-api/okx/code-preconditions.md`. Прежняя редакция
...
### AG3.4 Содержательный (шаг 7, H8 / RQ-4) — комиссии приходят в settle-ccy ◐ RUN 2026-08-30 — инвариант settle-ccy подтверждён — **ГЕЙТИТ `CODE`** (предусловие п. 9 — вторая проверка)
