...
- **Почему не гейтит доки:** маппинг взят **безопасным суперсетом** — native
  `fee` в used, `TRADE_FEE`/`REBATE` резолвятся по нему, `balChg` остаётся под
  сумму-сверку (`docs/models/mapping/DealCashFlow.md` §«Разделение ролей
  `balChg` и `fee`»). Суперсет корректен **при любой** гранулярности, поэтому
  ответ на этот вопрос **не меняет** маппинг — он лишь говорит, какая ветка
  фактически работает и нужен ли вычет `amount` − `externalFee` для
  realized-слагаемого.
- **Фикстура — та же §AG1.5** (закрытая позиция + bills за окно), лишнего
  прогона не создаёт.
