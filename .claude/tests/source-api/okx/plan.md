...
- **Дополнение вопросника (H24, `GAPS_CLOSE_7`) — метка entry-fee против
  `cTime` позиции.** Нижняя граница окна линковки bills — `begin` =
  `Position.externalCreatedAt` (= `cTime` открытия позиции). Если биржа
  штампует комиссию **входа** временем **раньше** создания позиции
  (`ts(entry-fee) < cTime(position)`), входная нога комиссии выпадает из
  окна на **каждой** сделке: расхождение сверки при этом сверх допуска
  (крупная сделка: notional 10 000, taker 0.05% ⇒ fee 5.0 против допуска
  max(0.01; 0.5%·Σ|amount| ≈ 2.5); мелкая: fee 0.05 против 0.025), то есть
  аномалия на каждой сделке и заниженная комиссия в разбивке при **верном**
  `resultProfit`.
  - **Проверка (нулевая цена — фикстура та же):** на bills фикстуры найти
    запись комиссии входа и сравнить её `ts` с `cTime` записи позиции.
  - **Ожидание:** `ts(entry-fee) ≥ cTime(position)` — граница включительная,
    зазор не нужен.
  - **Действие при отрицательном ответе (дефолт зафиксирован заранее):**
    `begin` = `externalCreatedAt` **первого отправленного `Order`** —
    **всегда**, а не только при отсутствии позиции. Окно шире, но лишних
    движений оно не захватывает: активная сделка на инструмент одна
    (`docs/models/domain/other/DealCashFlow.md` §«Линковка к `Deal`»).
  - **Статус:** ⏳ **PENDING** вместе с §AG1.5. Провенанс — H24 отчёта
    `phase-1-step-7-docs-check-7.md`.

### AG1.6 Содержательный (шаг 7, H5) — оси адресации записи без `posId` ⏳ PENDING — **ГЕЙТИТ `CODE`**
...
### AG1.7 Содержательный (шаг 7, H20) — семантика `fundingFee` записи ⏳ PENDING

`fundingFee` возвращён в used-набор (H20 `DOCS_CHECK_11`) как
авторитетный операнд де-микширования R-мультипликатора
(`docs/decisions/per-trade-risk-policy.md` §H25).

- **Что верифицировать:** накоплен ли `fundingFee` финализированной
  записи **за всю жизнь `posId`** или только **за последнее закрытие**.
...
- **Статус:** ⏳ **PENDING**. Провенанс — H20 `DOCS_CHECK_11`.
...
### AG6.1 Прямой — справочник типов bills
...
### AG6.2 Содержательный (шаг 7, H10) — перечень типов вне экономики сделки ⏳ PENDING — **ГЕЙТИТ `CODE`**
...
### MG7.5 Содержательный (шаг 7, H25) — носитель курса cross-ccy ⏳ PENDING — **ГЕЙТИТ `CODE`**
...
| Запрос | Проверки | Ожидаемый результат | Факт + наблюдения (RUN) |
|---|---|---|---|
| приватный вызов на пустых кредах | исключение до сети; `IllegalStateException`, сообщение содержит «OKX credentials not configured» | **I3 closed (`backlog` §I3):** fail-fast внятной `IllegalStateException` «OKX credentials not configured» до подписи и сети — не голый NPE (`getSecret().getBytes()` на `null`), сети не достигает | RUN 2026-06-20 ✓ — I3 closed: `IllegalStateException` «OKX credentials not configured» до сети (offline-юнит, `validate-icred-01.log`) |
