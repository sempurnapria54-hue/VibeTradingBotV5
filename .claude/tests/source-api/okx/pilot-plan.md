...
## Статус — СНЯТ (прокси-граничный пилот)

**Снят ре-базой контура на сырьё**
(`.claude/decisions/source-api-target-rebase.md`). Этот пилот бил в
**mapped-границу** (`OkxProxyController` → снапшоты/`ExchangeAck`) —
цель контура ушла на **сырьё** (сырой клиент + сырые DTO). Пилот не
актуальная методология; держится как трасса миграции до регенерации.

- **Дизайн-контент мигрирован** в
  `.claude/work/progress/source-api-rebase.md` (кейсы, цепочки
  предусловий, закрытая нога amend; surface-gap'ы прокси —
  растворяются).
- **Prod-блок (P1-P4) не переносится** — prod вне контура.
- **Регенерация** под сырьё (полный периметр манифеста ∩ сырой клиент,
  прямой кейс + порождённый негатив, гейт достижимости) — отдельным
  заходом по процессу `.claude/processes/source-api-testing.md`;
  заместит этот файл и `pilot-collection`/`pilot-environment`.

Ниже — историческое содержимое прокси-граничного пилота (не источник
методологии).
