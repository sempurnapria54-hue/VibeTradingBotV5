...
`.claude/work/progress/phase-1-step-7-gaps-close-27/node-H8.md`
...
(`.claude/work/progress/corpus-rewrite-mapping.md`) снятия таблиц не
...
| Fills 3d | GET `/trade/fills` | есть-док | ⚪ не-runtime | офдок | `fills.md`, `OkxFillResponse`; **`REFRESH_FILLS` снимается** на `CODE` шага 7 (`pnl-finalization-mechanics.md`; в коде команда пока жива) ⇒ эндпоинт в целевом runtime фазы 1 не используется (order-fill-метрики из `OkxOrderResponse`); контракт справочно |
...
| Bills 7d | GET `/account/bills` | есть-док | 🟡 в плане | офдок | `account-bills.md`, `OkxAccountBillResponse`; **:** команда **`REFRESH_BILLS_COMMAND`** → `DealCashFlow` (разбивка P&L; `DealCashFlow.md`), линковка по **окну + `instId`**, дедуп по паре (`exchangeId`, `billId`); метода клиента нет. Откат 🟢→🟡: три непокрытых кейса — `AG3.4` (**гейтит `CODE`**, п. 9), `AG3.5`, `AG3.6` |
