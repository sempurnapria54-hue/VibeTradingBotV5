...
- Не индексируются: содержимое `work/history/` и
  ...
  **содержимое** деревьев кода и манифестов — `donor/`, `services/`,
  `web/`, `deploy/`. У каждого такого дерева индексируется
  ровно один файл — его `README.md`-указатель: он отвечает вопросом
  («что кладётся в этот каталог»), а код — прогоном.
...
├── README.md — С чего начать, впервые открыв репозиторий?
...
│   │   └── 2026-08-26-step-7-question-flow-analysis.md — Какие источники дали поток вопросов к пользователю на шаге 7 фазы 1?
...
│   │   ├── roadmap-step-execution.md — Как устроено исполнение одного шага роадмапа?
...
│   │   ├── backlog-section-form.md — Какое правило формы секции `.claude/work/backlog.md`?
...
│   │   ├── edit-kind-obligations.md — Какое правило обязанностей полноты у правки корпуса?
│   │   ├── external-source-sync.md — Какое правило синхронизации файлов с внешним источником правды?
│   │   ├── handoff-worklist.md — Какое правило перечня работ, адресованного чужому заходу?
│   │   ├── knowledge-ownership-by-service.md — Какое правило второй оси размещения (какой сервис владеет носителем)?
│   │   ├── measurement-commands.md — Какое правило проверочных команд корпуса?
...
│   │   ├── self-description-form.md — Какое правило формы числа и исхода,
│   │   │       которые носитель пишет о собственном предмете?
...
│   │   ├── concept-review.md — Как проверить концепцию шага на достаточность для старта кода?
...
│   │   ├── local-stand.md — Как поднять, вести и снять локальный стенд платформы?
...
│   │   ├── session-chain.md — Как гнать роадмап цепочкой неинтерактивных сессий без держателя?
│   │   ├── test-code.md — Как написать код тестов по утверждённым кейсам?
│   │   ├── test-collection.md — Как построить исполняемую тест-коллекцию (Postman)?
│   │   ├── test-design.md — Как описать тест-кейсы предмета без кода?
│   │   ├── test-review.md — Как сделать адверсариальное ревью тест-кейсов, кода тестов и отчёта прогона?
...
│   │   └── snapshot-vN.md — Где мы сейчас? (в папке лежит ровно один — актуальный;
│   │           номер здесь не пишется: он стареет с каждым снапшотом, а читается
│   │           каталогом — `.claude/rules/self-description-form.md`. Старые — в
...
│   │       ├── review-report.md — Каков формат отчёта фокуса ревью (поля находки, принципы верификации)?
│   │       └── test-plan.md — Каков формат документа тест-кейсов (план и бланк отчёта прогона)?
│   ├── tests/ — Как проверяем API источника?
│   │   └── source-api/okx/
│   │       ├── plan.md — Как проверяем API OKX на уровне сырья?
...
│       ├── progress/ — На каком шаге мы в этой активной операции? (лежит хроника шага 12 фазы 2)
...
│       │   ├── phase-1.md — В каком статусе каждый шаг Фазы 1? (растворена, `FOLDED`)
│       │   ├── phase-2.md — В каком статусе каждый шаг Фазы 2?
│       │   ├── phase-3.md — В каком статусе каждый шаг Фазы 3?
│       │   ├── phase-4.md — В каком статусе каждый шаг Фазы 4?
│       │   ├── phase-5.md — В каком статусе каждый шаг Фазы 5?
│       │   ├── phase-6.md — В каком статусе каждый шаг Фазы 6?
│       │   └── prod-gate.md — Какие условия гейтят первый прод-запуск?
...
│   ├── architecture/ — Из каких сервисов состоит система и как они делят данные и контракты?
│   │   ├── contracts.md — Как сервисы обращаются друг к другу?
│   │   ├── data-ownership.md — Кто владеет какими данными и где они хранятся?
│   │   ├── market-data-collection.md — Какие рыночные данные собираются и по какому правилу?
│   │   ├── platform.md — На чём система развёрнута и как наблюдается?
│   │   ├── reserved-extension-points.md — Где зарезервированы места под отложенное?
│   │   ├── services.md — Из каких сервисов состоит система и где проходят их границы?
│   │   ├── services/ — Какую работу несёт этот сервис?
│   │   │   ├── audit.md — Какую работу несёт сервис `audit`?
│   │   │   ├── auth.md — Какую работу несёт сервис `auth`?
│   │   │   ├── bff.md — Какую работу несёт сервис `bff`?
│   │   │   ├── connector-okx.md — Какую работу несёт сервис `connector-okx`?
│   │   │   ├── market-data.md — Какую работу несёт сервис `market-data`?
│   │   │   ├── statistics.md — Какую работу несёт сервис `statistics`?
│   │   │   ├── strategies.md — Какую работу несёт сервис `strategies`?
│   │   │   ├── trading-core.md — Какую работу несёт сервис `trading-core`?
│   │   │   └── web.md — Какую работу несёт единица `web`?
│   │   ├── signal-strategy-allocator.md — Как сигнал, стратегия и аллокатор делят решение о входе?
...
│   │   ├── ReceptionStateJob.md — Кто ведёт состояние приёма журнала?
...
│   │   │   │   ├── ExchangeAccount.md — Что это за сущность `ExchangeAccount`?
│   │   │   │   ├── Tenant.md — Что это за сущность `Tenant`?
...
│   │   │       ├── Membership.md — Что такое членство пользователя в тенанте?
│   │   │       ├── StatisticsFact.md — Что это за модель — факт статистики?
...
│   │   ├── api-access-policy.md — Какое правило определяет, кто вправе обращаться к поверхности системы?
...
│   │   ├── decimal-arithmetic.md — Какое правило действует при делении и округлении денежных величин?
│   │   ├── durable-consumer-reception.md — Какое правило действует для приёма события durable-потребителем?
...
│   │   ├── statistics-aggregates.md — Какое правило действует для агрегатов статистики?
...
│   │   ├── durable-reception.json — Что durable-потребитель объявляет читателю о полноте своего приёма?
│   │   ├── deal-condition.json — Чему равен предикат условия шага сделки?
...
│   │   ├── environment-contour.json — Допускает ли окружение платформы биржевой счёт этого контура?
│   │   ├── event-actor-presence.json — У каких классов событий актор едет содержимым?
│   │   ├── external-status-resolution.json — Во что резолвится сырой статус источника?
│   │   ├── hard-rung-shutdown-reason.json — Какую причину выхода из штатного ведения присваивает ребро энфорсмента жёсткой ступени?
...
├── donor/ — (Код монолита фазы 1; индексируется только указатель каталога.)
│   └── README.md — Что лежит в этом каталоге?
├── services/ — (Код единиц развёртывания и общих артефактов `common/*`; индексируется только указатель каталога.)
│   └── README.md — Что кладётся в этот каталог?
├── web/ — (Код SPA; индексируется только указатель каталога.)
│   └── README.md — Что кладётся в этот каталог?
├── deploy/ — (Манифесты окружений; индексируется только указатель каталога.)
│   └── README.md — Что кладётся в этот каталог?
