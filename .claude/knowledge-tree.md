...
- Не индексируются: содержимое `work/history/` и
  ...
  **содержимое** деревьев кода и манифестов — `donor/`, `services/`,
  `libs/`, `web/`, `deploy/`. У каждого такого дерева индексируется
...
│       ├── progress/ — На каком шаге мы в этой активной операции? (шаг 10 фазы 2 закрыт; лежат отчёт его перехода — см. бэклог, — отчёт межшагового окна 2-10 → 2-11 и отчёт закрепления семантики даты `history/`)
...
├── libs/ — (Код общих артефактов; индексируется только указатель каталога.)
│   └── README.md — Что кладётся в этот каталог?
