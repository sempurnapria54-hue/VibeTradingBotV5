...
│       ├── progress/ — На каком шаге мы в этой активной операции? (лежат хроника шага 12 фазы 2, отчёт ревью его концепции, ревизия существующего набора тестов, по каждому предмету отчёты ревью кейсов: у `auth` — отчёты петли, которую снял критерий апрува (адверсариальное ревью, правка автором, перепроверка — по отчёту на круг), у прошедших по критерию — отчёт апрува на предмет; плюс отчёт захода, закрывшего развилку перечня предметов уровня 2, и отчёты передачи автора на ревью по новым предметам уровня 2 — по отчёту на предмет)
