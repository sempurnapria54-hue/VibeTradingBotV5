...
| Kafka (оператор Strimzi) | несрочное взаимодействие и факты между сервисами; конверт события с `tenantId` ключом партиции (`docs/architecture/contracts.md`) | `trial` — клиент введён в сборку ядра шагом 7 фазы 2 (`spring-kafka`), тема `trading-core.facts` заведена манифестом, реле публикует синхронно с подтверждением всех синхронных реплик. **`adopt` — по первому живому прогону реле**: производитель написан, потребителя ни у одного класса ещё нет (аудит — шаг 10, `bff` — шаг 9), и до них форма проверяется чтением, а не прогоном. Реестр схем не заводится, пока все стороны в одном монорепозитории (дизайн-проход 2026-09-04) |
