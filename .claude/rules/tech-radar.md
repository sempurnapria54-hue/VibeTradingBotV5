...
| Raw-JDBC (`DataSource`/`Connection`) ради advisory-замка | Postgres advisory lock на **одном соединении** на весь проход оркестратора для мультиинстанс-сериализации (гейт D-M1). **Снят из фазы 1 (2026-07-01):** один экземпляр монолита — межэкземплярной конкуренции нет, оркестратор пользуется in-process `JobExecutionGuard` как остальные джобы; advisory-замок и его raw-JDBC (`OrchestratorPassLock`, удалён) **отложены на фазу 3** (несколько экземпляров конкурируют за проход), тогда и вернутся. Ограничение при возврате прежнее: raw-JDBC — только ради advisory-замка на одном соединении (JPA-репозиторий connection-pinning не гарантирует), не для обычного доступа к данным. | `hold` (снят из фазы 1; вернётся в фазу 3) |
