...
| `docs/components/` — четвёртого файла владельца `audit-statistics` нет, и это **решение** той же формы | пересчёт проекции держит `docs/rules/statistics-aggregates.md` §«Пересчёт — проекция, а не накопитель», поверхность чтения — `docs/models/domain/other/AuditRecord.md` §«Как журнал читается» (состав точек — `.claude/work/backlog.md` §«Состав точек чтения `audit-statistics` — по появлению их потребителя»). Условие возврата то же: исполнитель, чьё поведение ни одним домом не покрыто. **Строку catch-all ниже он не наследует:** без этой строки его доки достались бы `trading-core` умолчанием |
