...
| `docs/components/` — файлов владельца `audit-statistics` нет, и это **решение**, той же формы, что у `strategies` и у `bff` | поведение трёх его исполнителей уже имеет дом: приём и запись строки — `docs/models/domain/other/AuditRecord.md` §«Когда строка не пишется» и §«Позиция чтения и с какого момента журнал полон»; пересчёт проекции — `docs/rules/statistics-aggregates.md` §«Пересчёт — проекция, а не накопитель»; поверхность чтения — `docs/models/domain/other/AuditRecord.md` §«Как журнал читается» (состав точек — `.claude/work/backlog.md` §«Состав точек чтения `audit-statistics` — по появлению их потребителя»). Компонент-док стал бы ещё одним носителем того же поведения (`.claude/rules/policy-home.md`); условие возврата — исполнитель, чьё поведение ни одним домом не покрыто. **Строку catch-all ниже он не наследует:** без этой строки его доки достались бы `trading-core` умолчанием |
