...
| `docs/rules/durable-consumer-reception.md` | сквозное: форму свидетельства о полноте приёма читают все durable-потребители, и носитель одного из них домом такой истины не бывает (`.claude/rules/carrier-levels.md`). Заведён узлом 4 `GAPS_CLOSE_1` шага 11 фазы 2: прежде форма жила у журнала аудита с доводом «предмета вне журнала нет», и раздел `audit-statistics` этот довод снял. Экземпляры и их особенности — у владельцев следствия приёма (`docs/models/domain/other/AuditRecord.md`, `docs/models/domain/other/StatisticsFact.md`) |
...
| `docs/models/domain/other/AuditRecord.md`, `docs/models/domain/other/StatisticsFact.md`, `docs/rules/statistics-aggregates.md`, `docs/spec/statistics-aggregates.json` | `audit-statistics` |
...
| `docs/spec/audit-journal.json` | сквозное: это **исполнимая форма** тех же двух величин, чей дом сквозной (`docs/rules/durable-consumer-reception.md`), и довод у неё тот же — форму полноты приёма читают все durable-потребители, а носитель одной из сторон домом такой истины не бывает (`.claude/rules/carrier-levels.md`). Имя файла и имена двух её величин остались журнальными — названный остаток, переименование идёт кодовым ходом (`.claude/work/backlog.md` §«Дельта кода шага 11 фазы 2 по собственному свидетельству полноты у `statistics`») |
