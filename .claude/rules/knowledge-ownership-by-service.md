...
| `docs/spec/` | по владельцу величины: сайзинг, риск, сделки, транши, холды — `trading-core`; свежесть, фаза — `market-data`; стратегия — `strategies` / `common/strategy-engine`; статусы площадки — коннектор; полнота журнала — `audit-statistics`; **присутствие актора в содержимом — сквозное**: его дом `docs/architecture/contracts.md`, а предмет — правило, обязанное иметь исход на классе каждого из пяти производителей |
