...
- **Сервисный** носитель читает и правит один сервис; его `DOCS` и
  `REVIEW` идут на шаге, который этот сервис строит.
...
его: они — вход его `DOCS` и его порта
...
| `docs/concepts/` | по предмету дока: файл, названный именем единицы инвентаря, — эта единица; сквозная тема — сквозное. Поимённо строки не заводятся по тому же доводу, что у `docs/architecture/services/`: имя владельца выводится из имени файла. Сегодня каталог держит одну сквозную тему — `testing.md` |
| Файл каталога `docs/architecture/services/` | **владелец — та самая единица, чьё имя он носит**: файл отвечает на вопрос «какую работу несёт вот этот сервис» и межсервисной истины не держит (`.claude/rules/structure.md`, строка `docs/architecture/`). Поимённо строки не заводятся: имя владельца выводится из имени файла, и вторая его запись разошлась бы с первой при первом же переименовании единицы. Каталог накрыт кластером выше, и это **названное ограничение разметки**: прогон покрытие мерит, а верность владельца читает критик |
...
| `docs/rules/durable-consumer-reception.md` | сквозное: форму свидетельства о полноте приёма читают все durable-потребители, и носитель одного из них домом такой истины не бывает (`.claude/rules/carrier-levels.md`). Заведён шагом 11 фазы 2: прежде форма жила у журнала аудита с доводом «предмета вне журнала нет», и раздел на два сервиса этот довод снял. Экземпляры и их особенности — у владельцев следствия приёма (`docs/models/domain/other/AuditRecord.md`, `docs/models/domain/other/StatisticsFact.md`) |
...
| `docs/rules/`: `strategy-condition-contract`, `strategy-step-once-per-episode`, `condition-ruletype-granularity` | `common/strategy-engine` |
...
| `docs/models/domain/core/`: `Order`, `Position`, `AlgoOrder`, `BalanceContainer` | формы — `common/model/domain`; зеркало и писатели — `trading-core` |
...
| `docs/models/domain/core/Tenant.md` | форма — `common/model/domain`; данные и писатель — `auth` (`docs/architecture/data-ownership.md`) |
| `docs/models/domain/other/Membership.md` | `auth`: членство читает и пишет только он, а прочие сервисы получают из него уже резолвленный контекст тенанта и роль. В общий артефакт форма не уезжает — читателя на другой стороне провода у неё нет (`docs/architecture/contracts.md`) |
...
| `docs/models/domain/aggregate/Strategy.md` | делится: определение, его состав и валидация — `strategies`; копия в базе ядра, писатель её статуса и резолв идентичностей в числовые FK — `trading-core`; исполняемая форма — `common/strategy-engine`. Критерий — §«Где живёт определение и где — его копия» того же дока |
...
| `docs/models/domain/other/Auditable.md` | `common/model/domain` |
| `docs/models/domain/other/AuditRecord.md` | `audit` |
| `docs/models/domain/other/StatisticsFact.md`, `docs/rules/statistics-aggregates.md`, `docs/spec/statistics-aggregates.json` | `statistics` |
...
| `docs/spec/durable-reception.json` | сквозное: это **исполнимая форма** тех же двух величин, чей дом сквозной (`docs/rules/durable-consumer-reception.md`), и довод у неё тот же — форму полноты приёма читают все durable-потребители, а носитель одной из сторон домом такой истины не бывает (`.claude/rules/carrier-levels.md`). Имя файла и имена её величин **переименованы шагом 11 фазы 2** вместе с разъездом сервисов: `durable-reception.json`, `consequenceSeriesStart`, `receptionLowerBound` |
| `docs/components/ReceptionStateJob.md` | сквозное: тик состояния приёма есть форма исполнителя **любого** durable-потребителя с параметрами владельца (группа, темы, база, метка рядов), как реле у производителя. Экземпляров у него столько, сколько потребителей; второй записи формы не заводится (`.claude/rules/carrier-levels.md`) |
| `docs/components/` — компонент-дока слушателя приёма у `statistics` нет, и это **решение** той же формы, что у `strategies` и `bff` | поведение её слушателя уже имеет дом: следствие приёма и отметка обработанного — `docs/models/domain/other/StatisticsFact.md`, величины приёма и форма исполнителя — `docs/rules/durable-consumer-reception.md`, порядок фиксации смещения — `docs/architecture/data-ownership.md`. Компонент-док стал бы четвёртым носителем (`.claude/rules/policy-home.md`); условие возврата — исполнитель приёма, чьё поведение ни одним домом не покрыто |
| `docs/components/`: `AuditEventListener`, `JournalCleanupJob` | `audit`. **Прежде компонент-доков у него не было, и это тоже было решением** (Д1074): поведение его исполнителей держал док модели, а компонент-док стал бы вторым носителем. **Условие возврата, названное там же, сработало** — дизайн-проход по форме исполнителя приёма (`.claude/decisions/reception-executor-design-pass.md`) развёл безымянного «потребителя» на трёх исполнителей, чьё поведение ни одним домом не покрывалось: слушатель приёма, тик состояния приёма, чистка журнала. Что каждый **пишет** и с какой семантикой, по-прежнему живёт у владельца предмета (`docs/models/domain/other/AuditRecord.md`); компонент-док отвечает на «кто выполняет» и второго носителя семантики не заводит |
| `docs/components/` — иных файлов владельца `audit` в `docs/components/` нет, и это **решение** той же формы | пересчёт проекции держит `docs/rules/statistics-aggregates.md` §«Пересчёт — проекция, а не накопитель», поверхность чтения — дом у каждой её выборки свой: `docs/models/domain/other/AuditRecord.md` §«Как журнал читается» у журнальной, `docs/rules/statistics-aggregates.md` §«Что это за числа и кто их читает» у агрегатной (состав экранных операций — `.claude/work/backlog.md` §«Состав экранных операций чтения истории — по появлению их потребителя»). Тем же условием обходится и **писатель строки отказа доступа** (построен кодовым заходом K4 шага 10 фазы 2): что он пишет и с какими инвариантами — `docs/models/domain/other/AccessDenial.md`, кому писать положено — `docs/rules/api-access-policy.md` §«След отказа пишет тот, у кого есть база»; компонент-док стал бы третьим носителем одного предмета. Условие возврата то же: исполнитель, чьё поведение ни одним домом не покрыто. **Строку catch-all ниже он не наследует:** без этой строки его доки достались бы `trading-core` умолчанием |
| `docs/models/integrations/okx/`, `docs/models/mapping/`, `docs/integrations/okx/` | `connector-okx` |
| `docs/models/api/` | `bff`: формы, которые периметр **порождает** сам (событие потока, оболочка агрегата, собственный отказ), — его api-модель; пересылаемое идёт формой владельца, и второго её носителя не заводится. Признак и его довод — `docs/architecture/contracts.md` §«Периметр: что `bff` отдаёт и чего не делает». **Сырого вызова источника** (`OkxRawApiRequest`) в целевой конструкции нет вовсе: он был тест-обращённой поверхностью донора, а решение держателя 2026-09-16 (`.claude/decisions/source-api-contour-retired.md`) такой поверхности сервисам не заводит — работа с площадкой проверяется чёрным ящиком коннектора. Прежняя редакция строки отдавала его коннектору ходом удаления донора; ход состоялся, и отдавать оказалось нечего |
...
| `docs/components/`: `StrategyConditionEvaluator`, `StrategyActionCalculator`, `PriceCalculator`, `SizeCalculator` и их runtime-модели | `common/strategy-engine` |
...
| `docs/processes/strategy-action-calculation.md` | `common/strategy-engine` |
| `docs/spec/` | по владельцу величины: сайзинг, риск, сделки, транши, холды — `trading-core`; свежесть, фаза — `market-data`; стратегия — `strategies` / `common/strategy-engine`; статусы площадки — коннектор; агрегаты статистики — `statistics`, а форма полноты durable-приёма — сквозное (поимённая строка `docs/spec/durable-reception.json`); **присутствие актора в содержимом — сквозное**: его дом `docs/architecture/contracts.md`, а предмет — правило, обязанное иметь исход на классе каждого из пяти производителей |
| `.claude/` целиком, `tools/`, `README.md`, `CLAUDE.md` | пайплайн монорепозитория — вне оси |
| `.claude/tests/case-material/connector-okx.md` | пайплайн; **предмет** — `connector-okx` (материал для его кейсов принадлежит проверочной деятельности, не сервису) |
