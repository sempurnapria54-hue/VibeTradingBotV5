...
1. **Провенанс ликвидации/ADL наблюдается и выбрасывается** (H16). Тип
   закрытия приземляется на `Position.externalCloseType`, но `triggerPx` из
   маппинга **выведен** (потребителя нет — H22), а на
...
`docs/components/CreateOrderExecutor.md` §«Канал доставки», **`RISK-Q3`**.
