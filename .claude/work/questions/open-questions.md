...
### CCY-Q1. Механизм курса для не-settle-ccy движений в `resultProfit` (владелец — `integrator` + `solution-designer`)

**Политика закрыта, механизм — нет** (`GAPS_CLOSE_6`, H5). Ратифицировано:
итоговый `Deal.resultProfit` — **в settle-ccy (USDT)**; движения чужой `ccy`
персистятся, линкуются к сделке и входят в число **USDT-эквивалентом по
курсу на момент закрытия сделки** (вариант 1b)
(`docs/decisions/pnl-finalization-mechanics.md` реш.5,
`docs/models/mapping/DealCashFlow.md` §«Число — в settle-ccy»). Открыт
**механизм получения курса**.

Развилки (взаимно ортогональны, решаются вместе):

1. **Приходит ли готовый эквивалент от источника.** В инвентаре
   `OkxAccountBillResponse` поля settle-ccy-эквивалента **нет** (`balChg`,
   `fee`, `pnl` — в валюте записи `ccy`). Требуется **инспекция офдока**
   (`integrator`): нет ли на bill-записи поля пересчёта, и что несёт `px`
   на не-торговых subType. Если готовый эквивалент есть — остальные ветки
   отпадают.
2. **Источник котировки, если считаем сами.** Кандидаты в периметре:
   `GET /market/index-tickers` (index-цена пары, `contracts/index-data.md`),
   `GET /public/mark-price`, спотовый `GET /market/ticker`. Для
   **исторического** момента закрытия — `GET /market/index-candles` /
   `history-index-candles` (минутная гранулярность). Развилка: index vs
   mark vs спот-тикер — при cross-ccy-издержке величины близки, но
   источник надо назвать один и обосновать.
3. **Живой вызов vs исторический бар.** Финализация идёт вскоре после
   закрытия ⇒ живой тикер ≈ курс закрытия. Но это **вводит exchange-вызов
   в финализацию**, которая по контракту off-exchange
   (`docs/components/FinalizeDealExitExecutor.md`), и делает число
   зависимым от задержки. Исторический бар по `uTime` записи
   positions-history — детерминирован и воспроизводим, но это ещё один
   read-путь.
4. **Хранить ли применённый курс.** Отдельное поле на `DealCashFlow`
   (`appliedRate` + его источник/время) делает число **воспроизводимым** и
   разбираемым постфактум; без него курс не восстановить. Против —
   поле-навес ради редкого нарушения инварианта.

**Крен CC:** (1) сначала инспекция офдока — она может закрыть вопрос
целиком; при отрицательном ответе — исторический index-бар по `uTime`
(детерминизм важнее одного лишнего read-пути) + **хранить применённый
курс** полем (число, полученное расчётом по внешней котировке, обязано быть
воспроизводимым — тот же принцип, по которому отвергнут fallback-ставки:
подставленное число не должно выглядеть фактом, не будучи проверяемым).

**Не гейтит `CODE` целиком, но гейтит cross-ccy-ветку:** до решения число
считается по settle-ccy-строкам, а cross-ccy-движение персистится,
линкуется и помечается `AnomalyReport` (это уже задано). Слагаемое
пересчёта — за этим вопросом.
Связано: `docs/decisions/pnl-finalization-mechanics.md` реш.5,
