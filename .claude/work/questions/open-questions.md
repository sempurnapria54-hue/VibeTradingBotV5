...
`StrategyConditionRuleType.VOLUME_FILTER_PASSED` и индикатор OBV
опираются на биржевой объём. Wash trades — стандартный приём
манипуляции («paint the tape») [Harris, гл. 12, с. 259-260, 273-274];
крипто-специфика фейкового объёма (биржи сами заинтересованы, нет
единого регулятора) — пробел корпуса
(`.claude/library/trading/distilled/microstructure.md` §9). Находка
ТР4 `DOCS_CHECK_8` (торговый фокус `trading-review`).

Не решено: как учитывать манипулируемость объёма при расчёте и
использовании volume-индикаторов — фильтрация/сглаживание источника,
