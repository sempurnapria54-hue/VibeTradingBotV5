...
**Смежный операнд, зависящий от того же ответа (H10 `GAPS_CLOSE_13`):**
уровень стопа, под который считался `plannedRiskAmount`, читается с
**защиты той же входной ноги**. Пока защита входной ноги — attached SL
входного ордера либо алго-строка того же входа, операнд резолвится
структурно и трейлингом не сдвигается (трейлинг живёт на основной
standalone-защите). Если `RISK-Q4` разрешится так, что дом ноги входа
сменится, темпоральную привязку операнда придётся перепроверить —
необходимость правки от ответа не зависит, состав зависит.
...
### ANOM-Q5. Сколько акторов ведут процессные `STATE`-отчёты (владелец — `solution-designer`) — **ГЕЙТИТ `CODE` шага 7**

Хвост решения H17 `GAPS_CLOSE_13` (идемпотентность `STATE`-отчёта держится
**незавершённым статусом**, анкер-ключ снят). Поиск незавершённого отчёта и
его создание **не атомарны**: при более чем одном писателе два такта могут
завести **два** незавершённых отчёта на один факт — ровно то задвоение,
против которого механизм и вводился.

Сцепка: перечень писателей холда решением H12 `GAPS_CLOSE_13` объявлен
**открытым** (инвариант «единственный исполнитель блокировки» снят), то
есть посылка «писатель один» больше не действует.

Варианты: (1) **единственный актор** процессных отчётов — все производители
просят его, сами строк не заводят; (2) **ограничение по незавершённому
состоянию на уровне БД** — частичный уникальный индекс по
(`exchange_id`, `code`, `scope`, `fee_group_key`) при незавершённых
статусах: анкер возвращается, но уже как страховка от гонки, а не как
механизм идемпотентности; (3) сериализация на уровне транзакции
производителя (блокировка по ключу факта).
Связано: `docs/models/domain/other/AnomalyReport.md` §Инварианты структуры /
§Персистентность, `docs/components/HoldService.md` §«Кто зовёт».

### ANOM-Q6. Популяция, над которой действует ограничение уникальности отчёта (владелец — `solution-designer`)

Заведён при закрытии H17 `GAPS_CLOSE_13` (**в прогоне не поднят**). Любая
форма ограничения — уникальный ключ или поиск незавершённого — обязана
назвать **популяцию**: действует ли она над **всеми** строками сущности или
только над **неразобранными** (незавершённые статусы). Дефект наследуется
любой формой ключа, поэтому вопрос не снят переходом на статусный механизм.

Практическое следствие: при популяции «все строки» повторный факт после
разбора инцидента не заведёт нового отчёта вовсе (история одного факта
схлопнется в одну строку); при популяции «неразобранные» повторный факт
после ручного снятия холда заведёт новый отчёт — что и требуется журналу
расследования, но обязано быть записано, а не подразумеваться.
