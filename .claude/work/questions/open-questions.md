...
**Третья ось перебора — форма добора** (H20 `DOCS_CHECK_17`, вклад торговой
линзы; в носителях не выписана). Перебор шёл по осям «нужен ли потолок» и
«что считать остатком», но не по оси, **какой формой является многоногий
вход**. Формы две, и риск-профиль у них противоположный: **пирамидинг** (по
движению — каждая нога добавляется из реализованного запаса, агрегат может
быть мягче) и **усреднение вниз** (против движения — все ноги проигрывают
одновременно и коррелированно, агрегат обязан быть жёстким). Какая из них
рабочая для стратегий фазы 1, не записано: `Deal.md` рассматривает сетку
«набирающую против движения», `risk-validator-scope.md` называет
«scaling / pyramiding» равноправно. Различения нет ни в
`StrategyOrderAction`, ни в `Order.Type` (проверено грепом).

**Цена, названная для всех трёх потолков-кандидатов** (там же). Любой потолок
на **заявленную** сумму связывает не тогда, когда риск растёт: при усреднении
вниз риск сделки растёт **без новой ноги** — стоящие ноги ушли в минус при
общем стопе. Эту величину не выражает ни одно из трёх заведённых полей:
`plannedRiskAmount` — write-once-слагаемые, `incurredRiskAmount` не
отматывается, `currentRiskAmount` пропорционален размеру, а не расстоянию до
стопа. То есть все три кандидата контролируют величину, которая при
усреднении вниз меняется меньше всего.

**Зависимость от узла третьего числа** (H7 `DOCS_CHECK_17`): вариант (в)
опирается на `currentRiskAmount`, а тот в нынешней формуле к движению стопа
слеп — после переноса стопа в безубыток показывает полный взятый риск.
Исход «что означает третье число» предшествует выбору здесь.

Грунт вклада: «мартингейл в просадке не работает; работают анти-мартингейлы»
[Tharp гл.12]; «переплечо и недокапитализация — главные причины провала на
здоровой системе» [Tharp гл.6 с.145-146]; «плечо + crowded trade = топливо
хвостового каскада» [Harris гл.28 с.556-562; Carver ST гл.2].
