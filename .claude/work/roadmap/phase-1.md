...
- **Жёсткие гейты `DONE` шагов 6/7 (2026-06-12, из разбора ревью шага
  4).** **D-B3** (SUBMIT recovery-by-clientId — дубль ордера при ресабмите
  после краша между place и сохранением `externalId`) и **D-M1**
  (concurrency-guard вокруг исполнения команды — двойной SUBMIT при
  перекрытии триггеров) — деньги-дубли, латентны до включения
  оркестрационной петли. **Блокирующее условие:** оркестрационную петлю
  **нельзя включать** и шаг 6/7 **не уходит в `DONE`**, пока оба не
  закрыты. Это жёсткий гейт шага (проверяется при переходе 6/7 в `DONE`),
  не просто форвард-долг. Детали — `.claude/work/backlog.md` §Хвост шага 4.
- **Error-политика — за шагом 6, гейтит `CODE` шага 6 (2026-06-12).**
  Единая политика исключений (возбуждение / обработка / классификация;
  коды; `@ControllerAdvice` vs per-endpoint; документирование на
  контроллере) проектируется docs-first на шаге 6 и **закрывает TBD
  error-конвенцию** в `codestyle.md` §«Обработка ошибок». Неблокирующие
  майоры шагов 2 и 4, висящие на ней (500 вместо 422/409, невыровненные
  коды реджектов — `backlog.md` §Ретро-ревью шагов 1-3 / §Хвост шага 4),
  **ретро-закрываются вместе с политикой**.
