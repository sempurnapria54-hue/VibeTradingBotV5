...
Состав ответа: единственный generic-эндпоинт
`POST /api/proxy/okx/raw`, весь in-perimeter периметр манифеста, прямой
кейс и порождённый негатив на каждый эндпоинт, варианты
типа-дискриминатора, цепочки предусловий и teardown.
...
- **`M17.5`** (2026-08-29) — судьба встроенной защиты при отмене родителя,
  три цепочки, **31 строка**, ◐ прогнан 2026-08-30 частично, **гейтит
  `CODE`** (предусловие п. 17 реестра
  `.claude/tests/source-api/okx/code-preconditions.md`). Код-тест написан
  2026-08-30 (`M17CancelOrderLiveTest.m17_5_attachedProtectionOnParentCancel`);
  в прогон 2026-06-20 не входил. Кейс — **первое** покрытие
  `attachAlgoOrds` в плане.

**Кейсов шага 7, ожидающих прогона, было семнадцать; на 2026-08-30
прогнаны десять гейтящих** (`AG1.5`, `AG1.6`, `AG1.7`, `AG1.8`, `AG1.9`,
`AG3.4`, `AG6.1`, `AG6.2`, `MG7.5`, `M17.5`); двум факт дался частично
(`AG1.7` — п. 14 да, п. 7 нет; `AG1.8` — событие не наступило, гейтящий
статус пункта снят решением держателя 2026-08-30, кейс закрывается
заменителями). Негейтящие семь
(`M1.7`, `M15.7`, `AG3.5`, `AG3.6`, `AG12.4`, `AG12.5`, `MG9.5`) остаются
`PENDING`. **Сводного счёта «сколько дали факт целиком» здесь нет:**
состояние каждого слота держит реестр
`.claude/tests/source-api/okx/code-preconditions.md`, и второй счёт рядом
с ним разошёлся бы при первом же прогоне. Прежняя редакция
абзаца («семнадцать; прогнан ноль»):
...
**Какие из них гейтят `CODE` и сколько слотов несут — читается реестром,
а не отсюда:** `.claude/tests/source-api/okx/code-preconditions.md`
§Реестр, колонка статуса. Числа здесь не дублируются — второй носитель
счёта расходится с таблицей при первой же смене статуса, и это уже
происходило. Структура перекрытий (она не счёт и меняется реже): три
кейса закрывают по два пункта, и два пункта закрываются парами кейсов —
перечень пар в самом реестре.
...
## Структурный изоморфизм (план ↔ код-тесты)

- **эндпоинт** → раздел плана (`## <ID>n …`) / класс код-тестов;
- **кейс** → подраздел плана (`### <ID>n.k`) + таблица / тест-метод;
- **запрос** → строка таблицы / шаг-ассерт;
- **исход-перечень** → файл `observations/<КЕЙС>.md`, а ячейка «Факт +
  наблюдения (RUN)» несёт **указатель и мощность**, не сам перечень.

**Контракт последней строки записан, а не выведен.** Кейсов, чей исход
есть **перечень**, в плане четыре — `AG6.1`, `AG6.2`, `AG1.7`, `AG3.6`, —
и без записанного контракта каждый переоткрывает развилку «куда писать»
заново; этот механизм уже дал расхождение (ячейка утверждала сохранённый
перечень, которого в репозитории не было). Дом исхода один:
`observations/<КЕЙС>.md`, пишет прогон вызовом `persistObservation`,
руками файл не правится (`.claude/rules/structure.md`). Ячейка плана —
второй носитель только для **мощности** и указателя; полного перечня она
не держит никогда.

Исполнитель прогона — код-тесты (`test-code`), бьющие в `/raw`.
Справочная коллекция изоморфизмом не связана и аппрув-артефактом не
является (`.claude/decisions/collection-not-approval-artifact.md`).
...
- **из шестнадцати содержательных кейсов шага 7** код-тесты имеют
  **десять** (`M17.5`, `AG1.5`, `AG1.6`, `AG1.7`, `AG1.8`, `AG1.9`,
  `AG3.4`, `AG6.2`, `MG7.5`, плюс `AG6.1` вне этого перечня); носителя нет
  у **семи**: `M1.7`, `M15.7`, `AG3.5`, `AG3.6`, `AG12.4`, `AG12.5`,
  `MG9.5`; из них `AG3.6` упомянут **только в javadoc**
  базового класса, а упоминание в комментарии носителем не является;
- `AG6.1` — единственный из кейсов шага 7, представленный в код-тестах
  (аппрув 2026-06-19 его покрывает); его перепрогон меняет **фиксацию
  исхода**, не запрос.

Следствие процедурное: недостающие кейсы дельты требуют **код-тестов** —
исполнителя прогона, — а не запросов коллекции. Аппрув контура даётся на
**план** (`.claude/processes/source-api-testing.md` §APPROVE); коллекция
аппрув-артефактом не является и достройки на дельте не требует (решение
держателя 2026-08-31,
`.claude/decisions/collection-not-approval-artifact.md`). Сбор грунта по
гейтящим кейсам блокирован средой и написанием код-тестов, не
коллекцией. Прежняя формулировка «коллекция — 1:1 с этим планом»
безусловным клеймом полноты запрещала искать дальше и снята; снята и
редакция, требовавшая аппрува на плане и коллекции сразу, —
...
- **Инвариант восстановления состояния кейса.** Каждый **stateful-кейс**
  ...
  **Verify.end** (последний шаг после restore/teardown: ассерт «конец ==
  ...
  случае.
- **Поллинг — в код-тестах.** План мандатит
...
Зависимости разрешаются порядком и переменными исполнителя (полями
класса код-тестов).
...
| **`OBSERVED_ABSENT`** | кейс **прогнан**, событие за время прогона не наступило | **гейт не закрыт** — слот остаётся `PENDING` с этой причиной |

**Правило (редакция 2026-08-30).** Исход `OBSERVED_ABSENT` гейтящее
предусловие `CODE` **не закрывает**: решение держателя 2026-08-30
(дословно — «гейт исходом „не наступило“ не закрывается») снимает
закрывающую силу, введённую П18 валидации `GAPS_CLOSE_17`. Исход остаётся
**записью наблюдения**: слот реестра
(`.claude/tests/source-api/okx/code-preconditions.md`) получает статус
`PENDING` с названной причиной. Дом правила — реестр, §«Правило гейта»;
здесь — механика исходов.

**Что решение сняло.** Прежняя редакция закрывала гейт посылкой,
помеченной непроверенной, ради того, чтобы `CODE` не ждал бессрочно
события, которое биржа не обязана произвести. Держатель принял обратный
размен: `CODE` ждёт, а непроверенная посылка в код не уезжает. Кто
несёт последствие — держатель, и он его назвал.
...
**Цена новой редакции названа:** `CODE` шага 7 может ждать события,
которое биржа не обязана произвести. Развилка «снимать ли гейтящий статус
у условно наблюдаемого пункта» с тех пор **решена держателем**
(2026-08-30, `.claude/decisions/unorderable-fact-substitutes.md`): такой
пункт **выведен из-под гейта** и закрывается заменителями, каждый со
своим носителем. Первым и пока единственным так закрыт след
автоделевериджа.
...
(`.claude/tests/source-api/okx/coverage-manifest.md`) **переразмечена**
(2026-08-30): прежние `⚪ gap` (in-perimeter без метода клиента) покрыты
через `/raw`, легенда колонки восстановлена под `/raw`-семантику, метка
приведена к правилу отката. Счёт сошёлся с §Скоуп этого плана:
in-perimeter строк манифеста — **60**, ровно столько же эндпоинтов
покрывает план. Прежний раздел «client-coverage-gap'ы (не покрываем)»
снят: под `/raw` таких нет.

**Девять** строк манифеста несут `🟡 в плане` — это методы, у которых
есть **непокрытые** кейсы (16 кейсов; девять из них гейтят `CODE` —
`code-preconditions.md`). «Непокрытый» — нет ни своего код-теста, **ни
делегирования цепочке**: кейс, чьё тело здесь начинается строкой
`Покрыт <звенья>`, покрыт этими звеньями и метку не откатывает.
Значения меток, определение и правило отката — дом:
`.claude/processes/source-api-testing.md` §«Колонка покрытия в
манифесте»; счёт пересобирается командой из `coverage-manifest.md`
§Легенда, здесь не дублируется.
...
### M17.5 Содержательный (шаг 7, A8 `DOCS_CHECK_24`) — судьба встроенной защиты при отмене родителя ✅ RUN 2026-08-30 — все три цепочки наблюдены — **ГЕЙТИТ `CODE`** (предусловие п. 17)

**Факт (RUN 2026-08-30, `M17CancelOrderLiveTest.m17_5_attachedProtectionOnParentCancel`,
два независимых прогона).**

- **C17a (родитель без налива) — наблюдён.** Встроенная защита
  представлена элементом `attachAlgoOrds[0]` с `attachAlgoClOrdId` и
  присвоенным биржей `attachAlgoId`; самостоятельной записью в
  `orders-algo-pending` она **не видна** — ни до отмены, ни после. После
  отмены родителя `attachAlgoOrds[0]` **сохраняется в теле отменённого
  родителя** с прежними значениями и пустыми `failCode`/`failReason`;
  `cancelSource=1` («Order was canceled by you»). Отсюда следствие для
  резолвера: **присутствие `attachAlgoOrds` в снапшоте не означает живой
  защиты** — отменённый родитель несёт её тем же элементом.
- **C17b (наш `cancel-order` по частично налитому родителю) — наблюдён.**
  `partially_filled` при `accFillSz=0.01` из `sz=10.10`; после отмены
  родителя (`state=canceled`, `cancelSource=1`, налив сохранён)
  `orders-algo-pending` содержит **живую** запись `state=live`,
  `algoClOrdId=<attachAlgoClOrdId>`, `sz=0.01`, `reduceOnly=true`,
  `side=sell`.
- **C17c (терминал наводит биржа: `ordType=ioc`) — наблюдён.**
  `state=canceled` при `accFillSz=2.03` из `sz=10.09`,
  `cancelSource=14` («Immediate or cancel (IOC) order was canceled because
  it wasn’t filled completely»); `orders-algo-pending` содержит живую
  запись `sz=2.03`. Сочетание `ioc` + `attachAlgoOrds` источником
  **принимается** (`sCode=0`) — тоже факт контракта.

**Ответ предусловия п. 17 — вторая развязка: защита ПЕРЕЖИВАЕТ терминал
родителя.** Она не снимается вместе с ним и не уходит в
`orders-algo-history` (`state=canceled` там её не содержит): при непустом
наливе встроенная защита **материализуется самостоятельной живой
conditional-заявкой** на налитый объём. Четыре следствия, и каждое —
операнд кода:

1. **Судьба защиты от инициатора терминала не зависит.** Наша отмена
   (C17b, `cancelSource=1`) и снятие остатка биржей (C17c,
   `cancelSource=14`) дали один исход. Расхождения, которого опасался
   кейс, нет.
2. **`sz` живой защиты равен `accFillSz` родителя** (0.01 и 2.03
   соответственно) — покрытие ровно на налитый объём, а не на исходный
   `sz` родителя.
3. **`algoId` живой записи НЕ равен `attachAlgoId` родителя**
   (`3878921628419895297` против `3878921494101504002`;
   `3878926851502780420` против `3878926851469225986`). Матч по
   `attachAlgoId` не сойдётся ни разу; единственный сходящийся операнд —
   **клиентский** `algoClOrdId == attachAlgoClOrdId`, и это ровно тот
   ключ, на котором стои́т `docs/components/AttachedAlgoOrderStateResolver.md`.
4. **Обратной ссылки на родителя у живой записи нет:** `ordId=""`,
   `linkedOrd.ordId=""`. Связать её с родителем можно только клиентским
   идентификатором.

**Что это ломает — названо кейсом заранее и теперь стои́т на факте:**
исход `CANCEL_BY_PARENT` (`docs/lifecycles/Order.md`) помечает **живую**
защиту снятой, покрытие недосчитывается, и гейт покрытия уводит штатную
сделку в ступень 2 с flatten по **ложному** факту
(`docs/rules/exchange-hold.md`). Правка носителей — работа закрытия, не
плана.

**Потолок цепочки — фикстурный и поднятый** (решение держателя
2026-08-30): прежние 200 USDT давали 0.81 контракта, то есть меньше
любого наблюдённого верхнего уровня книги, и частичный налив был
недостижим **по счёту**, а не по гонке. Действующее значение — 2500 USDT
(≈10 контрактов); дом константы и её ограничения —
`M17CancelOrderLiveTest.FIXTURE_RISK_CEILING_USDT`. Вырождение попытки
(полный либо нулевой налив) осталось, но перестало быть неизбежным: в
двух прогонах из десяти попыток `partially_filled` собрался дважды.
...
ни один контракт-док: в `docs/integrations/okx/contracts/order.md`
`attachAlgoOrds` назван параметром постановки и amend'а, а его судьба при
отмене родителя не описана.
...
- **Потолок риска цепочки — фикстурный, не продуктовый.** `sz` берётся
  **весь потолок** (notional по `last` не выше **2500 USDT** ≈ 10
  контрактов); если на уровне лучшего аска стои́т больше половины потолка,
  попытка **пропускается** и книга перечитывается (см. C17b). Смысл
...
- **Статус:** ✅ **RUN 2026-08-30 — наблюдён, предусловие п. 17
  закрыто.** Провенанс — A8 `DOCS_CHECK_24` (вариант (1): «требует
  биржевого факта — жива ли attached после отмены родителя»); реестр —
...
аска** (маркетабелен ровно на один уровень книги) размером во **весь
фикстурный потолок** (notional ≤ 2500 USDT по `last`), при условии, что
объём лучшего аска не больше половины потолка. Тогда налив ≈ объём
лучшего уровня, а остаток — не меньше половины потолка — встаёт живым
лимитом по той же цене.

**Почему размер именно потолок, а не `2 × Σ sz(asks[0])`.** Прежняя
редакция ставила удвоенный объём верхнего уровня, и при тонком уровне
(наблюдено `asks[0] = 0.01`) остаток составлял один лот: книга поглощала
его между чтением и постановкой, попытка вырождалась в `filled`. Размер
в потолок переносит порог вырождения с лота на половину потолка.
...
перечитыванием книги, **до пяти попыток**. Если ни одна не дала
`partially_filled`, кейс остаётся **`PENDING`** и предусловие п. 17
**не закрывается**: «фикстуру собрать не удалось» — не то же, что
`OBSERVED_ABSENT` (§«Исходы содержательного кейса — их три, и третий гейт не закрывает»: третий исход — для
события, инициируемого биржей; частичный налив заказуем нами). Подмена
одного другим закрыла бы гейт ложно. Вырождение наблюдалось и под
поднятым потолком (`filled accFillSz=10.1` из `sz=10.10`), то есть
конструкция снимает не гонку, а её неизбежность.
...
| **C17b.get.** `POST /raw {method:GET, path:/api/v5/trade/order, query:{instId:ETH-USDT-SWAP, ordId:b_ordId}, signed:true}` (поллинг до стабилизации) | HTTP 200; `b.data[0].state="partially_filled"`; `0 < b.data[0].accFillSz < b_sz` | **Условие продолжения цепочки.** `state=filled` либо `accFillSz=0` ⇒ вырождение попытки (см. выше): teardown + повтор | RUN 2026-08-30 ✓ — попытка 2: `partially_filled`, `accFillSz=0.01` из `sz=10.10` (попытка 1 выродилась в `filled`) |
| **C17b.position.** `POST /raw {method:GET, path:/api/v5/account/positions, query:{instType:SWAP, instId:ETH-USDT-SWAP}, signed:true}` | HTTP 200; `b.data[0].pos` ≠ 0; `b.data[0].posId` непустой | Налитый объём — живая позиция: именно её покрывает спорная защита | RUN 2026-08-30 ✓ — `pos=0.01`, `posId=3878692455038332929` (тот же `posId`, что у прежних эпизодов — переиспользование, §AG1.9) |
...
`ordType=ioc` при `sz` больше объёма лучшего уровня даёт частично налитый
ордер, чей остаток снимает сама биржа: терминал `canceled` при
`accFillSz>0`. Цепочка отвечает на тот же вопрос о судьбе встроенной
защиты, но при **другом инициаторе** терминала, поэтому C17b она не
заменяет: расхождение исходов C17b и C17c — самостоятельный факт
(инициатор отмены значим). **`ioc` — фикстурное средство, не заявка на
покрытие варианта** `ordType`: адаптер его не строит, в инвариант полноты
он не входит.

**Клейм «без гонки» снят прогоном.** Прежняя редакция объявляла C17c
детерминированным дублёром: остаток снимает биржа, значит гонки нет. Это
неверно — гонка стои́т **раньше**, на самом наливе: попытка 1 прогона
2026-08-30 дала `accFillSz=0` (уровень ушёл между чтением книги и
постановкой), и `ioc` вернул `canceled` с пустым наливом, то есть цепочка
выродилась ровно так же, как вырождается C17b. Понадобилось пять попыток.
`ioc` снимает гонку **остатка**, а не гонку **уровня**; вырождение
попытки у C17c такое же, как у C17b, и обрабатывается так же.
...
| **C17c.place.** `POST /raw {method:POST, path:/api/v5/trade/order, body:{instId:ETH-USDT-SWAP, tdMode:isolated, side:buy, ordType:ioc, sz:c_sz, px:c_px, clOrdId:c_clId, attachAlgoOrds:[{attachAlgoClOrdId:c_attId, slTriggerPx:c_sl, slOrdPx:"-1", slTriggerPxType:"last"}]}, signed:true}` | HTTP 200; **наблюдение:** `b.data[0].sCode` — принят ли `attachAlgoOrds` вместе с `ioc` | Реджект сочетания — тоже факт контракта, фиксируется и снимает цепочку (не фейл) | RUN 2026-08-30 ✓ — сочетание `ioc` + `attachAlgoOrds` **принято** (`sCode=0`) |
| **C17c.get.** `POST /raw {method:GET, path:/api/v5/trade/order, query:{instId:ETH-USDT-SWAP, ordId:c_ordId}, signed:true}` (поллинг до терминала) | HTTP 200; `b.data[0].state="canceled"`; `b.data[0].accFillSz>0`; **наблюдение:** `attachAlgoOrds[0]` | Терминал при непустом наливе получен без гонки | RUN 2026-08-30 ✓ — `state=canceled`, `accFillSz=2.03` из `sz=10.09`, `cancelSource=14` (остаток снят биржей). Оговорка: «без гонки» **не подтвердилось** — попытка 1 дала `accFillSz=0` (уровень ушёл), понадобилось пять попыток |
| **C17c.algoPending.** `POST /raw {method:GET, path:/api/v5/trade/orders-algo-pending, query:{instType:SWAP, instId:ETH-USDT-SWAP, ordType:conditional}, signed:true}` | HTTP 200; **наблюдение:** жива ли запись по `c_attId` | Сравнивается с исходом C17b: совпало ⇒ судьба защиты от инициатора не зависит; разошлось ⇒ зависит, и это находка | RUN 2026-08-30 ✓ — **защита ЖИВА**: `n=1`, `state=live`, `sz=2.03` (== `accFillSz`), `algoId=3878926851502780420` ≠ `attachAlgoId`. **Совпало с C17b ⇒ судьба защиты от инициатора терминала не зависит** |
| **C17c.position.** `POST /raw {method:GET, path:/api/v5/account/positions, query:{instType:SWAP, instId:ETH-USDT-SWAP}, signed:true}` | HTTP 200; `b.data[0].pos` ≠ 0 | Налитый объём открыт (предмет покрытия) | RUN 2026-08-30 ✓ — `pos=2.03`, `posId=3878692455038332929` |
...
### AG1.5 Содержательный (шаг 7, N11) — семантика агрегации partial-close ✅ RUN 2026-08-30 — **ГЕЙТИТ `CODE`** (предусловие п. 1)

**Факт (RUN 2026-08-30, `Ag1DealFixtureLiveTest.ag1_5_partialCloseAggregation`).**
Источник **агрегирует**: эпизод «открыть 0.02 → reduce-only 0.01 → закрыть
остаток» дал **одну** запись, `closeTotalPos=0.02` (полный закрытый объём),
`realizedPnl` кумулятивен по обоим слайсам и **точно** равен
...
Момент финализации — `uTime` записи после выхода в flat. Ожидание кейса
подтверждено.
...
- **Статус:** ✅ **RUN 2026-08-30** — факт добыт: источник **агрегирует** закрытие в одну запись, `realizedPnl` кумулятивен (реестр предусловий, раздел «Что добыто»). Слот п. 1 закрыт. Провенанс — N11 `DOCS_CHECK_11`.
...
### AG1.6 Содержательный (шаг 7, H5) — оси адресации записи без `posId` ✅ RUN 2026-08-30 — **ГЕЙТИТ `CODE`** (предусловие п. 6)

**Факт (RUN 2026-08-30, `Ag1DealFixtureLiveTest.ag1_6_addressingAxes`).**
(1) Оси: `instId` принимается **без** `instType`; окно по `uTime`
(`before`/`after`) отбирает ровно записи окна сделки; `posId` принимается
фильтром, но одну запись **не адресует** (см. AG1.9). (2) Окно с двумя
эпизодами вернуло две записи, порядок — по убыванию `uTime`, циклы
различимы по `cTime`. (3) Запись несёт `cTime`, `direction` и `openAvgPx`
— операнды материализации `Position` на create-тропе. Эскалации по ветви
«записи неразличимы» не требуется: различитель есть, и это `cTime`.
...
- **Статус:** ✅ **RUN 2026-08-30** — оси адресации записи без `posId` наблюдены, слот п. 6 закрыт. Провенанс — H5 `DOCS_CHECK_11`.

### AG1.7 Содержательный (шаг 7, H20) — семантика и знаки числовых полей записи ◐ RUN 2026-08-30 — п. 14 закрыт, п. 7 `PENDING` — **ГЕЙТИТ `CODE`** (предусловия п. 7 — горизонт `fundingFee`; п. 14 — форма пустого значения; знаки (2) гейта не образуют)

**Факт (RUN 2026-08-30, `Ag1DealFixtureLiveTest.ag1_7_numericFieldSemantics`).**
- **(3) Форма пустого значения (предусловие п. 14) — ожидание подтверждено:**
  несобытийные `fundingFee` и `liqPenalty` приходят строковым **`"0"`**,
  ключ присутствует. Наблюдён и **контраст**: неприменимые к записи поля
  (`settledPnl`, `nonSettleAvgPx`, `triggerPx`) приходят **пустой строкой**
  — форма пустого значения у источника **различается по полю**, и
  конвенция «пусто = 0» верна для числовых слагаемых, а не для всех полей.
- **(2) Знаки:** `fee` приходит **отрицательным** (издержка) — заявление
  доков подтверждено прогоном. `fundingFee` и `liqPenalty` на фикстуре
  несобытийны, знак не наблюдён.
- **(1) Горизонт `fundingFee` (предусловие п. 7) — НЕ наблюдён:** границу
  funding-интервала за окно прогона не пересекали. Слот остаётся `PENDING`;
  что нужно — удержание позиции через **ближайшую границу интервала того
  инструмента, который взят фикстурой**. Интервал определяется разницей
  `fundingTime` и `nextFundingTime` его же ответа
  (`docs/integrations/okx/contracts/funding-rate.md`), а не расписанием
  восьмичасового: на demo из 142 живых USDT-свопов 89 считают фандинг
  каждые четыре часа (замер 2026-08-30).
...
- **Фикстура — та же §AG1.5**, дополнительно нужна позиция, пережившая
  **хотя бы один** funding-расчёт (удержание через границу funding-интервала).
  ...
  `liqPenalty` — та же фикстура, что у §AG1 по ADL-следу (тот пункт
  **выведен из-под гейта** 2026-08-30,
  `.claude/decisions/unorderable-fact-substitutes.md`).
- **Статус:** ◐ **RUN 2026-08-30** — п. 14 закрыт (горизонт funding наблюдён и записан персистентно), п. 7 остаётся ⏳ `PENDING`. Провенанс — H20 `DOCS_CHECK_11`; расширение
  состава — H15 `DOCS_CHECK_16`.

### AG1.8 Содержательный (шаг 7, H27) — след автоделевериджа в bills ⏳ PENDING — прогнан 2026-08-30, событие не наступило — **гейта не образует** (п. 10 выведен из-под гейта 2026-08-30)

**Факт (RUN 2026-08-30, `Ag1DealFixtureLiveTest.ag1_8_adlTrace`).** Bills
типов 5 (Forced liquidation) и 9 (ADL) — **ноль** и в окне сделки, и за
весь доступный семидневный период. Эпизод ADL инициирует биржа, на demo не
заказывается. По редакции §«Исходы содержательного кейса — их три, и третий гейт не закрывает» от 2026-08-30
исход «не наступило» гейт **не закрывает**, поэтому наблюдение не
состоялось и кейс остаётся `PENDING`.

**Гейтящий статус п. 10 снят решением держателя 2026-08-30**
(`.claude/decisions/unorderable-fact-substitutes.md`): гейт, исполнение
которого не в наших руках, перестаёт мерить готовность и начинает мерить
везение. Кейс из плана **не снимается** — если эпизод наступит,
наблюдение заместит заменители, — но `CODE` он больше не держит.

**Что добыто вместо наблюдения — форма события из собственного
справочника источника** (RUN 2026-08-30,
`Ag6BillSubtypesLiveTest.ag6_1_directDictionary` и прямой запрос
`GET /api/v5/account/subtypes`): ADL-движения перечислены парами
`9/125` (ADL close long), `9/126` (ADL close short), `9/127` (ADL buy),
`9/128` (ADL sell) **и** `5/100` (ADL close long), `5/101` (ADL close
short); подтип штрафа — отдельный, `5/116` (Balance deducted for
liquidation). Два следствия — след ADL живёт под **двумя** верхними
типами, и тип `5` **гетерогенен** (движения закрытия и движение штрафа под
одним типом) — записаны в
`docs/integrations/okx/contracts/account-bills.md` §«Семейство
принудительного закрытия — строки справочника».

**Заменители — три, каждый со своим носителем:** синтетическая фикстура
(три примера `docs/spec/pnl-reconciliation.json` «ФИКСТУРА п. 10 (ADL)»),
защитная обработка неизвестного источника сокращения (третий из них —
односторонний след штрафа даёт `mismatched`; плюс отказ от типового
умолчания у типа `5`), пункт живого наблюдения на запуске
(`.claude/work/prod-checks.md` §«Живое наблюдение на запуске»). Состав и цена
— в решении.

Заведён решением держателя П17 валидации `GAPS_CLOSE_17` (H27
`DOCS_CHECK_17`, варианты A + B). Пункт 10 реестра тогда был объявлен
гейтящим, а носителя проверки у него не существовало — ссылка на `AG1.7`
была битой: тот кейс проверяет знаки и горизонт, а не след ADL. С
2026-08-30 пункт **выведен из-под гейта**
(`.claude/decisions/unorderable-fact-substitutes.md`).
...
- **Прежнее условие закрытия снято дважды.** Решение П17 валидации
  `GAPS_CLOSE_17` разрешало закрыть кейс исходом `OBSERVED_ABSENT`
  («прогнан, ADL не наступил»); решение держателя 2026-08-30 отняло у
  третьего исхода закрывающую силу (§«Исходы содержательного кейса — их три, и третий гейт не закрывает»), и
  тем же днём второе решение держателя вывело п. 10 из-под гейта. Обе
  редакции названы, чтобы следующий прогон не прочитал снятие шире, чем
  оно есть: **гейта нет, наблюдения тоже нет** — есть заменители.
- **Статус:** ⏳ **PENDING бессрочно, гейта не образует.** Провенанс —
  H13 `DOCS_CHECK_15` (статус гейта), H27 `DOCS_CHECK_17` (отсутствие
  носителя), решение держателя П17 (заведение кейса), решение держателя
  2026-08-30 (снятие гейтящего статуса,
  `.claude/decisions/unorderable-fact-substitutes.md`).

### AG1.9 Содержательный (шаг 7, T3) — окно с несколькими записями = эпизоды сделки ✅ RUN 2026-08-30 — **ГЕЙТИТ `CODE`** (предусловия пп. 1 и 15)

**Факт (RUN 2026-08-30, `Ag1DealFixtureLiveTest.ag1_9_episodesInWindow`).**
Записей столько, сколько эпизодов — источник их **не схлопывает**, и
`realizedPnl` каждой кумулятивен внутри своего эпизода. **Вторая половина
кейса разрешилась в ложную сторону: `posId` ПЕРЕИСПОЛЬЗУЕТСЯ.** Два
эпизода в одном окне получили **один и тот же** `posId`
(`3878692455038332929`) при разных `cTime`; запрос с фильтром `posId`
вернул все записи инструмента, а не одну. Наблюдено дважды независимо
(ручной прогон и код-тест), плюс историческая часть счёта: пять записей
подряд под общим `posId=3670806710316703744`. **Дозамер 2026-08-30:**
восемь последовательных эпизодов за ~110 минут — один и тот же
`posId=3878692455038332929` при восьми разных `cTime`, то есть
переиспользование не артефакт узкого окна.

**Второй операнд пары проверен, а не предположен.** `cTime` различает
эпизоды **на обеих сторонах**: у восьми записей истории с общим `posId`
— восемь разных `cTime`, а **живая** запись (`GET /account/positions`)
несёт `cTime` **текущего** эпизода, а не первого. Без этой второй
половины пара работала бы только на истории, и дискриминатор первой ноги
добычи чинить было бы нечем.

Адресуемая единица — **пара (`posId`, `cTime`)**. Отсюда правятся
носители, названные ниже, и ещё один, планом не названный.
...
- **Третий носитель план не назвал, и он тяжелее двух названных**
  (найдено закрытием `GAPS_CLOSE_29`): на **первой** ноге добычи
  «тот же эпизод» резолвится сравнением идентификатора живого снапшота
  со строкой. Под переиспользованным `posId` переоткрытие **между
  тиками** читается как «тот же эпизод», живая строка обновляется на
  месте, эпизод 1 не закрывается — и вторая нога, чей предикат «строка
  закрыта и положения закрытия не несёт», по нему **не срабатывает
  вовсе**. Результат сделки недосчитывает эпизод **молча**. Клейм
  `docs/components/RefreshPositionExecutor.md` «покрывает эпизод,
  схлопнувшийся и переоткрывшийся между тиками» был ложен ровно на этой
  тропе. Закрыто там же переходом дискриминатора на пару.
- **Статус:** ✅ **RUN 2026-08-30 — наблюдён, предусловия пп. 1 и 15
  закрыты.** Продуктовые правки приземлены `GAPS_CLOSE_29`; дом
  адресуемой единицы — `docs/models/domain/core/Position.md` §«Адресуемая единица эпизода — пара, а не идентификатор».
...
### AG3.4 Содержательный (шаг 7, H8 / RQ-4) — комиссии приходят в settle-ccy ◐ RUN 2026-08-30 — инвариант settle-ccy подтверждён — предусловие п. 9 собрано; вторая проверка (состав `realizedPnl` при cross-ccy-издержке) фикстурой не разрешена и гейта за собой не несёт

**Факт (RUN 2026-08-30, `Ag1DealFixtureLiveTest.ag3_4_feeCurrency`).** У
всех движений окна `ccy="USDT"` — инвариант settle-ccy держится. Комиссия
приходит **полем `fee` внутри торгового bill'а**, отдельной fee-записи
источник не эмитит (это же наблюдение отвечает и на вопрос гранулярности
§AG3.5). Движений вне settle-ccy в окне **не наблюдалось**, поэтому вторая
проверка (состав `realizedPnl` при cross-ccy-издержке) этой фикстурой не
разрешена; гейта за ней нет.
...
- **Статус:** ◐ **RUN 2026-08-30** — инвариант settle-ccy подтверждён, слот п. 9 закрыт; хвост RQ-4 (гранулярность) идёт в общем порядке. Провенанс —
...
`docs/models/integrations/okx/AccountBillOkxResponse.md` объявляет
«сужение до used зафиксировано», но собственный контракт-док источника
(`docs/integrations/okx/contracts/account-bills.md` §«Deep-архив с 2021 (двухшаговый async-флоу)»,
...
- **Проверка:** множество ключей записи ⊇ used-перечень; разница между
  ...
  доводом.
- **Куда записывается:** `AccountBillOkxResponse.md` (провенанс
...
### AG6.1 Прямой — справочник типов bills ✅ RUN 2026-08-30 (перепрогон) — **ГЕЙТИТ `CODE`** (предусловия пп. 2 и 16: наблюдение сохранено мощностью, не перечнем)

| Запрос | Проверки (сырой JSON OKX) | Ожидаемый результат | Факт + наблюдения (RUN) |
|---|---|---|---|
| `POST /raw {method:GET, path:/api/v5/account/subtypes, signed:true}` | HTTP 200; `b.code="0"`; `b.data` — массив непустой; `b.data[0]` несёт пары type/subType | Справочник bill types: непустой перечень. Включает funding-подтипы `173`(expense)/`174`(income). Точная форма элемента — наблюдение (сверка с офдоком при RUN). **Наблюдение записывается перечнем, а не мощностью** — см. клаузу ниже | RUN 2026-08-30 — наблюдение сохранено **мощностью, не перечнем**: форма элемента — `{type, typeDesc, subTypeDetails:[{subType, subTypeDesc}]}`, **32 верхних типа и 179 пар**; сами пары ушли в лог, файла `observations/AG6_1.md` в репозитории нет. Прежнее «data.size=32» (RUN 2026-06-20) было счётом **верхних типов**, а не пар — то есть измеряло не ту величину. Ожидание кейса подтверждено: funding-подтипы `173` и `174` присутствуют под типом `8` (Funding rate). Перепрогон записывает перечень в `observations/AG6_1.md` и **закрывает слот п. 2 × `AG6.1` целиком**; у слота **п. 16** он закрывает лишь **грунтовую половину** — предмет п. 16 не перечень, а ОТОБРАЖЕНИЕ пар в экономические категории, и оно фиксируется семантической разметкой (`docs/integrations/okx/rules/cash-flow-categories.md`, писатель — хэнд-офф `integrator`), а не прогоном |
...
- **Чем перечень сохраняется — названо, потому что носителя у требования
  ...
  Инструмент написан (`OkxSourceApiLiveTestBase.observeContent`), но
  печатает в лог; носителем исхода служит `persistObservation` того же
  класса.
- **Что делать при перепрогоне:** сохранить **все** пары `type`/`subType`
  с описанием источника — в `observations/AG6_1.md` вызовом
  `persistObservation(...)`. Инструмент наблюдения состава
  (`observeContent`) в базовом классе контура **уже написан**; недостающим
  был персистентный носитель, и он тоже есть.
...
### AG6.2 Содержательный (шаг 7, H10) — перечень типов вне экономики сделки ◐ RUN 2026-08-30 — исход-перечень не сохранён, слот открыт — **ГЕЙТИТ `CODE`** (предусловие п. 2)

**Факт (RUN 2026-08-30, `Ag1DealFixtureLiveTest.ag6_2_typesOutsideDealEconomics`).**
В окне сделки фактически появились **две** пары справочника: `2/1` (Trade
/ Buy) и `2/2` (Trade / Sell). **Маржинальных движений `type=6` в окне
нет**: при isolated-марже перевод маржи идёт полем `posBalChg` **внутри**
торгового bill'а и отдельной записи не порождает. Отсюда контроль **не**
мёртв (движения маржи Σ|amount| не раздувают). Принадлежность остальных
пар справочника экономике сделки наблюдение окна **не устанавливает**:
«вне окна» ≠ «вне экономики» — как минимум три пары вне окна в экономику
входят (финансирование `8/173`, `8/174` и штраф `5/116` — правые
операнды пар сверки), поэтому перечень «вне окна» — **кандидаты**, а не
состав списка исключений.
...
- **Ожидание (проверяется, не предполагается):** перечень исключаемых
  ...
  `@ConfigurationProperties` per-exchange, `reconciliationExclusions`;
  хэнд-офф `integrator` исполнен — рабочий лист снят в
  `.claude/work/history/2026-09-03-phase-1-step-7-deals-and-pnl/phase-1-step-7-code-delta-worksheet.md`).
- **Статус:** ⏳ **PENDING** — прогон 2026-08-30 состоялся, но исход-**перечень** остался в логе. Кейс правлен — пишет перечень **кандидатов** (пары вне окна) в `observations/AG6_2.md`. Закрытие слота — **два шага**: перепрогон закрывает грунтовую половину (полный перечень пар и пары окна персистентно); **состав** списка исключений строится **семантической разметкой справочника** по сохранённому перечню — шаг «кандидаты → состав», писатель — хэнд-офф `integrator`, тем же ходом, что отображение категорий п. 16; слот закрыт, когда состав зафиксирован в конфиге исключений (позиция снята из бэклога как исполненная — `.claude/work/history/2026-09-11-backlog-diet.md`). Провенанс — H10 `DOCS_CHECK_11`; контракт слота пересобран по F-2 `DOCS_CHECK_33`.
...
### MG7.5 Содержательный (шаг 7, H25) — носитель курса cross-ccy ✅ RUN 2026-08-30 — **ГЕЙТИТ `CODE`** (предусловие п. 5)

**Факт (RUN 2026-08-30, `Mg7IndexCandlesLiveTest.mg7_5_crossCcyRateCarrier`).**
1. **Секундное разрешение есть:** `index-candles(ETH-USDT, bar=1s)` отдаёт
   данные; приняты `1s, 1m, 3m, 5m, 15m, 1H, 1D`.
2. **Глубина и деградация:** свежий `index-candles` окно **в прошлом не
   обслуживает вовсе** (`n=0` на любом `after` в прошлом) — носитель
   глубины `history-index-candles`. По нему `bar=1s` достаёт назад до
   **~150 дней** (на 180 уже пусто), `bar=1m` — не менее **365 дней**.
   Правило деградации: 1s в пределах ~5 месяцев, дальше 1m.
3. **Цена из свечи:** строка — `[ts, o, h, l, c, confirm]`, close по
   индексу 4, признак закрытости — индекс 5.
4. **Квота:** одним запросом отдаётся **300** строк — диапазон читается
   разом, по-строчный запрос не нужен.
5. **Доступность пары:** индексная свеча по `ETH-USDT` доступна; рыночная
   по тому же символу тоже отдаёт `1s` (спотовый инструмент), но индексная
   предпочтительнее по устойчивости — агрегат площадок против одной.
...
- **Статус:** ✅ **RUN 2026-08-30** — носитель курса cross-ccy наблюдён, слот п. 5 закрыт. Провенанс — H25 `DOCS_CHECK_11`.
...
- **Объект:** любой приватный вызов сырого клиента (напр. `getBalance`)
  ...
  **Тип:** регрессионный гард I3 (auth-негатив). **Не через `/raw`** —
  требует изолированной конфигурации без кредов (поднятый app креды
  имеет); offline-юнит-тест над `OkxSigningInterceptor`, без сети.
...
# SEC1 — перечень auth-отказов источника (грунт шага 9)

- **Объект:** приватный `GET /api/v5/account/balance` с намеренно
  **испорченными** кредами. **Предусловие:** нет (read-only, состояния
  счёта не меняет). **Среда:** demo. **Teardown:** не требуется.
  **Тип:** грунтовый — добывает перечень, а не проверяет ожидание.
- **Не через `/raw`, и это не отступление от механики 1:1.** Прокси
  подписывает вызов кредами поднятого app, то есть верными; предмет
  кейса — ответ источника на **неверные**. Подпись собирается в кейсе по
  той же схеме, что у границы (`OkxSigningInterceptor`).
- **Контроль обязателен и идёт первым:** вызов с верными кредами обязан
  дать `code=0`. Без него перечень отказов не отличим от «контур сломан».

### SEC1.1 Грунтовый — коды auth-отказа по формам порчи кредов ✅ RUN 2026-09-04 — **ГЕЙТИТ `CODE`** (предусловие п. 18)

| Запрос | Проверки (сырой JSON OKX) | Ожидаемый результат | Факт + наблюдения (RUN) |
|---|---|---|---|
| семь прямых вызовов `GET /api/v5/account/balance`: контроль (верные креды) плюс шесть форм порчи — неизвестный ключ, испорченная подпись, неверная passphrase, ключ неверной формы, ключ не того окружения, отсутствующий заголовок ключа, устаревший timestamp | контроль → `code="0"`; каждая форма порчи → `code≠"0"` и код непуст | Перечень кодов, по которым граница опознаёт отказ кредов; точные коды — наблюдение RUN | RUN 2026-09-04 ✓ — перечень сохранён в `observations/SEC1_1.md`: 50119 / 50113 / 50105 / 50111 / 50101 — отказ кредов; 50103 / 50102 — дефект нашей стороны, разбор в `docs/integrations/okx/rules/auth-rejection-codes.md` |
