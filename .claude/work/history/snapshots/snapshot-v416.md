...
Сменяет v415. Фаза 1 — `FOLDED`. **Фаза 2 — `IN_PROGRESS`**: шаги 1-11
...
**Режим — автономия до прод-рубежа.** Решения — Д250-Д2065
(`.claude/work/decision-digest.md`).

## Что сделано после v415

- **Первый доковый заход `DOCS` шага 12 закрыт** — заход 134
  (`.claude/work/progress/phase-2-step-12-chronicle.md` §«Шестьдесят шестой
  заход (134) — первый доковый заход `DOCS`: набор тестов», Д2065).
- **Обзор набора тестов получил дом** — `.claude/tests/test-suite.md`: уровни,
  подмена внешнего мира, метки и выбор прогона, чтение исхода.
- **Приведено к коду:** состав `common/test-support` в
  `docs/architecture/services.md`, перечень общих артефактов в
  `services/README.md`, ложная клауза о `connector-okx` в
  `.claude/skills/test-code.md`; команда прогона красных клеток — в доме
  команд.

## Вход следующей сессии

**Единица — доковый заход `DOCS` шага 12: сверка доков с продуктовой дельтой
шага по артефакту `trading-core`.** Под-шаг 3 правил и `src/main`; перечень
артефактов и порядок — хроника шага, заход 134 (команда там же). Дальше —
`strategies`, `statistics`, `bff`, `common/model/domain`, `connector-okx`,
`auth`; после последнего — `REVIEW`.
