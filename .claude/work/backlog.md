...
**Чем шаг 5 закрыт без этого.** Коннектор наблюдает исполнение **опросом**,
как донор (`docs/rules/ack-not-runtime-truth.md`): гарантия полноты
фактов от способа наблюдения не зависит — она стои́т на наблюдении ядром,
а не на пуше. WS меняет **задержку**, а не гарантию.

**Владелец** — `integrator` (контракт источника) с `tester` (контур
прогона) и `code-writer` (слушатель).

## Входной контракт коннектора — проектирование операций в доменных типах

**Что сделать.** Спроектировать вход коннектора по перечню операций
`docs/architecture/contracts.md` §«Синхронные вызовы» (выставить,
заменить, отменить, закрыть, добыть факт) **в доменных типах**, а не в
примитивах и снапшотах; привести `docs/components/IntegrationService.md` к
целевой форме; портировать OKX-сторону под спроектированный вход.

**Оживит:** ход, берущийся за шаг 5 фазы 2 (коннектор OKX) — эта работа и
есть его содержание, а не хвост.

**Причина.** Найдено контактом с кодом на шаге 5
(`.claude/work/history/…/phase-2-step-5-docs-check-3.md`, находка X1):
копирование донорского интеграционного слоя даёт 184 ошибки компиляции, и
ни одна не механическая. У целевого коннектора **другой вход и другой
выход**, чем у донорского: донор возвращает снапшоты источника и принимает
типы командного слоя, целевой — «доменные запросы на входе, общая модель
на выходе» (`docs/architecture/services.md`). Порт копированием невозможен
по построению.

**Что уже сделано и остаётся сделанным.** Общие контракты, которые
попытка порта правильно опознала, перенесены в `libs/domain-model`:
`StatusResolveResult`, `OrderExternalStatusResolver`,
`AlgoOrderExternalStatusResolver`, `ProtectionHistoryLeg`, `ExchangeAck`.
Критерий границы, предъявленный компилятором: **контракт, чей параметр
сформирован источником, общим не бывает** (`PositionStatusResolver`
принимает снапшот и потому остаётся у коннектора).

**Отвергнутые альтернативы и их цена** — в отчёте X1: отложить коннектор
до шага 7 (меняет порядок роадмапа); портировать донорскую границу как
есть (клейм «на выходе общая модель» становится ложным, а снапшот
источника уезжает к ядру — против `raw-exchange-dto-boundary` и Д615).

**Владелец** — `solution-designer` (контракт) с `code-writer` (порт).
