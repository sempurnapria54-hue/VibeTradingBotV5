...
### S0-реестр. Слот шага 9 в реестре предусловий контура — не заведён

**Что не сделано.** Грунт S0 (перечень auth-отказов источника) держится
**двумя** носителями — этой секцией и полем `ждёт-грунта` компонента
`CredentialsRejectionResolver` в `.claude/work/code-gate-ledger.json`. В
**реестре гейтящих предусловий** контура
(`.claude/tests/source-api/okx/code-preconditions.md`) слота под него нет.

**Что от этого не работает и что работает.** Не работает: сверка
`bash tools/preconditions-check.sh` («реестр == план == фактика») шага 9 не
покрывает — её клейм верен только для слотов шага 7. Работает:
`py -3 tools/code-gate-check.py` компонент **не пропускает** — ось 15
роняет гейт на компоненте, начатом при открытом слоте грунта, поэтому
`CredentialsRejectionResolver` до сбора факта начат быть не может.

**Почему не заведено этим заходом.** Слот реестра связан с
**гейт-заголовком плана контура** и с носителем прогона — код-тестом,
которого ещё нет; `preconditions-check.sh` сверяет все три. Завести слот
без плана и носителя значит уронить сверку либо объявить пару, которой
нечем закрыться. Это работа владельца контура, а не побочный ход перехода
статуса.

*Оживит:* заход владельца контура по S0 — кейс с заведомо неверным ключом
на demo-контуре; слот заводится **вместе** с планом и носителем.
*Причина:* сегодня нечем закрыть пару «план × носитель».
*Владелец:* `tester`.

### S0. Перечень auth-отказов источника — грунт шага 9

**Дефицит — `грунт`, не работа.** Класс реакции на отвергнутые креды
выведен и записан (`docs/rules/exchange-hold.md` §«Ступень 2 —
сворачивание», п.5, код `EXCHANGE_CREDENTIALS_REJECTED`). Чего нет —
**фактики источника**: какими кодами и HTTP-статусами OKX отвечает на
отвергнутую подпись, отозванный ключ, неверную passphrase и
приостановленный доступ. В корпусе по этому предмету ноль носителей
(счётный греп по auth-кодам в `docs/rules/`, `docs/integrations/` —
0 попаданий).

*Оживит:* прогон контура `source-api` (`.claude/processes/source-api-testing.md`)
— достаточно одного кейса с заведомо неверным ключом на demo-контуре.
*Причина:* выдумкой не закрывается — резолвер обязан отличать отказ
кредов от прочих отказов по **фактическому** коду, а не по
правдоподобному. До сбора грунта тропа резолва не пишется.

**Порядок работ этим не блокируется:** дефицит несёт `грунт` **без**
сопутствующего `работа`, поэтому сбор идёт сразу, не дожидаясь чистоты
остальных прогонов (`.claude/processes/roadmap-step-execution.md`
§«Сбор внешнего грунта не ждёт чистого прогона»).
...
**Креды контура тестов источника — часть того же хода.** Коллекция контура
обращается к `POST /api/proxy/okx/raw` напрямую, и закрытие поверхности
принципалом потребует у неё предъявляемых кредов
(`docs/rules/api-access-policy.md`, §«Поверхность сырого вызова
источника»). Владелец — владелец контура; *оживит* — заход `CODE` шага 9,
которым поверхность закрывается; *причина* — сегодня предъявлять нечего,
принципала ещё нет.

**Носитель журнальной строки отказа доступа заведён:**
`docs/models/domain/other/AccessDenial.md` (модель, писатель, состав
полей). Кодовая дельта — таблица `access_denials` плюс запись из точки
входа отказа фильтр-цепочки; компонент реестра гейтов — `ApiAccessPolicy`.
