...
- **грунт `integrator` / `tester`** — **одиннадцать** гейтящих
  ...
  пп. 1, 2, 5, 6, 7, 9, 10, 14, 15, 16, **17**) и их кейсы: `AG1.5` (п. 1), `AG1.9` (пп. 1 и 15 — вторая
...
- **Итог сделки — по `docs/spec/deal-result.json`:** `resultProfit`
  считается суммой net по эпизодам плюс слагаемое чужой валюты; строка с
  неполученным курсом **блокирует** публикацию итога и завершение
  финализации выхода. Три новые колонки `positions` (см. схему выше) —
  правые операнды пар сверки.
- **Композиция четырёх пар сверки — по
  `docs/spec/pnl-reconciliation.json`:** левая сторона по категориям,
  правая суммами по эпизодам, конвенция знака сырая (финансирование
  де-нормализуется).
...
- **Ложный клейм `backlog.md` §«Агентское ревью всей кодовой базы
  (2026-07-02) — 2 блокера, НЕ закрыты»:** оба блокера **закрыты в
  коде** (проверено чтением `CreateAlgoOrderActionExecutor` и
  `EntryFinalizedHandler`), пути файлов в секции устарели. Секцию
  переписать или снять.
