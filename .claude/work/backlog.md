...
**Оживит:** роадмап упирается в инфраструктурные шаги — то есть шаг 2
фазы 2 доходит до места, где манифест перестаёт быть провайдер-нейтральным,
либо открывается условие 2 прод-рубежа (`.claude/work/roadmap/prod-gate.md`).
До этого момента проработка не делается: её материал (цены, лимиты,
доступность) протухает быстрее, чем роадмап до него доходит.
...
## Манифесты развёртывания сервисов

**Что сделать.** Завести на каждый построенный сервис его `Deployment`,
`Service`, `ServiceAccount` и пары `NetworkPolicy` по таблице
`docs/architecture/contracts.md` §«Синхронные вызовы»; зарегистрировать в
`deploy/base/kustomization.yaml`; проставить теги образов командой
`tools/deploy-set-image-tag.sh`. На момент парковки ждут `auth` (шаг 4) и
`connector-okx` (шаг 5).

**Оживит:** появление кластера в среде **и** выбор реестра образов
(ARCH-Q1, `.claude/work/questions/open-questions.md`). Первое — действие
держателя в среде, второе — его же развилка.

**Причина парковки.** Дефицит `носитель` и `грунт`, не `работа`.
`Deployment` обязан назвать: образ (реестр не выбран — до выбора в
`tools/deploy-set-image-tag.sh` стои́т `registry.invalid/vibetrading`),
адрес провайдера идентичности и его realm (провайдер в кластере, которого
нет), ресурсные лимиты — а их дом прямо говорит, что назначает их
разворачивающий **по наблюдаемому потреблению**
(`docs/architecture/platform.md` §«Чем различаются окружения»), то есть
наблюдение и есть недостающий факт. Манифест, написанный на догадках,
был бы переписан первым же настоящим развёртыванием, а проверить его до
кластера нечем: `kubectl` в среде нет.

**Тот же довод уже применён в корпусе** к `Ingress` и выдаче
сертификатов: они приезжают вместе с поверхностью, потому что «маршрут в
никуда и сертификат ни на что заводить незачем»
(`docs/architecture/platform.md` §Безопасность).

**Владелец** — `code-writer` (манифесты) с держателем (кластер, реестр).
...
**Оживит:** закрепление тега образа кластера
(`deploy/base/data/postgres-cluster.yaml`, сейчас
`PIN-ON-FIRST-DEPLOY-timescaledb-image`).
