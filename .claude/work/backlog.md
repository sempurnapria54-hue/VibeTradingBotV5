...
1. **Числа конфигурации не заданы.** Схема катастрофического потолка
   ...
   (`docs/rules/risk-policy.md` §«Конфигурация держит три числа»), здесь
...
5. **Смежное — операнд потолков на открытой просадке**
   ...
   **реализованной** просадке (`docs/rules/risk-policy.md` §«База
   невозрастающая у всех четырёх; вверх — только рукой»); расширение
...
**Заведено `GAPS_CLOSE_28`** (F4, F5 линзы F). Гейт грунта не снимется,
пока у гейтящих кейсов нет **чем прогонять**: слоты реестра
(`.claude/tests/source-api/okx/code-preconditions.md`) закрываются
наблюдением, а наблюдать нечем — исполнимый носитель есть у одного кейса
из десяти, и тот производит не то наблюдение.

**Что сделать, по порядку контура** (`.claude/processes/source-api-testing.md`):

1. **Достроить коллекцию** до соответствия плану на дельте — шестнадцать
   содержательных кейсов шага 7 в неё не входят.
2. **Провести `test-review` и аппрув пары «план + коллекция»** на этой
   дельте. Аппрув 2026-06-19 её не покрывает. **Это процессный гейт
   этапа 3, а не дефицит знания:** без него написание код-тестов не
   начинается. **Требует действия пользователя** — вынос назван в отчёте
   прогона.
3. **Написать код-тесты** (этап 4) на девять кейсов без носителя и
   перевести `AG6.1` на сохранение перечня, а не мощности.
4. **Поднять среду** — состав и блокеры в §«Средовой дефицит автономного
   RUN тестов (контур source-api)».

Владелец — `tester` (шаги 1, 3), `reviewer` (шаг 2), держатель (аппрув).
Пока пункт не исполнен, реестр держит `CODE` штатно, и это не дефект
реестра, а его назначение.
...
**Что сделать:** свести внутрифайловые адресные перекрёстия `docs/**` к
смысловым ссылкам на дом. Счёт пересобирается командой, а не хранится
здесь: `grep -ro '§«' docs/ --include=*.md --include=*.json | wc -l`.
...
- **X2. `Position.CloseReason.MANUAL_CLOSE` — то же** —
  `docs/models/domain/core/Position.md` §Енумы +
  `docs/models/mapping/Position.md`. Ручная остановка его не производит:
  закрытие идёт kill-switch'ем и несёт `KILL_SWITCH`.
...
- **CODE-дельта риск-контура, узел Н6 `GAPS_CLOSE_27`** (дома —
  ...
  - **пол дистанции стопа в `RiskValidator`**: накапливаемая проверка
    ...
    (`stopDistanceFloor`, `stopDistanceAboveFloor`);
...
**Происхождение.** Решение держателя по эскалации `DOCS_CHECK_29` —
`.claude/decisions/proof-method-change.md`: способ доказательства
корпуса меняется цельным проходом до следующего закрытия; предметная
концепция не трогается. `GAPS_CLOSE_29` открывается этим проходом и
дальше ведёт узлы посерийно по правленному процессу
(`.claude/processes/roadmap-step-execution.md` §«Узлы закрытия идут
посерийно»).

**Состав.**

1. **Мутационный замер по всему телу спек + доводка наличных
   предикатов.** Ввести воспроизводимую команду мутационного замера
   (нейтрализация каждой величины — объявленные проверки обязаны
   упасть) по всему телу `docs/spec/**`; величины, переживающие
   нейтрализацию, довести до стандарта приёмки — по прогону их 47 из
   350, среди них восемь create-проверок и пятнадцать инвариантов
   (цена диагноза — порядка 25 экземпляров предикатов). Стандарт —
   `roadmap-step-execution.md` §«Мутационная проба — условие приёмки
   спеки, а не только правки».
2. **Слияние четырёх домов закрытой формы риска на стопе в один.**
   Расхождения по знаку, клэмпу и якорю разрешаются в доме; остальные
   носители подключаются или ссылаются; потребители пересчитываются
   обходом (§«Узел отвечает за всех потребителей, не только за
   собственный механизм»).
3. **Ось C детектора областей видимости** — §«Энфорсер запрета копии
   формулы в исполнимой спецификации» (ниже); момент поднят решением:
   часть прохода.
4. **Симметричная охрана полноты графа.** Операнд полноты — каждому
   читателю, считающему агрегат по загружаемой коллекции; перечень
   читателей собирается механически (агрегатные операции спек плюс
   потребители прохода загрузки контекста); конструкция — та же, что
   у существующей охраны результата; `CODE`-дельта прирастает входным
   fail-fast гейтом полноты графа и его кодом.

**Владелец** — CC в ролях по карте делегирования. **Момент** — до
`GAPS_CLOSE_29`; узлы `_29` не открываются раньше прохода.

## Энфорсер запрета копии формулы в исполнимой спецификации

**Встречный якорь.** Правило есть, энфорсера у него нет.
`.claude/rules/structure.md` §«Таблица размещения» (строка `docs/spec/`)
объявляет копию формулы под другим именем или на другом префиксе
операнда дефектом — с доводом «расхождение копий не ловится ничем».
Сегодня и сама копия не ловится ничем: проверка, поставившая дословную
копию величины дома под другим именем в соседнюю спеку, оставила зелёными
**оба** инструмента — `tools/spec-run.sh` и `py tools/spec-scope-check.py`
(A: 0, B: 0).

Детектор областей видимости меряет две другие оси: разрыв подключения (A)
и подмену дома гостевым состоянием (B). Копия формы — третья ось, и её
нет.

**Что сделать.** Ввести ось C — «выражение, совпадающее с выражением
дома-величины, объявлено под собственным именем» — с падающей пробой в
`tools/spec-scope-probe.sh` по стандарту доказательства осей
(`.claude/processes/roadmap-step-execution.md` §«Оси проверочной команды
доказываются поимённо»). Осторожно с ложными срабатываниями: совпадение
выражений у **разных предметов** дублем не считается — это записано в том
же правиле, и ось обязана их различать, иначе она зашумит корпус.

**Владелец** — `knowledge-curator` (инструмент — не продуктовое знание).
**Момент** — поднят решением держателя 2026-08-30
(`.claude/decisions/proof-method-change.md`): ось C — часть
дизайн-прохода смены способа доказательства (§«Дизайн-проход смены
способа доказательства — вход `GAPS_CLOSE_29`» выше), исполняется до
`GAPS_CLOSE_29`. Прежняя оценка «до `CODE` не обязателен» снята:
единственность дома сквозной величины получила мандат энфорсера, а не
ответа на конкретный довод.

**Происхождение** — независимая верификация хвоста узла Н6, находка «за
периметром».
