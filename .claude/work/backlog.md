...
Закрыто: компонент-доки приведены к единой форме; составные
вопросы переформулированы; **исполнимая спецификация — 14 предметов,
157 примеров**, прогон зелёный; эталонная стратегия проверяется
и деревом, и **пошаговым проходом сделки**; исполнители получили
разделы под конструкцию риск-контура; контракт-доки источника и
инвентари нативных моделей очищены от пересказа политики, хроники
расследований и процессной арматуры (номера находок, кейсов, прогонов,
шагов роадмапа и открытых вопросов).

Остаток — **только выносы держателю**, работы по корпусу нет:

1. **Два числа конфигурации не заданы.** Схема катастрофического потолка
   сделки ратифицирована и приземлена (дом — `docs/rules/risk-policy.md`);
   держателю осталось задать при запуске максимальный риск на сделку и
   максимальный множитель. Пока не заданы — штатный отказ (первое —
   риск-создающего действия, второе — создания стратегии). Это ожидаемое
...
`docs/rules/live-risk-protection.md`; здесь — носитель со
стороны **события**.
...
- **грунт `integrator` / `tester`** — **одиннадцать** гейтящих
  предусловий (`docs/rules/pnl-reconciliation.md`, пп. 1, 2, 5, 6, 7, 9, 10, 14, 15, 16,
...
- **CODE R-слота и формулы риска** (дом политики —
  `docs/models/domain/aggregate/Deal.md` / §«Взятый
  ...
  - **на `Deal` — четыре производных числа** + общая
    ...
    `Deal.md` / §«Предикат отбора слагаемых»;
...
- **CODE-дельта катастрофического потолка сделки** (схема ратифицирована
  ...
  - **неравенство катастрофического потолка в `RiskValidator`** (четвёртый
    потолок, пятое неравенство): `dealNotional ≤
    globalSimultaneousRiskPerDealPercent / 100 × база ×
    strategyCatastrophicRiskPerDealMultiplier`, операнд — по runtime graph
    (`liveLegsNotional + livePositionNotional`), код `DEAL_NOTIONAL_EXCEEDED`
    (в `RiskCheckCode` уже есть); отдельной константы кэпа номинала **не
