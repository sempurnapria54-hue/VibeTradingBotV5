...
- **CODE R-слота и формулы риска (`GAPS_CLOSE_7`, H9/H10; расширено
  H5/H6 `DOCS_CHECK_10`):**
  - `Deal.plannedRiskAmount` / `plannedRiskCurrency` **+
    `plannedEntryPrice` / `plannedSizeContracts`** + миграция; пишет
...
- **CODE cross-ccy (`GAPS_CLOSE_7`, H4; CCY-Q1 закрыт):** сравнение `ccy`
  ...
  Σ(`amount` × `appliedRate`) считает финализатор. Носитель расчётной
  валюты **определён** (`docs/decisions/instrument-currencies-home.md`);
  открыт только `CCY-Q2` (имя поля).
  ⚠ **Хэнд-офф `integrator` — эндпоинт котировки не назначен** (H11
  `DOCS_CHECK_10`): выбрать между `GET /market/ticker` (метод клиента
  `getTicker` **уже построен**, но пара `<CCY>-USDT` спотовая и вне
  SWAP-контура) и `GET /market/index-tickers` (не требует онбординга
  спота, но метода клиента нет); проверить **доступность пар котировки**
  при SWAP-only контуре; задать **ось запроса** (из пары `ccy` →
  settle-ccy построить идентификатор инструмента котировки) и завести
  строку в манифесте покрытия. Политика при недоступном курсе **уже
  задана** (`docs/components/RefreshBillsExecutor.md` §«Носитель курса»):
  `appliedRate = null`, слагаемое не вносится, `AnomalyReport`
  `CROSS_CCY_RATE_UNAVAILABLE`; курс не подставляется.
- **Хэнд-офф `integrator` — дом конфига исключений сверки по бирже**
  (H14 `DOCS_CHECK_10`). Область Σ-сверки задаётся перечнем `type`/`subType`,
  **исключаемых** из экономики сделки, и перечень **конфигурируется по
  бирже**. Определить: где живёт носитель настройки и кто его правит при
  добавлении новой биржи. До назначения дома список для OKX **пуст** —
  посторонние типы, если они есть, проявятся через непустую корзину
  `OTHER` (`AnomalyReport` `UNCLASSIFIED_CASH_FLOW`) и расхождение сверки
...
- **H14 — форма epsilon слепнет с ростом оборота — ЗАКРЫТО** (H15
  `DOCS_CHECK_10`). Допуск `0.5%·Σ|amount|` рос **линейно** с числом
  движений, а охраняемая ошибка (межэндпоинтное округление) — как
  `N × 0.01`; при `N ≳ 200` целое пропавшее движение проходило молча,
  то есть сверка слепла на **grid/partial** — целевом семействе бота.
  Закрыто **двухчастным тестом** (`min(композиционный, омиссионный)`):
  омиссионный член якорится на ожидаемую комиссию сделки и от числа
  движений не растёт (`docs/decisions/pnl-finalization-mechanics.md`
  реш.5 §epsilon). Остаётся провизорной только **калибровка величин**
  (`0.01`, `0.5%`, `k`) — она уже числится отдельным пунктом выше.
