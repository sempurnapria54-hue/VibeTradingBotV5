...
- **Ни того, ни другого в енуме** — `docs/models/domain/core/Exchange.md`
  (`CREATED`, `PENDING`, `ACTIVE`, `CLOSED`, `ERROR`); нота о парковке
  долга — там же §Енумы.

Имя в обороте — **`TRADE_BLOCKED`**. Сведение имени + материализация
статуса делаются **одним ходом** с полным lifecycle (весь набор
состояний разом), не точечным переименованием.
...
- **CODE fee-wiring (N9, доспецифицирован на `GAPS_CLOSE_3`, доведён на
  ...
  **гидрация ставки — в `InstrumentExternalRulesDataService`** (хранилищный
  ...
  инструментов группы по несвежести** ставки — kill-switch + `AnomalyReport` +
  ручное снятие (порог в конфиг, стартово 24 ч; H3/H4).
...
- **Двухосевое искажение измеряемой ожидаемости (торговый форвард-фокус):**
  переформулирован на `GAPS_CLOSE_4` — прежняя редакция («конвергенция трёх
  оптимистичных смещений») картину **недоописывала**: она знала только одну
  ось. Осей **две**, они не гасят друг друга и требуют разных поправок.
  - **Ось исходов — крен оптимистичный** (число лучше правды): H6 null-drop
    (`pnl-finalization-mechanics.md` реш.3) + N11 недосчёт агрегации + опущенный
    гэп-проскок (TR2, `per-trade-risk-policy.md` §«Без поправки на проскок») +
    **H16** — cross-ccy движение помечается `AnomalyReport`, но
    `Deal.resultProfit` уже записан net'ом и **не корректируется**, то есть
    завышает молча (`trading-constraints.md` §«Валюта комиссии» сам это
    признаёт). H16 — четвёртый механизм, условный на нарушении инварианта;
    урок H6 («пометка фиксирует факт, но не устраняет смещение») к нему **не
    применён**.
  - **Ось возможностей — крен пессимистичный** (сделок меньше/мельче правды):
    **H13** taker-консерватизм при maker-входах = систематический недосайзинг
    ...
    [Kaufman гл.1, PDF с.114-119].
  - **Почему это один фокус, а не два пункта.** Крены живут на **разных осях и
    в разные стороны**, поэтому в сумме не компенсируются, а делают сравнение
    **бэктест ↔ live двусторонне несопоставимым**: оптимистичный по исходам
    сдвигает распределение R, пессимистичный по возможностям — его объём и
    состав. Мерить одно, не зная другого, нельзя. Владелец — фаза ожидаемости
    (`progress/phase-1-step-7-docs-check-4.md` §Сводка, Lens C).
...
- **H14 — форма epsilon слепнет с ростом оборота** (форвард; радиус ограничен):
  допуск `0.5%·Σ|amount|` растёт **линейно** с числом движений, а охраняемая
  ошибка (межэндпоинтное округление) — как `N × 0.01`. При `N ≳ 200` целое
  пропавшее движение проходит молча; `N` в сотнях реалистичен (funding 3×/сутки
  + partial-филлы grid) — то есть сверка слепнет на **grid/partial**, целевом
  семействе бота. Это свойство **формы**, а форма объявлена непровизорной
  (`mapping/DealCashFlow.md` §«Сверка и cross-ccy»: «величина провизорна, якорь
  — нет»). **Радиус ограничен:** epsilon гонит только `AnomalyReport`, не число
  и не терминал ⇒ «монитор слепнет», а не «число неверно» — потому не гейт.
  Пересмотр — вместе с калибровкой величины (бэктест/пользователь).
