...
**Решение держателя 2026-09-01** — `.claude/decisions/env-wait-deadline.md`
(там же состав решения, отвергнутые альтернативы и что снято фактами из
UI). Здесь — только что делать по наступлении срока.

**Что делать 2026-09-04, до первой же работы захода:**

1. **Замерить среду** — контрольный прогон по осям дома
   (`.claude/tests/source-api/okx/code-preconditions.md`
   §«Что среда всё ещё не даёт»). Замер идёт **чтением состояния уже
   висящих заявок**, новых не подаём.
2. **Среда ожила** → закрыть слоты п. 2 × `AG6.2` и п. 7 × `AG1.7`
   перепрогоном кейсов, затем кодировать `RefreshBillsExecutor` и
   `DealCashFlow`; снять поля `срок`/`по-сроку` у обоих компонентов
   реестра.
3. **Среда не ожила** → вернуть держателю `ENV-Q1` **с состоянием на тот
   момент**: что менял тикет 38426982, что показал контрольный замер,
   сколько заявок висит. Перечень вариантов — из решения (продлить срок,
   включить заменители, завести второй аккаунт, вывести слоты из-под
   гейта). **Численный порог CC не предлагает.**

**Чем срабатывает:** поле `срок` у ожидания грунта обоих компонентов в
`.claude/work/code-gate-ledger.json`; `python3 tools/code-gate-check.py`
роняет гейт по наступлении (оси 20-24) и называет оба компонента
поимённо. Пока срок не наступил, гейт зелен — проверено пробой на
подставленных датах.

**Пока ждём — живых заявок не подаём** (просьба поддержки + хвост из пяти
неснимаемых `live`). Область ограничения — всякий кейс контура,
создающий заявку, а не только фикстурный; дом ограничения —
`.claude/tests/source-api/okx/code-preconditions.md`
