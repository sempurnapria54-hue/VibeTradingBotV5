...
- **Схема** (пре-лонч политика: таблицы пусты, бэкфилла нет,
  ...
  - `+exchanges.risk_base` `numeric(36,18)`, `+exchanges.risk_base_currency`
    ...
    DEFAULT 0` — носители невозрастающей базы и серии убытков
...
- **Энфорсер запрета входа в окне сворачивания** —
  `TranchePrecheckHandler` получает операнд статуса сделки и четвёртую
  ветвь исхода условия; форма — `docs/spec/deal-tranche-lifecycle.json`
...
- **Судьба встроенной защиты при терминале родителя с непустым наливом**:
  `missingAttachedOutcome` получает операнд налива родителя; матч живой
  записи — по **клиентскому** идентификатору
  (`docs/lifecycles/Order.md` §«Отсутствие встроенной защиты в
  снапшоте»).
...
## Носители прогона гейтящих слотов шага 7 — хвост из одного слота

**Исполнено 2026-08-30.** Носители написаны, среда поднята, контур
прогнан автономно; **закрыто одиннадцать слотов из двенадцати**.
Состояние, добытые факты и воспроизводимые команды —
`.claude/tests/source-api/okx/code-preconditions.md`; хроника — chronicle
шага.

**Что осталось — один слот:**

1. **П. 7 — горизонт `fundingFee` (`AG1.7`).** Требуется удержание
   позиции через границу funding-интервала (00:00 / 08:00 / 16:00 UTC);
   ветвь сверки в код-тесте уже есть, нужен только прогон в подходящее
   окно. Владелец — `tester`.
...
число и перечень; оси инструмента доказаны падающими пробами
(`bash tools/anchor-probe.sh`). Число здесь **не хранится** — оно
пересобирается прогоном.

**Что сделать:** пройти перечень, у каждого адреса восстановить дословное
имя целевого пассажа либо снять адрес, если целевого пассажа больше нет.
Механическая половина (адрес — сокращение существующего имени) уже
исполнена этим закрытием; осталось то, где целевого имени нет ни в одном
кандидате, — это переименования и переезды, и разбирать их надо по
одному.
...
- **X1. `ShutdownReason.MANUAL_STOP` — писателя нет и ручкой не
  появляется** — `docs/models/domain/aggregate/Deal.md` §Енумы +
  `docs/lifecycles/Deal.md` §«Причина сворачивания». Полный ручной режим
  поднимает ступень, а сворачивание сделки приходит существующей строкой
  каскада (`EXCHANGE_HOLD` / `RISK_POLICY`); различение «ручное /
  автоматическое» на стороне сделки не воспроизводится — оно живёт в
  `AnomalyReport.code`. Отсюда исходная развилка находки P5 (снять
  значение без достижимого писателя) ручной поверхностью **не
  закрывается** и остаётся в силе.
...
- **X9. `closeReason = RISK_POLICY` при ручном сворачивании
  инструмента** — `docs/lifecycles/Deal.md` §«Причина сворачивания».
  Каскад ручного полного режима на инструментном радиусе ставит причину
  риск-политики, хотя риск-политика не нарушалась; различение сегодня
  живёт только в `AnomalyReport.code`. Назвать это в доме причины либо
  развести значение.
- **X11. Исключающий ключ доведения недоделанного (CODE)** —
  ...
  инвариант `concurrentKillSwitchOnScope` в `docs/spec/manual-halt.json`.
...
- **Невозрастающая база риска** — `Exchange.riskBase` /
  `riskBaseCurrency`: **первое значение пишет рефреш баланса** той же
  транзакцией, что приземляет снимок средств, и только на строго
  положительном остатке; вниз двигают **оба терминальных исполнителя**
  (`MarkDealClosedExecutor`, `MarkDealEmergencyClosedExecutor`) после
  коммита терминала — финализация выхода писателем базы **не** является;
  вверх — только явное действие держателя; все четыре потолка
  считаются от снимка на сделке. Пустая база даёт отказ, а не ноль, и
  ненаблюдение отличимо в данных — формы и коды в домах
  (`docs/rules/risk-policy.md`, `docs/spec/risk-limits.json`,
  `docs/components/RefreshBalanceExecutor.md`). Колонка уже nullable —
  схема правки не требует.
...
- **CODE-дельта `GAPS_CLOSE_21`** (дом каждой позиции назван, здесь —
  ...
  - **база риска** — величина, её движение и её единственность для всех
    ...
    `Deal.plannedRiskEquityBase`) — C6 `DOCS_CHECK_21`. Прежняя редакция
    позиции воспроизводила **снятую** форму — «строка `Balance`
    расчётной валюты, читаемая на ходу»; действующая база невозрастающая
    и хранимая (F14 `DOCS_CHECK_28`);
  - **операнды сделочных лимитов считаются в `RiskValidator` по
    ...
    здесь они не воспроизводятся. Прежняя редакция несла отменённое
    слагаемое `incurredCanceled` у `dealRiskTaken` — отмена записана
    ниже в этом же файле, в дельте `GAPS_CLOSE_23`
    — C1-C3 `DOCS_CHECK_21`, F14 `DOCS_CHECK_28`;
...
- **Калибровка epsilon сверки bills↔net (N10):** величины (`0.01`,
  ...
  `GAPS_CLOSE_19`; реестр §«Что в реестр НЕ входит», п. 8): прежняя
...
- **`mvnw`/wrapper в репо — ОСТАЁТСЯ.** Прогон опирается на дистрибутив
  Maven, лежащий в кэше wrapper'а пользователя
  (`~/.m2/wrapper/dists/apache-maven-3.9.11-bin/*`), и на вручную
  собранный `PATH`. В репозитории нет ни `mvnw`, ни `mvnw.cmd`; на чистой
  машине прогон не воспроизводится.
...
- **Переносимость инструментов — СНЯТА.** `tools/spec-run.sh` и
  `tools/spec-scope-probe.sh` зашивали разделитель classpath, путь JDK и
  лаунчер `py` в форме Windows и на Linux не запускались вовсе;
  обе команды сделаны переносимыми.
...
**Происхождение** — `GAPS_CLOSE_29`, §«Что найдено попутно».
