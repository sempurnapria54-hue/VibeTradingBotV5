...
вопросы переформулированы; **исполнимая спецификация зелёная** — счёт
не переписывается сюда, а берётся прогоном `bash tools/spec-run.sh`
(на 2026-08-29 — 17 спецификаций, 221 пример, все сошлись; прежняя
запись «14 предметов, 157 примеров» отстала от перечня `docs/spec/`,
P4 `DOCS_CHECK_27`); эталонная стратегия проверяется
...
- **Схема** (пре-лонч политика: таблицы пусты, бэкфилла нет,
  ...
  - `deals` в остальном без изменений: слот, окно линковки, результат,
    признаки отбора, четыре числа риска, эпизоды — агрегатные.
...
- **гейт грунта** — все слоты реестра закрыты. Дом правила, таблица
  ...
  **перечень не дублируется**: пересказ реестра в рабочем файле и был
  ...
  Величины читаются из таблицы реестра, а не из этой строки: на
  2026-08-29 — тринадцать слотов, одиннадцать пунктов, десять кейсов,
  закрыто ноль.
...
- **X4. Перечень писателей safety-статусов инструмента** —
  `docs/lifecycles/Instrument.md` §«Safety-статусы вне этого охвата».
  **Закрыт в раунде 3** правкой самого пассажа: ручной вызов держателя
  внесён в перечень писателей. Строка оставлена для трассы находки.
...
- **Аварийный терминал пишет все четыре признака отбора**, каждый по
  ...
  §«Признаки отбора»); ветка числа переключается на `resultAvailable`, а
  не на добытость положений закрытия.
...
- **Невозрастающая база риска** — `Exchange.riskBase` /
  `riskBaseCurrency`: вниз двигает финализация сделки, вверх — только
  явное действие держателя; все четыре потолка считаются от снимка на
  сделке (`docs/rules/risk-policy.md`, `docs/spec/risk-limits.json`).
- **Энфорсер П1** — счётчик `Exchange.consecutiveLossCount`, конфиг
  `globalConsecutiveLossLimit`, реакция мягким холдом биржи
...
- **CODE-дельта `GAPS_CLOSE_22`** (дом каждой позиции назван, здесь —
  ...
  - **комиссионная компонента строки** берётся по
    ...
    `amount − externalFee = pnl`»; пустота — `docs/rules/absent-value-semantics.md` (B3);
...
- **CODE-дельты `GAPS_CLOSE_25` нет, и это не пропуск регистра** (P5
  `DOCS_CHECK_27`): штатного закрытия `_25` не было — детектор стагнации
  сработал на входе в `GAPS_CLOSE_25` по трём сигналам из четырёх,
  держатель выбрал «сменить концепцию», и дальше шло переписывание
  корпуса (хроника шага — `.claude/work/progress/phase-1-step-7-chronicle.md`
  §«Смена концепции»). Дельта прогона `_25` в `CODE` не уезжает, потому
  что закрытия, которое бы её произвело, не было.
...
- **CODE cross-ccy** (политика —
  ...
  null`, `rateStatus = RATE_UNAVAILABLE`, слагаемое не вносится, курс не
  подставляется. **Догон курса — нога цикла добычи, не бессрочный долг:**
  строка блокирующей области без применённого курса означает «факт не
  добыт», звено не завершается и повторяется по бюджету попыток своего
  исполнения; исчерпание бюджета — отказ строки исполнения, уровень 3 и
  аварийный терминал. Дом — `docs/rules/pnl-reconciliation.md`
  §«Ожидание курса чужой валюты»; собственного срока ожидания, счётчика
  и детектора не заводить.
...
- **CODE-дельта риск-контура, узел Н6 `GAPS_CLOSE_27`** (дома —
  ...
  - **`BREAKEVEN`** — значение `StopLossCalculationType` **уже заведено в
    ...
    формуле `якорь × (1 + f)/(1 − f)` / `якорь × (1 − f)/(1 + f)`, без
    `distancePercents`) и **два** правила create-валидации: «доля
  ...
  - **формула `Deal.plannedRiskAmount` изменилась**: выбывшая входная нога
    входит налитой долей, а не выпадает целиком; пересчёт — те же семь
    исполнителей, формула — дом `docs/spec/deal-risk-numbers.json`
    (`dealPlannedRisk`). Кумулятивный потолок берёт это же число, второго
    слагаемого `incurredCanceled` в `RiskValidator` **не заводить**.

- **Вопрос: числа риска отчётности мерятся от плановой цены входа, а не от
  фактической** (поднят узлом Н6 `GAPS_CLOSE_27`, свип предмета 2; **дом —
  `docs/models/domain/aggregate/Deal.md` §«Четыре числа риска»**, форма —
  `docs/spec/deal-risk-numbers.json`).

  `legSignedStopDistance` (`:51`) и `legRiskAtCurrentStop` (`:56`) берут
  `Order.plannedEntryPrice`. При фактическом входе, отличном от планового,
  это то же смещение, что было закрыто у безубытка
  (`docs/rules/risk-policy.md` §«Пол дистанции стопа», якорь), только в
  отчётности. **Счёт:** нога 100 контрактов, `plannedEntryPrice` 3000,
  фактическая средняя 3020, `stopCurrent` 2910, `contractValue` 0.1,
  ставка 5 б.п. Корпус даёт `legRiskAtCurrentStop` = 90 × 100 × 0.1 +
  0.0005 × 5910 × 100 × 0.1 = **929.55**; от фактической средней —
  110 × 100 × 0.1 + 0.0005 × 5930 × 100 × 0.1 = **1129.65**. Занижение
  200.10 (**17.7 %**), и оно **разрешающего** направления. Следствие:
  `protectionRelievedRisk = episodeIncurredRisk − episodeRiskAtCurrentStop`
  на ту же величину **завышен** — отчётность утверждает, что защита сняла
  больше риска, чем сняла. Операндом потолков эти числа не служат
  (`docs/rules/risk-policy.md` §«Заявленное и взятое разведены»), поэтому
  потолок не пробивается; но П1 §3 требует называть направление, а оно не
  названо.

  **Что проверено и закрыто счётом, а не осталось вопросом:**
  `feeRate` (`deal-risk-numbers.json:46`) обязан остаться на **плановых**
  ценах — он восстанавливает ставку обращением закрытой формы сайзинга, и
  подмена якоря его ломает. Счёт: `plannedRiskAmount` = 929.55 даёт
  `(92.955 − |3000 − 2910|) / (3000 + 2910)` = **0.0005** (точно); с
  подстановкой фактической средней — `(92.955 − 110) / 5930` =
  **−0.00287**, то есть издержка превращается в ребейт. Прежняя редакция
  отчёта Н6 распространяла этот довод на все три величины безусловно —
  верен он только для `feeRate`.

  **Вопрос владельцу дома:** должны ли `legSignedStopDistance` и
  `legRiskAtCurrentStop` мериться от фактической средней цены живого
  эпизода (при сохранении плановых цен внутри `feeRate`), и что делать с
  ногами закрытых эпизодов, у которых своей средней уже нет.
...
**Оба блокера закрыты в коде** — проверено чтением 2026-08-29 (P1
`DOCS_CHECK_27`): `CreateAlgoOrderActionExecutor` строит `Condition` с
`trigger`/`trailing` из `getCalculatedPrice()`
(`domain/deal/action/CreateAlgoOrderActionExecutor.java`, `buildCondition`),
`EntryFinalizedHandler.toManagingIfProtected` проверяет **активность**
защиты (`hasActiveAttachedProtection()`), не её наличие
(`domain/deal/handler/EntryFinalizedHandler.java`). Прежний заголовок
секции («2 блокера, НЕ закрыты») и пути файлов в ней были ложным клеймом:
его констатировала §«Гигиена, заведённая прогоном `_26`» этого же файла с
предписанием «переписать или снять», и предписание не исполнялось.
