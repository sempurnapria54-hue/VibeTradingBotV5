...
- **Строка наблюдения одна на ОКНО, а не на всё стоящее состояние.** Дедуп
  журнальной строки гейтится окном подтверждения, поэтому по истечении окна
  наблюдение записывается заново. Дом правила требует «одна, пока состояние
  держится» (`docs/rules/error-handling-policy.md`); разведение двух ролей
  одной строки — дедупа и операнда подтверждения — требует **отдельного
  носителя** «последнее наблюдение», которого в модели нет.
  *Оживит:* ход, вводящий такой носитель, либо пересмотр окна по первому
  периоду прогонов (§«Гистерезис детекторов аномалий»), где станет счётно,
  сколько строк реально накапливается. *Причина:* без носителя выбор между
  «дедуп без окна» (детектор не перевзводится после ручного снятия ступени)
  и нынешней формой не выводится.
