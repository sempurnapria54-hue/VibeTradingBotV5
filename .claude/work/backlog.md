...
- **Искажение измеряемой ожидаемости: две оси × две стороны (торговый
  ...
  - **Исходы × оптимистично** (число лучше правды): H6 null-drop
    ...
    проскок») + **H16** — cross-ccy движение помечается `AnomalyReport`, но
    ...
    признаёт). H16 — механизм, условный на нарушении инварианта; урок H6
    («пометка фиксирует факт, но не устраняет смещение») к нему **не
    применён**.
  - **Исходы × пессимистично:** в фазе 1 пусто. Механизм-кандидат
    (kill-switch по несвежести усекал живые сделки по рынку в
    некоррелированный с рынком момент, `EMERGENCY_CLOSED` с реальным числом
    входил в R-выборку) снят `GAPS_CLOSE_5` (H2: kill-switch по несвежести
    убран).
  ...
  - **Возможности × оптимистично:** пусто.
