...
### P1. Референс-доки для `code-writer` перед `CODE` первого шага

Перед под-шагом `CODE` первого шага роадмапа, где появится код,
приложить референс-доки — источник примеров для example-скилла
`code-writer`. Первый набор создаётся через чат; дальше
референс-доки растут итеративно по мере накопления примеров.
Состав тулинга и контекст решения — в
`.claude/notes/2026-05-29-ростер-тулинга-роадмап.md` (раздел
«Код → `code-writer`»).
