...
- **Проброс Vault-токена — механизм СНЯТ, токен ПРОТУХ.** Механизм жив:
  `.env.vault.test.local` читается `source`'ом из shell,
  `tools/boot-test.sh` его использует. Регресс 2026-08-31: Vault
  пересоздан (не-dev режим), токен в файле новым инстансом не принят —
  автономный бут из shell не поднимется до перевыпуска (следующий пункт).
- **Токен `.env.vault.test.local` перевыпустить — ОСТАЁТСЯ.** Требование
  `.env.vault.test.local.example` — токен, ограниченный тестовыми путями;
  прежний токен читал и prod-пути, нынешний невалиден вовсе (факты — дом
  среды, §«Что среда всё ещё не даёт»). Scoped-политики и выпуск —
  адресат шага 9 «Безопасность».
...
Владелец — `knowledge-curator`. Класс `ИЗМЕРЕНИЕ`; заморозка снята
входом в `CODE`, но эти позиции продукта не гейтят.

- **A2** — популяция записи «суррогат границы» в `tools/retired-check.py`
  не содержит дом адресуемой единицы эпизода; шаблон встречную форму не
  видит. Возврат — **перед кодовым заходом по `RefreshPositionExecutor`**
  (правка дома и есть то, что популяция обязана поймать).
- **B4** — популяция `stepEligible` и `tools/derive/step-eligibility-classes.sh`
  объявляют две оси при трёх конъюнктах правила. Возврат — **перед кодовым
  заходом по `StrategyActionOrchestrator`**: ложный клейм адресован ровно
  тому исполнителю.
- **B9** — `derive.from` популяции ветви свежести указывает на саму
  проверяемую спеку. Возврат — **перед кодовым заходом по
  `MarketDataExpirationChecker`**.
- **E2** — популяция ветвей свежести объявляет достижимыми два члена,
  невозможных по формулам дома операндов. Возврат — тот же заход.
- **F5б** — ложный клейм строки колонки «Носитель прогона» проходит
  зелёным: клейм читается только формой `<Класс>LiveTest`. Возврат —
  сбор грунта по открытым слотам.
- **G1** — `retired-check.py` объявляет область `tools/**`, свипает
  `tools/*`; реестр гейтов вне области вовсе. Возврат — курационный заход
  после первого кодового захода.
...
- **E1** — клейм `docs/concept.md` §Ссылки «переименование величины
  ...
  курационный заход после первого кодового захода.
- **E3** — `edgeDeclared` и `transitionAllowed` объявлены в двух спеках
  каждое при правиле «величина объявляется ровно в одной спеке». Возврат —
  **перед кодовым заходом по `DealStateMachine` и `DealTrancheStateMachine`**.
- **F5а** — реестр предусловий несёт ложный клейм «ось 3 грепает по
  коллекции»; носителя нет в популяции записи о снятой роли коллекции.
  Возврат — сбор грунта по открытым слотам.
...
- **A4а** — посылка однородности покрытий сверки (таймстамп bill входного
  ...
  Возврат — **перед кодовым заходом по `DealOpeningService`**; цена
  ложной посылки счётна (допуск вырождается в пол, каждая восстановленная
  сделка уходит в `MISMATCHED` со ступенью 1).
- **A4б** — клейм однородности стои́т в трёх доках безусловно, а его
  неподтверждённость названа в одном и процессной арматурой внутри
  продуктового корпуса. Возврат — тот же заход: либо замер подтверждает
  посылку, либо клейм становится условным.
