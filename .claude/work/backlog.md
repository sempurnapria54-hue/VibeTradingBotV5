...
- у диспетчера — **именованный** ловец контролируемого исключения рядом с
  ловцом отказа кредов: строка закрывается отказом, бросок идёт наружу
  нетронутым (`docs/components/ServiceCommandExecutor.md` §«Контракт
  броска»);
- **компенсатор `controlledViolationHold` не портировать.** Он поднимает
  жёсткую биржевую ступень по коду `VALIDATION_ERROR` на строке
  исполнения, а этот код носит и **наш собственный** дефект инварианта:
...
**Отсюда приоритет сместился с переписывания на измерение.** Переписать
можно и потом; без замера каждый следующий сервис заводит свою порцию
заново. Ось замера предъявлена и работает — греп всех
`(docs|.claude)/…\.(md|json)` из тела кода с проверкой существования
файла; остаётся дать ей дом в исполнимой команде и область **всех** живых
деревьев кода, а не одного.
...
**Условие возврата сработало: шаг 7 фазы 2 открыт, доковая часть закрыта
`GAPS_CLOSE_1`.** Дом реестра назван — `trading_core`, ключ уникальности
пересобран на биржевой счёт (`docs/models/domain/other/TradeFeeRate.md`
§Персистентность); строка перечня таблиц дополнена
(`docs/architecture/data-ownership.md`). Остаётся **кодовая** часть — ряд и
его синк в порте ядра, и она идёт CODE-заходом шага 7.

**Что сделать.** Завести ряд `TradeFeeRate` и его синк там, где есть
биржевой счёт: чтение ставок требует ключей, а гидрирует навес правил
инструмента тот, кто ставку хранит.

**Оживит:** шаг 7 фазы 2 (`trading-core`) — он держит торговое состояние
счёта и он же единственный потребитель прогнозной комиссии в сайзинге.

**Причина парковки — не отсрочка, а граница.** В монолите ставки собирал
тот же тик, что и правила инструмента; в сервисной конструкции это
невозможно: `market-data` ходит к площадке ТОЛЬКО публичными чтениями
(`docs/architecture/contracts.md` §«Синхронные вызовы»), а ставка —
атрибут комиссионного уровня СЧЁТА
(`docs/models/domain/other/TradeFeeRate.md`). Навес инструмента несёт
только КЛЮЧ комиссионной группы, и его `market-data` собирает как прежде.

**Что от этого не страдает сейчас.** Потребителей прогнозной комиссии до
шага 7 не существует: сайзинг и преконтроль риска приезжают с ядром.

**Владелец** — `code-writer` с `solution-designer` (ось владельца модели).
