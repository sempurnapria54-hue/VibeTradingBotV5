...
## Контракт броска диспетчера команд разошёлся с кодом — предмет шага 7

**Найдено пост-хок концепт-гейтом шага 9 (2026-09-04), но дельтой шага 9 не
является:** предмет — машинерия шага 7, и правка её внутри закрытия чужого
шага была бы работой без своего гейта.

**Что расходится.** `docs/components/ServiceCommandExecutor.md` §«Контракт
броска» объявляет, что **контролируемое исключение интеграции** переводит
строку в отказ **и бросается** — и что его ловит выделенный обработчик
оркестратора, поднимающий безусловную биржевую ступень 2
(`docs/rules/controlled-exchange-exceptions.md`). В коде диспетчер ловит его
общим `catch (RuntimeException)`, классифицирует `VALIDATION_ERROR` и
**возвращает результатом**; наружу оно не выходит.

**Почему это не обязательно дефект кода.** Два рефреш-исполнителя ловят
контролируемое исключение **сами** — ровно как предписывает дом категорий
(«граница рефреша ловит исключение, обновляет сущность и отдаёт факты
обработчику»). Возможно, остальные тропы к нему и не приходят, и тогда
расходится **док**, а не код. Возможно, приходят — и тогда безусловная
ступень 2 на этих тропах не поднимается вовсе.

**Что требуется, чтобы закрыть:** перебрать исполнителей, у которых
контролируемое исключение достижимо и не ловится ими самими, и по исходу
править либо док, либо диспетчер. Это работа с популяцией троп шага 7.

*Оживит:* курационный либо кодовый заход по шагу 7 — тот же, что держит
прочие его хвосты.
*Владелец:* `solution-designer` (развилка «док или код») + `code-writer`
(правка).
...
«в конце фазы 1»), не сейчас.** В целевой конструкции числа принадлежат
тенанту и назначаются его владельцем (`docs/architecture/tenant-and-exchange.md`);
пункт остаётся: первый тенант — держатель. До тех пор `application-prod.yaml` заполнен **значениями
профиля test** — demo-контур, ненастоящие деньги. Числа стоя́т не потому, что
их выбрали для боя, а потому, что `RiskAppetiteStartupCheck` иначе не даёт
приложению подняться в `prod`, и пустой профиль блокировал бы всякую работу с
боевым контуром до конца фазы.
...
**Что сделать до перевода прод-рубежа в `DONE`** — держатель назначает три числа
секции `risk-appetite` в `donor/src/main/resources/application-prod.yaml` (либо
переменными окружения того же имени):

| Ключ | Единица | Что задаёт | Временное значение |
|---|---|---|---|
| `global-simultaneous-risk-per-deal-percent` | проценты базы риска | максимальный риск на сделку — потолок одновременного риска и первый сомножитель катастрофического потолка | `1.0` (из test) |
| `global-catastrophic-risk-per-deal-multiplier` | безразмерный множитель | предел множителя, которым стратегия вправе растянуть катастрофический потолок; он же кэп суммарного номинала сделки | `100.0` (из test) |
| `global-consecutive-loss-limit` | количество сделок | сколько подряд ценово-убыточных закрытых сделок останавливают торговлю целиком | `3` (из test) |
...
- **Мультибиржевая корректность прохода.** Срез читается один на тик и
  применяется ко всем строкам `exchanges`; контур биржей не сужается.
  *Оживит:* заведение второй строки биржи (горизонт — фаза 6 Bybit; коннектор по площадке — `docs/architecture/services.md`; там же
  advisory-замок мультиинстанса). *Причина:* сужение по бирже требует
  per-exchange интеграционного сервиса, которого в фазе 1 нет.
...
- построить **ручную сервисную поверхность** safety-остановки — обе
  ...
  **Остаётся постановка** — обе ступени, оба радиуса. Имя ступени в
  ...
  (`HoldRung.SOFT` / `HARD`), и мягкая биржевая ступень выражается парой
  `(EXCHANGE, SOFT)`, а не третьим значением.
...
- **CODE-дельта `GAPS_CLOSE_21`** (дом каждой позиции назван, здесь —
  ...
  - **`+strategy_details.strategy_simultaneous_risk_per_deal_percent`**
    ...
    (`globalSimultaneousRiskPerDealPercent`; носитель заведён —
    `RiskAppetiteProperties`, значение живёт в конфиге профиля и здесь не
    называется), прежнее безмаркерное имя снято (C9);
...
- **CODE-дельта катастрофического потолка сделки** (схема ратифицирована
  ...
  - **второй параметр `@ConfigurationProperties`** —
    ...
    `globalSimultaneousRiskPerDealPercent` (носитель заведён —
    `RiskAppetiteProperties`); незаданный отвергает **create
