...
**Позиция стала гейтом шага 8** (`DOCS_CHECK_1`/`GAPS_CLOSE_1`, 2026-09-03).
Прежде её оживление ждало снятия разведочного режима допуска сверки;
теперь ту же пару `(EXCHANGE, SOFT)` поднимает **предел слепоты**
проактивной детекции — третий подряд неполный проход
(`docs/components/AnomalyJob.md` §«Гейт полноты среза»). Без исполнителя
предел объявлен и не исполняется, то есть детектор `A10` кодом не
закрывается.
