...
фаза 1 растворена, прежние номера фаз в пунктах, записанных до этой даты,
читаются по соответствию — старая «фаза 2» (бэктест) → фаза 3, старая
«фаза 3» (портфель) → фаза 4, старые «фазы 4-5» → фазы 7-8; «конец
фазы 1» как граница запуска → прод-рубеж (`.claude/work/roadmap/prod-gate.md`).
...
5. **Пересмотреть режим автономии и выключенную механику делегирования**
   ...
   автономии «до прод-рубежа» — предложение, ждёт подтверждения
   (`.claude/work/questions/open-questions.md` ARCH-Q3).
...
секции `risk-appetite` в `src/main/resources/application-prod.yaml` (либо
