...
- **Инвентарь периодических джоб — держать сверенным.** Состояние на
  ...
  сделки»; прочие детекторы перечня — этот шаг: дом перечня —
  `docs/components/AnomalyJob.md` §«Что ищет», здесь он не
  воспроизводится. Пришло хвостом шага 7, позиция Т7: заводить детекторы
  «по одному признаку» значило бы назначить реакцию, которой корпус ещё
  не выбрал — у каждого свой такт, свой гистерезис и своя классификация);
  **только название** —
...
- ввести статус **`Exchange.HOLD`** (ступень 1, мягкий холд) + его
  ...
  **Цена отсутствия стала счётной (хвост шага 7, Т11).** Производитель
  запроса ступени 1 на биржевом радиусе уже построен: расхождение сверки
  P&L в боевом режиме допуска возвращает сигнал с кодом
  `PNL_RECONCILIATION_MISMATCH` (`MarkDealClosedExecutor`,
  `MarkDealEmergencyClosedExecutor`), а исполнитель блокировки ступень не
  поднимает — пишет предупреждение и выходит; ограничение названо в
  `docs/components/HoldService.md` §«Мягкая биржевая ступень сюда не
  приходит». До снятия разведочного режима
  допуска ступень не запрашивается вовсе, поэтому разрыв ненаблюдаем; он
  раскроется ровно тем ходом, который снимет флаг — §«Закрытие фазы 1 —
  снятие разведочного режима допуска сверки». **Срок: не позже него**;
