...
- **Однократность шага стратегии на эпизод транша.** Две колонки:
  `deal_tranches.episode_seq` (`integer not null`, 1 при заведении, +1 на
  ребре переоткрытия — пишет обработчик, гейтящий ребро) и
  `deal_action_states.tranche_episode_seq` (`integer`, обязательна у
  потраншевых строк — пишет тот, кто заводит строку). Обработчик
...
- **Цикл добычи материализованной защиты — реализация.**
  ...
  затем `GET /api/v5/trade/orders-algo-history` (`instType`, `instId`,
  `ordType=conditional`), матч по `algoClOrdId` в **ответе** — фильтра по
  клиентскому идентификатору у эндпоинтов нет. Терминал исчерпания у
  него свой: вторая ступень исхода (`PROTECTION_LOST` /
  ...
  добавляется `sz`. Вход `AttachedAlgoOrderStateResolver` становится
  двухместным.
...
- **Рантайм-хвост на той же фикстуре §AG1.5** — **негейтящий**, идёт в
  ...
  **`RQ-4` (`ccy` fee-bills = USDT, §AG3.4) из этого перечня выведен:**
  ...
  пересказывается (F7 `DOCS_CHECK_27`: прежняя редакция «после чистого
  `DOCS_CHECK` — порядок последовательный» ставила гейтящий кейс в петлю
  «факт ждёт чистого прогона, прогон без факта чистым не станет»).
