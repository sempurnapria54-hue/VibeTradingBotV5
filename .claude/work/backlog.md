...
- **Остаток холдов L3/L4** (реактивный enforcement построен на шаге 6 —
  ...
  (3) **аудит ручного un-hold** (кем/когда) — **шаг 9 / п.9**
...
**Оживит:** шаг, строящий сервис-производитель, — по порядку это шаги
4-8 фазы 2 (`auth`, `connector-okx`, `market-data`, `trading-core`,
`strategies`).
...
**Что оживит.** Первый читатель конверта: компонент
`outbox: таблица, писатели событий, OutboxRelayJob`
(`.claude/work/code-gate-ledger.json`, шаг 7 фазы 2) либо потребители
шага 10 фазы 2 — что придёт раньше.
