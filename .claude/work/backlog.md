...
**Оживит:** первый сервис, пишущий объём, — `market-data` (шаг 6 фазы 2):
он собирает ряды по всему листингу, и его потребление первым перестаёт
быть догадкой.
...
## Сериализационная аннотация на доменной модели

**Что сделать.** Снять `@JsonIgnore` с `InstrumentExternalRules`
(`libs/domain-model`) и выразить «поле не попадает в JSONB-навес»
средствами персистентности, а не аннотацией на форме. Вместе с этим —
убрать зависимость `jackson-annotations` из `libs/domain-model/pom.xml`,
если других её потребителей не появится.

**Оживит:** шаг 6 фазы 2 (`market-data`) — владелец
`InstrumentExternalRules` по оси владельца-сервиса.

**Причина парковки.** Аннотация обнаружена сборкой библиотеки на шаге 3:
она заставила общий артефакт объявить зависимость от Jackson, то есть
форма знает о своём хранении. Снять её на шаге 3 нельзя — потребовалось
бы завести персистентный слой сервиса, которого ещё нет; сам jar
`jackson-annotations` маркерный, без собственных зависимостей, и цена
парковки — одна строка в pom.

**Владелец** — `solution-designer` (граница формы и хранения) с
`code-writer` (pom и аннотация).
