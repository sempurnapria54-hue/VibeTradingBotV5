...
**Измерено курационным свипом 2026-09-02.** В `src/**` **61 указатель**
...
**Объявленный долг границы слоя** (`codestyle` §Слои: константы источника
границу не переходят). `Order.side` объявлено `String` и несёт литерал
источника (`"buy"` / `"sell"`); отсюда `CreateOrderActionExecutor.toSide`
— единственное место, где доменный слой читает `Constants.Okx`.

**Почему не правится клаузой конвенции.** Правка — смена типа поля на
доменный перечень (`Order.Side`), миграция колонки `orders.side`,
конвертация на границе мапперами и обход читателей. Это ход уровня
модели, а не письма.

*Оживит:* ближайший ход, трогающий модель заявки, — либо шаг, вводящий
второй источник, где чужой словарь в домене перестанет быть безобидным.
*Причина:* сегодня источник один, и литерал в домене даёт ровно одну
точку расхождения; цена правки — миграция и обход читателей — выше цены
названного долга.
...
тот же предмет и другая область — `src/**`.
...
носители. Его кодовая дельта — два файла оснастки, и адверсариальные
фокусы `performance` / `disaster` / `security`, которых требует гейт
`DONE`, предмета на ней не имеют.

**Что сделано пока.** Шагу проставлен `CODE` — статус, ближайший по
фактике (код был написан); чего не хватает до `DONE`, названо поимённо в
`.claude/work/progress/phase-2-step-1-chronicle.md` §«Что осталось до
`DONE`». Обойти гейт молчанием — тот же дефект, что и обойти его
неприменимым прогоном.

**Что решить.** Либо статус-набор получает форму для шага без продуктовой
дельты, либо гейт `DONE` получает клаузу «фокус без предмета закрывается
названным вердиктом, а не прогоном». Обе стороны — правка механизма, то
есть решение с альтернативами, а не правка на месте.
