...
- **Ступень реакции резолвится покрытием, радиус — типом исключения.**
  ...
  — по признаку покрытия транша из контекста прохода
  (`docs/spec/protection-coverage.json`, `trancheCovered`). Безусловной
  полной формы у триггера исчерпания больше нет; при нарушенном покрытии
  реакцию поднимает не он, а инвариант покрытия. Дом —
  `docs/rules/instrument-hold.md`, контракт сигнала —
...
- **Цикл добычи материализованной защиты — реализация.**
  ...
  цикл (не пятую ногу первого): `GET /api/v5/trade/orders-algo-pending`,
  затем **два вызова** `GET /api/v5/trade/orders-algo-history`
  (`instType`, `instId`, `ordType=conditional`, `state=effective`; затем
  `state=canceled`) — `state` либо `algoId` у эндпоинта обязателен, а
  `algoId` материализованной записи неизвестен по построению. Матч по
  `algoClOrdId` в **ответе** — фильтра по клиентскому идентификатору у
  эндпоинтов нет; глубина — пагинацией `after`/`limit`, окна у эндпоинта
  нет. Гейт запуска — **терминальный родитель** (`TERMINAL_FILLED` /
  `TERMINAL_FILL_UNKNOWN`), не всякий `SEARCH_MORE`. Терминал исчерпания
  у цикла свой: вторая ступень исхода (`PROTECTION_LOST` /
  `ANALYSE_HISTORY`), а не `MISSING_AFTER_REFRESH`. Найденный
  `AlgoOrderOkxResponse` маппится в `AttachedAlgoOrderExternalSnapshot`
  (поля — `docs/models/mapping/Order.md`), в инвентарь источника
  добавляется `sz`; в снапшот добавляется поле `externalStatus` (сырой
  `state` записи — операнд исходов `TRIGGERED` / `ANALYSE_HISTORY`).
  Вход `AttachedAlgoOrderStateResolver` становится двухместным.
...
## Окно линковки движений у восстановленной сделки

**Заведено `GAPS_CLOSE_28`** (хенд-офф узла тропы восстановления).
Нижняя граница окна линковки у обычной сделки — время создания первой
отправленной входной заявки. У восстановленной такой заявки нет, и
суррогатом служит биржевой момент создания сделки — а он **позже**
фактического открытия позиции. Следствие: движения, случившиеся до
заведения сделки (финансирование, комиссии), в окно не попадают, и
разбивка выходит неполной **молча**. Наблюдаемо это только косвенно —
расхождением сверки и признаком неполноты разбивки.

**Кандидат-операнд назван, чтобы его не выводили заново:** биржевое
время открытия наблюдённой позиции, доступное детектору в том же
снимке, которым он позицию и обнаружил.

**Сюда же — обязательность `deals.external_created_at`.** Обе тропы
создания пишут его всегда, и на восстановительной он единственный операнд
границы; ограничением он не закреплён сознательно: на его пустоте стои́т
живой предикат резолвимости границы `boundResolved`
(`docs/spec/cash-flow-linkage.json`) и пример под него, и `NOT NULL`
сделал бы обе ложные ветви недостижимыми. Решать вместе с вопросом о самой
границе, одним ходом.

**Развилка отдельная:** который именно биржевой момент считать нижней
границей — предмет владельца окна линковки, а не тропы восстановления.
Дом величины — `docs/spec/cash-flow-linkage.json` (`lowerBound` /
`boundResolved`, заведённые ровно ради этой популяции); подстановка
суррогата — `docs/components/RefreshBillsExecutor.md`.
Владелец — `solution-designer`; горизонт — шаг, трогающий окно линковки.
...
`DOCS_CHECK_30`). `docs/concept.md` §«Опровержение правит на месте»
запрещает в корпусе оборот «прежняя редакция …»: знание, оказавшееся
неверным, переписывается, а не сопровождается пометкой. Измерено
2026-08-30: **15 вхождений в 13 файлах** `docs/**` (команда —
`LC_ALL=C.UTF-8 grep -rlP 'Прежняя редакция|прежней редакции|прежнюю
редакцию' docs --include='*.md'`). `GAPS_CLOSE_30` свёл вхождения,
попавшие под реестр снятых редакций (база риска — четыре носителя);
остальные разбираются по одному: у части довод переезжает в
`decisions/`, у части снимается вовсе. Механически этот класс не ловится
— энфорсер снятых редакций мерит **термины конкретной редакции**, а не
оборот.
...
- **Переносимость инструментов — СНЯТА.** Разделитель classpath, путь
  JDK и лаунчер были зашиты в форме Windows, и на Linux команды не
  запускались вовсе; оснастка вынесена в `tools/spec-runner-env.sh` и
  сделана переносимой.
- **`tools/anchor-check.py` на Windows не прогоняем — ОСТАЁТСЯ**
  (вскрыто 2026-08-31 прогоном с Windows-хоста). Механизм: индекс
  файлов строится путями с `\` (`glob`/`relpath`), а адреса в тексте и
  строки `tools/anchor-debt.txt` несут `/` — сравнение не сходится, и
  здоровый корпус даёт тысячи ложных «неразрешимых» плюс все строки
  долга как «не в корпусе» (батарея проб при этом зелёная — фикстуры
  дефект не покрывают). Вторично: печать находок падает
  `UnicodeEncodeError` на cp1251-консоли — обходится
  `PYTHONIOENCODING=utf-8`. Починка — нормализация разделителя до `/`
  в индексе и сравнении + проба на эту ось в батарее.
