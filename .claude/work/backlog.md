...
якорей.** Событие возврата здесь — не конец фазы 1, а **появление второго
субъекта, обращающегося к поверхности от своего имени**: второго человека
либо второго автомата. Оно может не наступить ни в фазе 1, ни позже, а
может наступить в середине фазы 2. Механического триггера у него нет и быть
не может — первым его видит держатель.
...
**Решение держателя 2026-09-03: боевые значения назначаются в конце фазы 1,
не сейчас.** До тех пор `application-prod.yaml` заполнен **значениями
...
**Что сделать до перевода фазы 1 в `DONE`** — держатель назначает три числа
...
первым периодом прогонов контура (шаг 10 фазы 1), — и правило, его
...
- **Мультибиржевая корректность прохода.** Срез читается один на тик и
  ...
  *Оживит:* заведение второй строки биржи (горизонт — фаза 3, там же
