...
- **`mvnw`/wrapper в репо — ОСТАЁТСЯ.** Прогон опирается на дистрибутив
  Maven из кэша wrapper'а пользователя и на вручную собранный `PATH`; на
  чистой машине не воспроизводится. Действующие пути, команда сборки и
  лаунчер Python — дом фактов среды
...
- **Проброс Vault-токена — СНЯТ.** `.env.vault.test.local` читается
  `source`'ом из shell, `tools/boot-test.sh` его использует; прогон
  `mvn -o test -Dgroups=source-api-live` поднимается автономно.
- **Область токена нарушена — ОСТАЁТСЯ.** `.env.vault.test.local.example`
  требует токена, ограниченного тестовыми путями; фактический токен
  читает и `secret/tradingbot/okx`, и `secret/tradingbot/postgres`
  (проверено). Расхождение среды с собственным требованием; адресат —
  шаг 9 «Безопасность».
