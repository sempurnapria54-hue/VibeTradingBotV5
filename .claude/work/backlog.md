...
## Носители прогона гейтящих слотов шага 7 — хвост из трёх слотов

**Исполнено 2026-08-30.** Носители написаны, среда поднята, контур
прогнан автономно; **закрыто десять слотов из тринадцати**. Состояние,
добытые факты и воспроизводимые команды —
`.claude/tests/source-api/okx/code-preconditions.md`; хроника — chronicle
шага.

**Что осталось, по слотам:**
...
2. **П. 10 — след автоделевериджа (`AG1.8`).** Эпизод инициирует биржа,
   на demo не заказывается. Исходом «не наступило» гейт **не
   закрывается** (решение держателя 2026-08-30), поэтому слот открыт
   бессрочно. **Развилка держателя:** снять гейтящий статус у пункта либо
   принять ожидание.
3. **П. 17 — судьба встроенной защиты при непустом наливе (`M17.5`).**
   Частичный налив под потолком риска 200 USDT на `ETH-USDT-SWAP`
   недостижим: объём лучшего уровня книги demo кратно превышает половину
   потолка (наблюдено 55.6, 14.82, 3043.85 контракта при потолке 0.81).
   **Развилка держателя:** поднять потолок риска цепочки; взять
   инструмент с тонкой книгой (правка плана — кейс перестаёт быть про
   `ETH-USDT-SWAP`); либо ловить налив пассивной заявкой, что делает
   цепочку недетерминированной.
