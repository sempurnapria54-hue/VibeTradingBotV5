...
- **ингресс на стенде теперь есть** — он приехал с периметром, и таблица
  доступа ниже (прокидывание порта) остаётся верной только для
  инфраструктурных интерфейсов.
...
Инфраструктурные интерфейсы за ингрессом не стоя́т (он маршрутизирует
`/api/v1` на периметр), поэтому доступ к ним — прокидыванием порта. Каждая команда занимает терминал; закрывать
по Ctrl+C.
