...
| `dev` | окружение: Postgres+TimescaleDB, Kafka, Elasticsearch с Kibana и сборщиком логов, Jaeger плюс **всё, что лежит в `deploy/base/services/`** — перечня здесь нет, его печатает `ls deploy/base/services/` |

**Что стенд поднимает, а что на нём собирается — разные множества, и
расходятся они законно.** `up.sh` применяет оверлей целиком, то есть заводит
объекты **всех** сервисов окружения; образы кладёт `deploy-services.sh`, и
его список по умолчанию у́же. Сервис, чей манифест применён, а образ не
собран, останется без запущенного пода — это видимое состояние, а не тихий
пропуск (чем именно оно видимо — `.claude/work/backlog.md` §«Образ сервиса без
продвинутого тега резолвится подвижным `latest`»). Оба списка выводятся из
своих домов — `ls deploy/base/services/` и объявление `SERVICES=(…)` в самой
команде, — и третьей их копии здесь не заводится.

**Что требуется довести при ближайшей постановке** (счёта единиц здесь нет —
он менялся бы с каждым построенным сервисом):

- секрет роли базы `postgres-role-strategies` — как у прочих владельцев
  баз;
- секрет `bff-ticket-secret` с ключом `secret` — подпись билета подписки
  у периметра; **общий у реплик**, поэтому генерируется один раз на
  окружение, а не подом:
  `kubectl -n dev create secret generic bff-ticket-secret
  --from-literal=secret="$(openssl rand -base64 32)"`.

## Тестовые данные стенда

Строки тенанта `tn-stand-0001` и биржевого счёта `ea-stand-0001` — **данные
стенда, а не остаток недоделанной работы**: они заведены, чтобы проверить
синк реестра счетов, и **остаются** (решение держателя 2026-09-06). Чистить
их можно при необходимости — своей истины они не несут, и снятие их ничего
не ломает. Вопрос «удалять или нет» закрыт и в снапшотах больше не носится.

## Доступ к интерфейсам

Входа снаружи на стенде нет (§«Что стенд НЕ даёт»), поэтому доступ к
интерфейсам — прокидыванием порта. Каждая команда занимает терминал; закрывать
по Ctrl+C.
...
bash tools/stand/deploy-services.sh                  # список по умолчанию
```

Список по умолчанию — в самой команде (`SERVICES=(…)` рядом с разбором
аргументов); второй его копии здесь не заводится: она разошлась бы с
командой первой же построенной единицей и разошлась бы **молча**.
...
(`D:\Docker containers\DockerDesktopWSL\disk\docker_data.vhdx` — Disk image
location Docker Desktop; держатель перенёс туда хранилище 2026-09-17, прежний
адрес `%LOCALAPPDATA%\Docker\wsl` пуст) смонтирован **без
...
`SESSION_MIN_FREE_GIB` (умолчание — 10 ГиБ, `tools/session-loop.sh`). Мерит он
диск, на котором лежит образ, — тот же адрес, что и команды ниже; **путь
подтверждается файлом `*.vhdx`, а не существованием каталога**: перенос
оставил по старому адресу пустой каталог, и проверка «каталог есть» заставила
бы мерить не тот диск молча. Хранилище переехало снова — адрес задаётся
`SESSION_DOCKER_DATA_DIR`, а не правкой скрипта.
...
```powershell
$v = "D:\Docker containers\DockerDesktopWSL\disk\docker_data.vhdx"
@"
select vdisk file="$v"
...
**В пути пробел, и держат его кавычки в двух местах:** `$v` в двойных
кавычках PowerShell и значение `file="$v"` внутри скрипта diskpart. Без
вторых `select vdisk` разберёт только первое слово пути и ответит, что файла
нет. Проверено прогоном: сгенерированный `compact.txt` несёт путь целиком;
сам `diskpart` без прав администратора не запускается (UAC), поэтому его
разбор здесь не прогонялся.

Замер 2026-09-09 (образ ещё под `%LOCALAPPDATA%`): 33.69 → 24.83 ГиБ, свободного на диске 9 → 18 ГиБ; все
...
- **ингресса нет** — объект `Ingress` приехал с периметром, а контроллера,
  который его исполнит, нет ни в манифестах, ни на стенде
  (`.claude/work/backlog.md` §«Ингресс окружения исполнять некому:
  контроллера нет ни в манифестах, ни на стенде»); наружу стенд не
  опубликован вовсе;
