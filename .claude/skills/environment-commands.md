...
| Сборка дерева | реактор из корня, обёрткой `bash tools/reactor-test.sh`; прогон модулей с тестовой дельтой — `bash tools/reactor-test.sh --modules <модуль>[,<модуль>…]` (зависимости собираются, тесты — только у названных; замер 2026-09-25: `strategy-engine` — 12 с, `tests` — 35 мин, 91 тест: время сквозного набора — его собственные тропы; зовётся с таймаутом `7200000`); тронутые классы сессии тестера — `bash tools/reactor-test.sh --modules <модуль>[,…] --classes <Класс>[,…]` (та же сборка, тесты — только у названных классов); точечно — `mvn -o -am -pl <модуль> test`, и `-am` исполняет тесты всех зависимостей. Лог обёртки — постоянный `target/reactor-test.log`: опустошается первым ходом прогона и остаётся после него. **Длительность реактора — 46 мин** с тремя сквозными тропами (замер 2026-09-25: 46 мин, 1746 файлов, 5113 тестов, `REACTOR_MVN_ARGS="-fae verify"`); в умолчание таймаута вызова он **не укладывается** и зовётся с таймаутом `7200000` | прогон обёрткой; замер — `date +%s` до и после |
| Предел вызова инструмента | **умолчание 30 мин, предел 120 мин** — `BASH_DEFAULT_TIMEOUT_MS=1800000`, `BASH_MAX_TIMEOUT_MS=7200000` в `.claude/settings.json`; действуют в каждой сессии проекта, включая цепочку. Прогон идёт на переднем плане одним вызовом (`.claude/rules/long-run-wait.md`); реактор — с явным таймаутом, умолчания ему мало (строка «Сборка дерева»); прежний предел CLI — 10 мин — реактор не вмещал, и сессии уходили в фон с опросом | проба 2026-09-22: сессия `claude -p` печатает обе переменные из `settings.json` |
...
bash tools/reactor-test.sh --modules services/bff,tests   # те же оси, но тесты — только у названных модулей;
                                             # закрывающий прогон тестовой дельты
...
bash tools/reactor-test.sh --modules tests --classes ExitOrderPathTest,ExitJournalPathTest
                                             # та же сборка, тесты — только у названных классов;
                                             # закрывающий прогон сессии тестера — тронутые классы
