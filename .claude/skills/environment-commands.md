...
| Сборка дерева | реактор из корня, обёрткой `bash tools/reactor-test.sh`; точечно — `mvn -o -am -pl <модуль> test`. **Длительность реактора — 46 мин** с тремя сквозными тропами (замер 2026-09-25: 46 мин, 1746 файлов, 5113 тестов, `REACTOR_MVN_ARGS="-fae verify"`); в умолчание таймаута вызова он **не укладывается** и зовётся с таймаутом `7200000` | прогон обёрткой; замер — `date +%s` до и после |
