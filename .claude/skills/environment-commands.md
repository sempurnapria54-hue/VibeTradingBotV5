...
| Сборка дерева | реактор из корня, обёрткой `bash tools/reactor-test.sh`; прогон модулей с тестовой дельтой — `bash tools/reactor-test.sh --modules <модуль>[,<модуль>…]` (зависимости собираются, тесты — только у названных; замер 2026-09-25: `strategy-engine` — 12 с, `tests` — 35 мин, 91 тест: время сквозного набора — его собственные тропы; зовётся с таймаутом `7200000`); точечно — `mvn -o -am -pl <модуль> test`, и `-am` исполняет тесты всех зависимостей. **Длительность реактора — 46 мин** с тремя сквозными тропами (замер 2026-09-25: 46 мин, 1746 файлов, 5113 тестов, `REACTOR_MVN_ARGS="-fae verify"`); в умолчание таймаута вызова он **не укладывается** и зовётся с таймаутом `7200000` | прогон обёрткой; замер — `date +%s` до и после |
