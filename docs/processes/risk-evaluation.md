...
`DOCS_CHECK_23`). Реджект защитного действия, уменьшающего покрытие, —
не признак рассогласования, а **сработавший контроль**: старая защита
остаётся живой (`REPLACE` идёт place-new → cancel-old), покрытие
сохраняется, и уводить сделку в `ERROR` значило бы рвать живой риск там,
где он под контролем. Дом предиката —
