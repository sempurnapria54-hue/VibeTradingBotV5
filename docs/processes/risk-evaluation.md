...
**`BLOCKED` по `RISK_PER_DEAL_CUMULATIVE_EXCEEDED` и
`RISK_PER_DEAL_SIMULTANEOUS_EXCEEDED` в `ERROR` не уводит** (C6
`DOCS_CHECK_20` + `RISK-Q3-A`). Исчерпанный бюджет сделки — ожидаемый
