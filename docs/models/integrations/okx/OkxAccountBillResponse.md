...
| `instId` | инструмент движения → `externalInstrumentId` (сырой, nullable). Не только фильтр матчинга: операнд предиката **отложенной** линковки по persisted-строкам (H11 `DOCS_CHECK_11`) |
...
- **Инструмент / режим:** `instType`, `mgnMode`.
  **`instId` в used переведён** (H11 `DOCS_CHECK_11`): он не только
  фильтрует матчинг, но и **пишется** в `DealCashFlow.externalInstrumentId`
  — операнд предиката **отложенной** линковки, где множество «уже
  сохранённых строк окна» адресуемо только колонками строки
  (`docs/models/mapping/DealCashFlow.md`). Прежняя запись «участвует в
  матчинге как фильтр, в поле не пишется» была верна, пока линковка шла
  только по свежепрочитанному ответу.
