...
`StrategyAlgoOrderAction`) переоткрывает состав таблиц операндов
планового риска — сегодня они живут только в `orders`
...
### Операнды планового риска — дом не здесь (`RISK-Q4` закрыт)

Операнды планового риска (`plannedEntryPrice`, `plannedSizeContracts`) —
атрибуты **ноги входа**, а входной тропы алго-ордером не существует
(§Назначение): дом — **только `Order`**
(`docs/models/domain/core/Order.md` §«Операнды планового риска»);
`algo_orders` ценовых колонок под них не получает — это была бы мёртвая
пара колонок с живым именем, читающаяся как «тропа есть». Прежняя
