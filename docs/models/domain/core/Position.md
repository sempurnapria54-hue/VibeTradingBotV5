...
- **`uk_position_deal_live`** — частичный уникальный индекс
  ...
  (§Live risk);
- **`uk_position_deal_external`** — `unique (deal_id, external_id)`:
  идемпотентность материализации эпизода по `posId`. Строка без
  наблюдённого `posId` (`external_id is null`) ключом не охраняется —
  `NULL`'ы в `UNIQUE` различны; такая строка бывает не более чем одна и
  охраняется первым ключом, пока жива
  (`docs/rules/idempotency-via-unique.md` §«Уникальность среди живых»);
