...
- `HOLD` — **заморозка** (ступень 1): блок `SUBMIT_*` по всем
  инструментам биржи, safety/read разрешены, живые сделки не трогаются;
  без kill-switch. Вход — из `ACTIVE`; снятие — **вручную** в `ACTIVE`.
  В коде статуса ещё нет — вводится на `CODE`
  (`docs/rules/exchange-hold.md` §«Состояние носителей»).
- `TRADE_BLOCKED` — **полная реакция** (ступень 2): то же + kill-switch
  flatten всей биржи + каскад активных сделок в `ERROR`. Вход — из
...
> Доки, звавшие статус `Exchange.HOLD` (заморозка), и код/аппарат
