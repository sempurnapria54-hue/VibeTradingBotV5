...
Оба safety-статуса: вход — только из `ACTIVE`, снятие — **вручную** в
`ACTIVE`; `ENTRY_BLOCKED` эскалируется в `TRADE_BLOCKED` при
kill-switch-триггере, обратной эскалации нет
