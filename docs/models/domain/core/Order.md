...
| `bookDepthAtPlacement` | `BigDecimal` | **Наблюдаемая ёмкость стакана на момент постановки ноги** (объём своей стороны топа) — измеритель, не операнд тождества риска (§«`bookDepthAtPlacement` — наблюдаемая ёмкость стакана на момент постановки»). Write-once; **пуст, когда свежей `MarketPriceData` в контексте не было**. В инвариант «шесть или ни одной» **не входит** (P10 `DOCS_CHECK_24`). |
...
attached-защита — предикат **наличия**; операндом инварианта защиты он
**больше не служит**, там считается покрытие,
`docs/rules/risk-creating-entry-protection.md` §Правило. Останется ли
метод после `CODE` — решается там: его последний потребитель, гейт
`EntryFinalizedHandler`, переводится на покрытие), `toCancel(reason)`,
`toComplete()` (→ COMPLETED/FILLED), `toError(reason)`.
...
### `bookDepthAtPlacement` — наблюдаемая ёмкость стакана на момент постановки

P10 `DOCS_CHECK_24`, решение держателя `GAPS_CLOSE_24` (крен (4)). Глубина
**своей** стороны топа стакана на момент постановки ноги:

```text
bookDepthAtPlacement_i = BUY  ⇒ MarketPriceData.externalAskSize
                         SELL ⇒ MarketPriceData.externalBidSize
```

фиксируется **на момент постановки**, write-once, тем же ходом, что
`liquidationDistanceRatio` (`orders.book_depth_at_placement`,
`numeric(36,18)`, nullable). Пишет `CreateOrderExecutor` из
`CalculationContext`.

- **Зачем.** Ёмкость инструмента системой не контролируется, и корпусный
  guard позиционных лимитов отклонён именно потому, что его операнд **не
  измеряется** (`docs/decisions/per-trade-risk-policy.md` §«Трёхуровневая
  модель риска; в фазе 1 — только риск на сделку»). Без этого числа
  отношение «мой размер / доступная ликвидность» не наблюдаемо ни одним
  полем, и решение о потолке остаётся неинформированным навсегда.
- **Это измеритель, а не контроль** — ровно как соседнее число:
  постановку не блокирует, в реджекты не входит, в инвариант «шесть или
  ни одной» **не входит**.
- **Операнд пуст ⇒ поле пусто.** Свежей `MarketPriceData` в
  `CalculationContext` может не быть на аварийных тропах; благоприятное
  умолчание не подставляется (`docs/rules/absent-value-semantics.md`).
- **Условие снятия неопределённости** — распределение значения после
  первого периода живой торговли; якорь —
  `.claude/work/backlog.md` §«Пересмотр worst-case открывающего входа — по
  данным первого периода живой торговли».
...
**Писатель — `RefreshPositionExecutor`** (A9 `DOCS_CHECK_24`), в момент
материализации или наблюдения живого эпизода: он проставляет `position_id`
всем ногам сделки с непустым `accumulated_fill_size` и пустым
`position_id`. Значение — `id` живого эпизода; **write-once** (guard
`where position_id is null`): нога принадлежит тому эпизоду, который
открыла или в котором исполнилась, и переприсвоение исказило бы обе
стороны.
...
- **Почему не `RefreshOrderExecutor`** (прежняя редакция; A9
  `DOCS_CHECK_24`). Она называла его писателем «при наблюдении **первого**
  филла ноги» и обосновывала это тем, что «в том же проходе `DealContext`
  несёт живую строку `Position`». На **открывающем входе** это неверно:
  локальной строки `Position` в этот момент ещё нет — её материализует
  следующее звено, `REFRESH_POSITION_COMMAND`
  (`docs/lifecycles/Position.md` §«Легитимное окно появления позиции»), а
  за проход выдаётся одна команда. Write-once + единственный момент записи
  давали бы **вечно пустую** колонку у открывающей ноги, то есть тот же
  дефект, которым отвергнут `CreateOrderExecutor`, — и тихо: три формулы
  шага (`currentRiskAmount`, предикат пары
  `protectionRelievedRiskAmount`, деградационная форма
  `livePositionRiskAtStop`) читают пустоту как ратифицированную ветку.
  Момент у `RefreshPositionExecutor` достижим на **обеих** тропах: к
  моменту, когда он видит эпизод, строка существует по построению.
- **Названная цена — атрибуция по первому наблюдению.** Нога, филлующаяся
  в двух эпизодах, отходит тому, который наблюдался первым; это то же
  приближение, что уже принято (§«Почему приближение всё же остаётся»
  в `docs/models/domain/aggregate/Deal.md`), новой потери точности нет.
...
- **`book_depth_at_placement`** (`numeric(36,18)`, nullable, write-once) —
  измеритель ёмкости стакана на момент постановки (P10 `DOCS_CHECK_24`,
  решение держателя). В инвариант «шесть или ни одной» не входит, как и
  соседний измеритель.
- **`liquidation_distance_ratio` — седьмое ЧИСЛО риска** (не седьмая
  колонка `ALTER`: тот вводит **девять** колонок — шесть тождества,
  `position_id`, это и `book_depth_at_placement`; база счёта названа
  B7 `DOCS_CHECK_23`, прежняя подпись «седьмая колонка» читалась как
  «колонок семь»)
...
| `size` | `BigDecimal` | Размер защиты **в контрактах** (для SWAP/FUTURES — та же единица, что у `Order.size`/`accumulatedFillSize`; единица источника подтверждена рантайм-прогоном, офдок её не называет — N2 `DOCS_CHECK_24`). **Пишет постановщик** — `CreateOrderExecutor` из `AttachedProtectionPayload.size`; при рефреше приезжает эхо биржи (`attachAlgoOrds[*].sz`). Операнд предиката покрытия — `min(size, Order.accumulatedFillSize)` родительской ноги (дом — `docs/rules/risk-creating-entry-protection.md` §Правило): поле **может** быть меньше ноги (защита на часть), поэтому одного `accumulatedFillSize` недостаточно, а одного `size` — недостаточно при частичном филле. |
