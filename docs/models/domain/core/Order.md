...
| **Факт (производная)** | нога с **непустым `plannedRiskAmount`** — следствие первых двух: у reduce-only преконтроль не гоняется, пять чисел не производятся | `docs/components/FinalizeDealExitExecutor.md` §epsilon |

**Три носителя не расходятся — их связывает инвариант**
`Type = REDUCE_ONLY ⇔ positionReducingOnly = true` (§Енумы), а непустота
`plannedRiskAmount` следует из второго: преконтроль, который пишет пять
...
**Чем пользоваться где — по одной форме на задачу, не конъюнкцией:**

- **отбор строк `orders`** (сумма `R`, epsilon, отчёт) — **непустота
  `plannedRiskAmount`**, и только она. Добавлять к ней проверку типа
  нельзя: исход она не меняет никогда, а при рассогласовании пары
  `Type` ↔ `positionReducingOnly` **маскирует** нарушение инварианта,
  тихо выкидывая ногу из выборки
...
- **бизнес-тип** — для **чтения человеком и валидации**: он называет роль
  ноги в строке `orders`, но предикатом отбора не служит.
