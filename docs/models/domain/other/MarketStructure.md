...
Что это за модель `MarketStructure`.
...
**Уровень типа для стопа и размещения — ПОСЛЕДНИЙ подтверждённый.** Свингов
одного типа в окне несколько; стоп по структуре и размещение `SWING_*`
строятся от последнего по моменту подтверждения — от структуры, которую
сломает цена, а не от самого старого пивота окна [Kaufman гл. 23; Tharp
гл. 9]. Известный момент подтверждения бьёт неизвестный; при равных моментах
либо двух неизвестных побеждает стоящий в перечне позже. Граничных уровней (`RANGE_*`, `SUPPORT`, `RESISTANCE`) у типа один, и
выбор на них не влияет.
...
**Условие `RANGE_BREAKOUT_CONFIRMED` читает направление, а не только
наличие события.** Направление объявляет правило вторым операндом —
константой перечня `Direction`, в той же форме, что у равенства типа
структуры (`MARKET_STRUCTURE_IS`): `EQ` — пробой в объявленную сторону, `NE` —
в противоположную. Без направления вход в лонг «на подтверждённом пробое»
открывался бы и на пробое вниз [Kaufman гл. 5]. Неизвестное либо пустое
направление и пустой оператор дают ложь, создание их отвергает
