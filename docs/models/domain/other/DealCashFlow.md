...
**Операнды окна названы** (H14, `GAPS_CLOSE_6`; операнд `end`
переопределён — H1, `GAPS_CLOSE_7`). Обе границы берутся из дат, **уже
лежащих на `Deal`/`Position`** к моменту, когда линковка исполняется:

- `begin` = `Position.externalCreatedAt` сделки (биржевое `cTime` открытия
  позиции); позиции нет (сделка до неё не доехала) — `externalCreatedAt`
  первого отправленного `Order`.
- `end` = `Position.externalModifiedAt` — `uTime` записи закрытия, которую
  вторая нога `REFRESH_POSITION` **уже приземлила на `Position`**
  (`docs/models/domain/core/Position.md` §«Положение закрытия»); позиции
  нет либо запись не добыта — `Deal.modifiedAt` (момент последнего движения
  сделки).

Границы **включительные**; искусственного зазора не вводим.

**Прежняя редакция была неисполнима.** `end` определялся как
`externalUpdatedAt` **транзитного снапшота**, живущего в памяти чужого
действия, — а `REFRESH_BILLS` идёт **отдельным проходом** и доступа к этой
памяти не имеет. Клауза была мертва на обеих тропах, окно оставалось не
ограниченным сверху, и при линковке по `instId` движения **следующей**
сделки по тому же инструменту получали бы `deal_id` предыдущей
(необратимо — дедуп `UNIQUE(external_bill_id)`).
...
`REFRESH_BILLS` линкует движения **только пока `Deal` не терминализован**
...
- **Дефолт при отрицательном ответе зафиксирован заранее:** `begin` =
  `externalCreatedAt` **первого отправленного `Order`** — **всегда**, а не
  только при отсутствии позиции. Окно шире, но лишних движений не
  захватывает: активная сделка на инструмент одна (тот же инвариант слота,
  что держит верхнюю границу).
