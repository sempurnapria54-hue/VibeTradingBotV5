...
- `APPLIED` — курс получен из свечи на момент операции и зафиксирован.
  ...
  cross-ccy-слагаемое. Несёт также **разрешение свечи**, по которой курс
  взят (секундное или огрублённое — `docs/components/RefreshBillsExecutor.md`
  §«Носитель курса»), чтобы деградация не читалась как штатный случай.
