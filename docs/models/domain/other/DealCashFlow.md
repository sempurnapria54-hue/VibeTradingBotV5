...
| `category` | `CashFlowCategory` | да | Категория движения (доменный enum; резолвится из `type`/`subType` bill-записи). |
...
| `externalFee` | `BigDecimal` | нет | Знаковая **комиссионная компонента** записи (минус — комиссия, плюс — ребейт; сырой `fee` bill-записи). Отвечает на вопрос «**сколько комиссии**»: комиссия сделки = Σ`externalFee`. `null` для движений без комиссионной компоненты (например funding). Взята явно, а не выведена из знака `amount` — иначе комбинированная запись (`amount` = pnl + комиссия) исказит разбивку (`docs/models/mapping/DealCashFlow.md` §«Разделение ролей `balChg` и `fee`»). |
...
- `TRADE_FEE` — торговая комиссия (списание за исполнение ордера; знак
  `externalFee` минус).
- `FUNDING` — funding-платёж по SWAP (списание/начисление; `subType` `173`
  expense / `174` income).
- `REBATE` — ребейт (возврат/скидка комиссии; знак `externalFee` плюс).
- `LIQ_PENALTY` — ликвидационный штраф (издержка принудительного закрытия
  позиции биржей).
- `REALIZED_PNL` — движение реализованного pnl (изменение баланса по факту
  закрытия/частичного закрытия позиции).
