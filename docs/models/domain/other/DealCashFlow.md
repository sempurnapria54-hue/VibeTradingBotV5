...
| `amount` | `BigDecimal` | да | Знаковая сумма движения (минус — списание, плюс — начисление). |
| `ccy` | `String` | да | Валюта движения (`USDT`). **Обязательно** — без валюты cross-ccy движение (например комиссия в OKB) теряется молча при сверке (`pnl-finalization-mechanics.md` реш.5). |
...
- `FUNDING` — funding-платёж по SWAP (списание/начисление; `subType` `173`
  expense / `174` income).
- `REBATE` — ребейт (возврат/скидка комиссии; знак `fee` плюс).
