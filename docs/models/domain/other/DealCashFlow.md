...
- Колонка `external_instrument_id` — nullable (сырой `instId`); входит в
  предикат отложенной линковки и в индекс отбора несвязанных строк.
...
- `end` **запроса** = **текущее время** прохода (подвижная граница, Р7
  `GAPS_CLOSE_16`, решение держателя): добыча не ждёт записи закрытия.
- `end` **линковки и контроля полноты** = `Deal.billsWindowEnd` — `uTime`
  записи закрытия, пишет вторая нога `REFRESH_POSITION_COMMAND`
  транзакционно с полями положения закрытия. Разбор двух величин —
