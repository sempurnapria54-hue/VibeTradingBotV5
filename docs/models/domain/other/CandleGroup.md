...
**`ONE_SECOND` введено шагом 7** (H10 `DOCS_CHECK_15`, решение
пользователя) — под ссылку на свечу курса cross-ccy-пересчёта
(`docs/models/domain/other/DealCashFlow.md` §Структура,
`appliedRateCandleTimeframe`). Прежде минимальным значением было
`ONE_MINUTE`, и поле, заведённое ровно под различение «взят по секундной /
взят по минутной за неимением секундной», секундную свечу выразить не
могло.
...
- **в группах свечей — не допускается.** `CandleGroup` секундного
  ...
  (§«Целостность по count», `docs/rules/market-data-retention.md`).
  Гейт — валидация при заведении группы;
- **в дереве стратегии — не допускается.** Настройки индикаторов и
  структуры (`docs/models/domain/aggregate/Strategy.md` §TimeFrame) читают
...
- **единственный легальный потребитель** — координата 2 ссылки на свечу
  ...
  персистится (`docs/components/RefreshBillsExecutor.md` §«Носитель
  курса»).

**Доступность секундного разрешения у источника — открытое предусловие**
(`integrator`, предусловие `CODE` п. 5,
`docs/decisions/pnl-finalization-mechanics.md`). Решением оно **не
закрыто**: енум обязан уметь выразить разрешение независимо от того, какое
из них окажется доступным, — иначе лестница деградации не выразима вовсе.
...
> раздел в `Strategy.md` сведён к ссылке сюда (TIME-Q1 закрыт на
> `GAPS_CLOSE_1` шага 2).
