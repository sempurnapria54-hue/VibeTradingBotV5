...
**(б) `target` STRATEGY-строк — переносится (бэкфилл).** У существующих
строк jsonb-колонка `target` несёт `RuntimeTarget`; она расплющивается в
колонки:

- `target` → `target_entity_type` / `target_entity_id` (бэкфилл
  `UPDATE` из jsonb);
- затем `DROP COLUMN target` — двух представлений одного факта не
  оставляем;
- у перенесённых STRATEGY-строк `action_kind = STRATEGY`,
  `system_action_type = null`.

**Почему асимметрия (а) и (б).** STRATEGY-строки **осмысленны**: они
связывают узел стратегии с порождённой runtime-сущностью, и эта связь
нигде больше не хранится (`docs/rules/audit-not-runtime-source.md`).
Финализационные строки несут только прогресс исполнения, воспроизводимый
из фактов сделки.
