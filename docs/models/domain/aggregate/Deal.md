...
пользователя; дом — `InstrumentExternalRules`,
`docs/decisions/instrument-currencies-home.md`). Прежде носителей было
...
Расчётная валюта — новое поле JSONB-навеса: на существующих строках она
появляется только после ближайшего тика синка. Ветка задана по образцу
...
сделок, открытых после ввода: они остаются для сделок, живых на момент
ввода.
...
(`docs/rules/trading-constraints.md` §«Гейт открытия сделки»).
...
- **Предикат составной по тропам** (H2 `DOCS_CHECK_11`, решение
  пользователя) — единого операнда у него нет, и это не пробел, а форма:
  тропы различаются тем, **дошла ли заявка до биржи**.
  ...
  - **Durable-носитель факта движения** — строки `DealCashFlow`,
    прилинкованные к сделке: движение средств и есть то, что они
    фиксируют, и они персистятся (`docs/models/domain/other/DealCashFlow.md`).
    Со стороны заявки тот же факт подтверждает
    `Order.accumulatedFillSize > 0` (`docs/models/domain/core/Order.md`).
    Оба операнда durable и выразимы запросом — это и требуется исторической
    выборке.
  - **Почему статуса достаточно на двух верхних тропах.** Обращения к
    бирже там не было ни одного, поэтому «проверка факта операций»
    вырождается в факт, уже выраженный статусом; тянуть ради него
    exchange-read значило бы вводить недостоверный операнд там, где есть
    достоверный.
  - **Рестарт не образует дыры.** Сделка, застигнутая рестартом между
    отправкой и наблюдением, доводится до нужного статуса рефрешем на
    **следующем запуске FSM** — тем же механизмом, которым добывается
    любой недобытый факт; отдельной ветки восстановления предикат не
    требует.
...
- **Неисчислимые исключаются как `unknown` и считаются поштучно.**
  ...
  `unknown`'ов **известно**. Сцепление с идемпотентностью журнальных
  отчётов **закрыто** (H16 `DOCS_CHECK_10`): `RESULT_PROFIT_UNAVAILABLE` —
  отчёт природы `kind = EVENT`, анкера не имеет и по разным сделкам
  одного инструмента **не схлопывается**
  (`docs/models/domain/other/AnomalyReport.md` §Енумы, `Kind`).
  Счётность `unknown`'ов обеспечена структурно, а не соглашением.
...
- **Инвариант, который нельзя потерять: такие сделки обязаны быть
  счётными.** Молча выпасть из выборки они не могут — иначе популяция,
