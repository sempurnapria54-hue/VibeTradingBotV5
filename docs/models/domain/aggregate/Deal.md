...
| `closeOutcome` | `CloseOutcome` | **Торговый исход закрытия** (`NORMAL_EXIT` / `LIQUIDATION` / `FORCED_REDUCTION`) — признак отбора для отчёта. Пишет финализация (штатная и аварийная) из `Position.externalCloseType`; техническая тропа FSM на значение не влияет (§«Признаки отбора для отчёта»). |
| `reconciliationStatus` | `ReconciliationStatus` | **Статус сверки разбивки** (`NOT_RUN` / `MATCHED` / `MISMATCHED`) — признак отбора для отчёта. Свойство числа, а не членства в популяции: сделка остаётся в популяции при любом значении. |
| `breakdownIncomplete` | `Boolean` | **Разбивка структурно неполна** — сделка длиннее окна доступности движений источника, часть строк недобываема в принципе. Отделяет «сверка звенит по дефекту» от «звенит по построению». |
...
- **`CloseOutcome`** (шаг 7, узел F `DOCS_CHECK_12`) — **торговый** исход
  ...
  - `FORCED_REDUCTION` — принудительное сокращение (ADL).
...
- **`ReconciliationStatus`** (шаг 7, узел F `DOCS_CHECK_12`) — исход
  ...
  - `NOT_RUN` — сверка не выполнялась (аварийный терминал; пустой операнд
    epsilon);
  - `MATCHED` — Σ`amount` сошлась с net в пределах epsilon;
...
| Структурная неполнота разбивки | `breakdownIncomplete` | `true` — сделка длиннее окна доступности движений |

- **`closeOutcome` — торговый факт, не техническая тропа.** Пишется
  финализацией (штатной и аварийной) из `Position.externalCloseType`
  (`docs/models/domain/core/Position.md`): `3..6` — закрытие биржей,
  ликвидация либо принудительное сокращение (ADL); иначе — штатный выход.
...
- **`breakdownIncomplete` — структурная, а не дефектная неполнота** (H23
  ...
  (bills — 7 дней, архив — до 3 месяцев), горизонт удержания сделки ничем
  не ограничен; у сделки длиннее окна часть строк недобываема **в
  принципе**, поэтому Σ`amount` заведомо не сойдётся с net. Признак
...
- **Сделка без состоявшихся операций — не исход системы.** Признак —
  ...
  составной предикат по тропам). Несостоявшиеся входы относятся к оси
...
- **Колонки шага 7 — `ALTER`, в `V6`/`V9` их нет** (H21, `DOCS_CHECK_8`):
  ...
  (`boolean`) — добавляются миграцией шага 7; полная schema-дельта
...
- **Индекс отбора для отчёта** — по (`status`, `close_outcome`): выборка
  ...
  низкоселективны и в индекс не идут.
...
- Enum-поля (`status`, `direction`, `entry_reason`, `entry_step_type`,
  ...
  `reconciliation_status`) хранятся строкой (имя enum); enum —
