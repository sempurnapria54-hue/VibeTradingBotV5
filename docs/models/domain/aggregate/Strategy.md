...
- **Direct partial close запрещён** как постоянный инвариант (см.
  `docs/rules/no-partial-close.md`). Полное закрытие выражается **и**
  условием-переходом `MANAGING → EXIT_PENDING`, **и** явным действием шага
  `EXIT` (решение держателя `GAPS_CLOSE_16`; market-close в обоих случаях
  ведёт `ExitPendingHandler`);
...
| `riskPerActionPercent` | `BigDecimal` | **Поактный** лимит риска: % от **базы риска** (свободный остаток расчётной валюты, `docs/decisions/per-trade-risk-policy.md` §«Определение и база») на одно risk-creating действие. Он же — единица измерения кумулятивного потолка (`K × riskPerActionPercent`) и операнд статической проверки против максимума стратегии; см. там же §«Три лимита внутри уровня „риск на сделку“». |
| `cumulativeRiskPerDealMultiplier` | `BigDecimal` | **Кумулятивный потолок риска на сделку — множитель поактного лимита** (`K`): риск, взятый сделкой за жизнь, не превышает `K × riskPerActionPercent × min(Deal.plannedRiskEquityBase, база риска текущая)`. **Не валидируется** — ни снизу, ни сверху (решение держателя `RISK-Q3-A`); при `K < 1` первая же нога не проходит, и это выбор автора, а не ошибка конфигурации. См. там же. |
| `strategySimultaneousRiskPerDealPercent` | `BigDecimal` | **Максимум одновременного риска стратегии**: % от базы риска, который эта стратегия допускает под ударом на одной сделке. Обязателен у торгуемой детали; инвариант — **не выше глобального максимума** (`globalSimultaneousRiskPerDealPercent`, конфиг), проверяется на create (400). Динамический преконтроль считает `liveRiskNow` против него, глобальный — страховка сверху. См. `docs/decisions/per-trade-risk-policy.md` §«Одновременный лимит двухуровневый». |
...
**Третий лимит — одновременный риск на сделку — двухуровневый** (C9
`DOCS_CHECK_21`, расширение держателя): жёсткий системный потолок
`globalSimultaneousRiskPerDealPercent` (**глобальный конфиг**, `1%`)
плюс собственный максимум стратегии
`strategySimultaneousRiskPerDealPercent` (**поле детали**, обязательное
у торгуемой). Прежняя редакция несла только конфиг; право стратегии
**сузить** себе бюджет добавлено, право расширить — нет: инвариант
«максимум стратегии ≤ глобального» проверяется на create (400).
Деталь проверяется **статически на create** и против своего максимума:
множество ног, способных быть живыми одновременно, не может
превысить его (§Персистентность,
...
**Полное закрытие выражается действием шага `EXIT` — либо остаётся
условием-переходом** (решение держателя `GAPS_CLOSE_16`; клауза «полного
закрытия как действия нет, позиционного подтипа `POSITION` в модели нет»
снята). Частичное уменьшение — reduce-only (инвариант — см. §Архитектурные
инварианты, `docs/rules/no-partial-close.md`).

**Форма действия — решение держателя (`GAPS_CLOSE_16`, вариант B; уровень
действия уточнён позицией С1):**

| Что вводится | Значение |
|---|---|
| третье значение `actionKind` | `POSITION` |
| третий подтип `StrategyAction` | `StrategyPositionAction` (`key`, `actionType`, `level`) |
| четвёртое значение `StrategyActionType` | `EXIT_ACTION` — суффиксный маркер уровня (`.claude/rules/naming.md` §«Разведение уровней абстракции»), не `CLOSE_FULL` и не `CLOSE_ACTION` |
| что делает исполнитель | **эмитит последовательность команд сам** — не запрашивает переход (`docs/components/ExitActionExecutor.md`) |

**Действие выхода состоит из нескольких команд — это разные уровни**
(решение держателя, позиция С1):

- **`CLOSE_POSITION_COMMAND` — команда:** закрывает позицию, и всё;
- **`StrategyAction` — действие стратегии:** может состоять из нескольких
  команд;
- **в стратегии закрытие одной позиции самостоятельного смысла не имеет.**
  Осмысленное действие — **выход из сделки**, и отмена живых входных ног
  входит **в его состав**, а не является внешней дочисткой. Отсюда имя
  `EXIT_ACTION`: прежнее `CLOSE_ACTION` называло одну команду, а не действие;
- **закрытие позиции означает закрытие сделки** — закрывать одну позицию и
  открывать другую внутри сделки смысла нет.

**Порядок команд внутри действия — инвариант, записанный один раз**
(`docs/rules/exit-teardown-order.md`): сначала отмена живых входных ног,
потом `CLOSE_POSITION_COMMAND`.

**Названная цена — эмитентов команды закрытия два.** `CLOSE_POSITION_COMMAND`
эмитят и `ExitPendingHandler` (тропа условия-перехода), и `ExitActionExecutor`
(тропа явного действия). **Общий компонент teardown не заводится, и дублем
это не является** (решение держателя): последовательность несут две тропы
**разной природы** — системный kill-switch задаёт её сам, штатный выход идёт
по условиям и действиям стратегии; эмиссия действия стратегии с системной
тропы отвергнута.

**Что от решения НЕ зависит:** `DealActionState` под такое действие
заводится обычным порядком — у него есть runtime-сущность (`Position`),
потому что исполнитель эмитит команду, а не переход.
...
ORDER↔ORDER / ALGO↔ALGO, форма действия полного закрытия /
...
- Типы числовых полей: `*Percents` / `*Score` / `*Ratio` /
  ...
  `NO_TRADE`-детали риска нет); `cumulativeRiskPerDealMultiplier` и
  `strategySimultaneousRiskPerDealPercent` — там же и с тем же правилом
  nullability. Колонка нового поля —
  `strategy_simultaneous_risk_per_deal_percent numeric(36,18) null`
  (schema-дельта шага 7 —
...
- **Одновременный лимит проверяется статически** (400), двумя
  неравенствами: `strategySimultaneousRiskPerDealPercent ≤
  globalSimultaneousRiskPerDealPercent` (реджект
  `STRATEGY_SIMULTANEOUS_RISK_ABOVE_GLOBAL`) и `N_overlap ×
  riskPerActionPercent ≤ strategySimultaneousRiskPerDealPercent`
  (реджект `STRATEGY_SIMULTANEOUS_RISK_UNSATISFIABLE`). `N_overlap` —
  максимальная мощность множества risk-creating действий, способных
  быть живыми **одновременно**, по всему дереву: пакет одного шага
  целиком **плюс** действия соседних шагов, ноги которых поздний шаг не
  снимает. Прежняя редакция считала только пакет одного шага
  (`n_risk_creating(step)`) и тем молча полагала, что ноги разных шагов
  не пересекаются. Веер ног, не помещающийся в максимум стратегии
  **никогда**, есть ошибка конфигурации, и её место — create
...
**Ограничение для дерева: `ONE_SECOND` не допускается** (H10
`DOCS_CHECK_15`). Значение введено шагом 7 под ссылку на свечу курса
cross-ccy-пересчёта; настройки дерева читают свечи из `CandleGroup`, а
групп секундного таймфрейма не существует, поэтому значение **реджектится
валидацией дерева** (`docs/decisions/strategy-materialization-and-validation.md`).
Ограничение записано здесь, а не выводится читателем из отсутствия групп.
