...
*(Форма действия — открытая развилка, `.claude/work/progress/phase-1-step-7-gaps-close-16.md` §«Развилки, возвращаемые на валидацию», позиция «Форма действия полного закрытия».)*
От неё зависят: третье значение `actionKind`, третий подтип
`StrategyAction`, четвёртое значение `StrategyActionType` и то, заводится
ли под такое действие `DealActionState` без runtime-сущности. До её
прохождения ни одно из четырёх здесь не объявляется.
