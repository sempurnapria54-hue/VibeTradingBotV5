...
**Форма действия — решение держателя (`GAPS_CLOSE_16`, вариант B):**
...
| четвёртое значение `StrategyActionType` | `CLOSE_ACTION` — суффиксный маркер уровня (`.claude/rules/naming.md` §«Разведение уровней абстракции»), не `CLOSE_FULL` |
| что делает исполнитель | **эмитит `CLOSE_POSITION_COMMAND` сам** — не запрашивает переход |

**Названная цена — эмитентов команды становится два.** `CLOSE_POSITION_COMMAND`
теперь эмитят и `ExitPendingHandler` (тропа условия-перехода), и исполнитель
`CLOSE_ACTION` (тропа явного действия). Teardown («market-close всё +
дочистка») перестаёт быть в одних руках. Цена принята держателем;
**чем закрывается расхождение дочистки между двумя путями — открыто**
(`.claude/work/progress/phase-1-step-7-gaps-close-16.md` §«Развилки,
возвращаемые на валидацию», позиция «Дочистка на двух путях полного
закрытия»). До её прохождения не объявляется, кто отменяет живые ноги и
защиту на тропе явного действия и в каком порядке.
