...
нет ни одного поля; стратегия, объявившая такой источник размещения,
отвергается **на создании**
(`docs/models/domain/aggregate/Strategy.md`). Подстановка меняла бы ценовой домен
молча: базис — расхождение марк-цены с последней — уехал бы прямо в цену
заявки.
...
**`askSz`/`bidSz` введены** (шаг 7 фазы 2):
глубина топа стакана — **измеритель ёмкости инструмента**, фиксируемый в
момент постановки ноги на `Order.bookDepthAtPlacement`. Он ничего не блокирует: по его распределению после
