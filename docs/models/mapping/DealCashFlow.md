...
ниже). `deal_id` проставляется матчингом, не маппером. `appliedRate` источник
не несёт вовсе — его проставляет `RefreshBillsExecutor` при пересчёте
cross-ccy-строки (§«Число — в settle-ccy»), маппер к нему не причастен.
...
| cross-ccy-слагаемое = Σ(`amount` × `appliedRate`) | строки, у которых `ccy` ≠ расчётной валюты инструмента | довнести в число издержку, уплаченную вне settle-ccy (H5 `GAPS_CLOSE_6`; операнд и курс — H4 `GAPS_CLOSE_7`) |
...
- **Конфигурируется по бирже.** Перечень посторонних типов — свойство
  ...
  > (`.claude/work/backlog.md` §Шаг 7). До назначения дома список для OKX
  > пуст — это честный старт: пустой список исключений означает «всё
  > входит в сверку», и посторонние типы, если они есть, проявятся через
  > `OTHER` и расхождение сверки, а не спрячутся.
...
- **Epsilon — двухчастный, срабатывает меньший** (H15 `DOCS_CHECK_10`):
  `min( max(0.01 settle-ccy, 0.5% × Σ|amount|), k × ожидаемая комиссия
...
> инструмента»). Открытым остаётся именование поля расчётной валюты и
> область модели — CCY-Q2 (владелец `solution-designer`); до закрытия
> имена полей — предварительные.
