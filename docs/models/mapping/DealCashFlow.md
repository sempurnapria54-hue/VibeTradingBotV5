...
> **Незакрытый хвост узла: носитель расчётной валюты.** Сегодня
> base/quote/settle приходят **транзиентно** в `InstrumentExternalSnapshot` и
> персистентного дома не имеют (`docs/models/domain/core/Instrument.md`
> §«Биржевое воплощение»; `InstrumentExternalRules` на них **не претендует**).
> Валюту **надо начать хранить**; выбор носителя вынесен предложением на
> валидацию владельцу (`GAPS_CLOSE_7` §Предложения) и в этом под-шаге не
> материализован. До его выбора операнд объявлен, но дома не имеет — это
> **единственный** незакрытый хвост узла, и он помечен здесь явно, а не
> подразумевается.
...
исторический бар по `uTime`: он покупает детерминизм, который уже даёт
записанный `appliedRate`, ценой ещё одного read-пути.
