...
- **Ключи узлов копии — свои.** Переход domain → persistence у копии ядра
  не переносит ключа ни одного узла: снимок приезжает
  с шины, числовой ключ базы владельца границу сервиса не пересекает
  (`docs/architecture/data-ownership.md` §Идентификаторы), и ключи строкам
  заводит база ядра при вставке. Перенесённый чужой ключ сделал бы вставку
  слиянием — со строкой, которой нет, либо с чужой копией.
...
- **Снимок активации — копия доменного дерева без числовых ключей базы.**
  Содержимое `StrategyActivated` несёт дерево доменным типом, но ни у
  одного узла нет ключа строки: узел опознаётся положением в дереве,
  внешние ссылки — `internalId`
  (`docs/architecture/data-ownership.md` §Идентификаторы). Строится копия,
  а не очистка на месте: вызывающий держит то же дерево и после записи
  факта.
