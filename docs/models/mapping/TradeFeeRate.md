...
(`docs/models/domain/other/TradeFeeRate.md` §«Масштаб модели»): именно потому,
...
одном ключе (H7, `GAPS_CLOSE_4`). Проекция остаётся для runtime-логики.
...
- **Temporary API problem** (timeout, 5xx): тик синка логирует и уходит;
  ...
  группы»). Отдельного счётчика отказов нет — довод см. там же, §«Свежесть
...
**Механизм «не затирается» — отсутствие записи, не IGNORE-null** (H10,
`GAPS_CLOSE_4`). Последняя известная ставка сохраняется потому, что на
...
| `feeGroup[].taker` | `externalTakerFeeRate` (**× −1**, см. §«Знак ставки — снимается здесь») |
...
не наблюдаем (мы не в MM/ELP-программах — §«OKX validation notes»,
...
- **Ось запроса — группа, не инструмент:** один вызов
  ...
  **Частичное покрытие — определённый исход, не пробел спеки.** Группа, не
  ...
  только они (`docs/models/domain/other/TradeFeeRate.md` §«Свежесть → холд
  инструментов группы»). Свежесть измеряется **по группе**, поэтому вопрос
