...
- **Что снято.** Прежняя редакция (H7/H10 `DOCS_CHECK_14`) оставляла
  ...
  `resultProfit = null`, а биржевая заморозка (`Exchange.HOLD`,
  ступень 1) поднимается **параллельно** —
...
- **Что это снимает по построению.** Финализатор перестаёт быть местом
  ...
  класса — заморозка ступени 1 (`FREEZE`, фабрика
  `exchange(code, exchangeId)`; до лестницы — `FULL`) — из
...
- **Форма пустого значения несобытийных полей — предусловие, и теперь
  ...
  - Перенос проверки на границу цену ошибки поднимает: раньше она давала
    неверный признак, теперь — отказ добычи и биржевая заморозка
...
- **Invalid response / нарушен контракт записи** (`code != 0`,
  ...
  заморозку** (`Exchange.HOLD`, ступень 1 с ручным снятием,
