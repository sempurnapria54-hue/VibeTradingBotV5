...
- **Что остаётся у финализатора.** Только то, чей операнд **не** приходит
  ...
  нерезолвимом **уровне стопа** по предикату селекции защиты (H2
  `DOCS_CHECK_15`; `ctVal` из перечня выбыл — H5 `DOCS_CHECK_16`,
  персистится на ноге; `docs/components/FinalizeDealExitExecutor.md`
  §epsilon).
