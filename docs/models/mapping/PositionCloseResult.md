...
### Знак `fee` и `liqPenalty` — тем же прогоном, и он гейтит (H15 `DOCS_CHECK_16`)
...
**кейсом `AG1.7`**, фикстура та же, лишнего прогона нет. Внесено в реестр
предусловий `CODE` п. 7 по паритету с пп. 7 и 9
...
**`externalSize` на create-тропе остаётся пустым** (H14 `DOCS_CHECK_16`).
...
nullable-колонку. Пишется **`null`**, не `0`:

- «равен нулю» — **вывод из состояния** (позиция закрыта), а не наблюдение;
  запись вывода как факта стирает различение «наблюдали flat» ↔ «не
  наблюдали вовсе», а оно рабочее для шага 8;
- подстановка благоприятного значения по умолчанию там, где наблюдения не
  было, запрещена сквозным правилом
  (`docs/rules/absent-value-semantics.md` §«Благоприятное значение по
  умолчанию запрещено»; `docs/models/domain/aggregate/Deal.md` — «подстановка
  нуля запрещена»); запись `ZERO` как названного исключения завела бы
  прецедент против правила, введённого этим же шагом;
- факт «позиция закрыта» уже несёт `Position.status = CLOSED`, и
  `hasLiveRisk()` от пустого размера не меняется — предикат стоит на статусе.

Ни один предикат шага 7 от этого выбора не зависит — находка `CODE` не
гейтит; выбор записан, чтобы писатель не решал его молча.
...
- **Запись не найдена** — не ошибка команды: статус `CLOSED` уже поставлен
  ...
  действия**. Дальше — **по тропе, и они не одинаковы** (H19
  `DOCS_CHECK_16`; прежняя безусловная редакция «сделка уходит тропой
  "неисчислимо"» снята):
  - **штатная тропа** (`Deal.status = EXIT_PENDING`) — тропы «неисчислимо»
    здесь **нет**: `FINALIZE_DEAL_EXIT_COMMAND` эмитится только по
    терминальному исходу добычи и без числа не завершается; исчерпание
    бюджета уводит сделку **ошибочной** тропой + холд инструмента (узел 4
    `DOCS_CHECK_8`, вариант (а);
    `docs/components/FinalizeDealExitExecutor.md` §Назначение;
    `docs/decisions/pnl-finalization-mechanics.md` §«`FINALIZE_DEAL_EXIT_COMMAND`
    не завершается без числа»). Буквальное чтение прежней редакции давало
    чистый `CLOSED` с `resultProfit = null`, что запрещено инвариантом
    непустоты;
  - **аварийный контур** (`Deal.status = ERROR` → `EMERGENCY_CLOSED`) —
    тропа «неисчислимо» законна: `resultProfit = null` (не ноль), терминал
    ставится всё равно (реш.3).
