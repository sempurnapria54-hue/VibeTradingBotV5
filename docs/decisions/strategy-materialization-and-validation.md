...
- **Исключение из «торгово-суждённое — не на create»: одновременный
  лимит риска** (`RISK-Q3-A`, шаг 7). На create проверяется, что пакет
  действий любого шага в лимит **помещается**:
  ...
  где правая часть — **глобальный конфиг**, не поле стратегии
  (`docs/decisions/per-trade-risk-policy.md` §«Три лимита внутри уровня
  „риск на сделку“»). **Почему это create, а не activate:** проверка не
  суждённая, а арифметическая — она сравнивает объявленный веер ног с
  числом, известным до запуска, и её нарушение означает пакет, который
