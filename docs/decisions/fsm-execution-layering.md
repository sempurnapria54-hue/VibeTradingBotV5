...
`CLOSE_POSITION_COMMAND` и ничем иным. Владение teardown при этом
**раздваивается**: команду эмитит и `ExitPendingHandler`, и исполнитель
`CLOSE_ACTION` (вариант B решения держателя). Довод «вырожденный подтип +
пустая подтаблица» отвергнут — способ объявления выхода признан
самостоятельной ценностью, а не дублем условия; цена (два эмитента,
дублирование дочистки) названа и принята.

**Форма действия назначена** (`GAPS_CLOSE_16`, вариант B): `actionKind =
POSITION`, подтип `StrategyPositionAction`, `StrategyActionType.CLOSE_ACTION`;
исполнитель эмитит `CLOSE_POSITION_COMMAND` сам
(`docs/models/domain/aggregate/Strategy.md` §Действия). **Открытым остаётся
одно — дочистка на двух путях** (`.claude/work/progress/phase-1-step-7-gaps-close-16.md`
§«Развилки, возвращаемые на валидацию», позиция С1).
