...
действует только в части **механизма**: teardown по-прежнему один и
владеет им `ExitPendingHandler`. Довод «вырожденный подтип + пустая
подтаблица» держателем отвергнут — способ объявления выхода признан
самостоятельной ценностью, а не дублем условия.
