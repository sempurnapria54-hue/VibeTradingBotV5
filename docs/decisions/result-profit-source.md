...
- **Заголовочное число** `resultProfit` = **net realized P&L**, берётся
  **готовым** из positions-history (`GET /api/v5/account/positions-history`):
  ...
  без нашей композиции слагаемых.
  - **Сделка многоэпизодна ⇒ число — Σ по эпизодам**
    (`docs/decisions/multi-episode-deal.md`): у каждого эпизода свой
    `posId` и своя финализированная запись, и наша композиция здесь —
    **только сложение готовых net'ов**, не разложение на слагаемые.
    Прежняя редакция «одним запросом» описывала одноэпизодный случай;
    запрос по-прежнему один — записей в ответе может быть несколько.
