...
2. **Непустой список исключений сверки для OKX** — **гейтит** (кейсы
...
7. **Знак и горизонт накопления `fundingFee`** (`AG1.7`) — **гейтит**
   ...
   `docs/decisions/per-trade-risk-policy.md` §«Асимметрия числителя и знаменателя»). Знак
...
16. **Непустое отображение `type`/`subType` → `CashFlowCategory` для
    OKX** — **гейтит** (B1 `DOCS_CHECK_22`; кейс **`AG6.1`** — тот же
    справочник, что наполняет п. 2; имя уточнено N2 `DOCS_CHECK_23`:
    прежнее «`AG6`» — заголовок группы, не кейс, а оговорка «тот же, что
    у п. 2» указывала на `AG6.2` и расходилась с разметкой плана).
    Структурная половина закрыта: носитель — тот же per-exchange
    `@ConfigurationProperties`, что у списка исключений
    (`docs/models/mapping/DealCashFlow.md` §«Резолв категории»). Гейтит
    **содержание**: без отображения все строки садятся в `OTHER`, и
    тогда одновременно детерминированный `MISMATCHED` на каждой сделке,
    `UNCLASSIFIED_CASH_FLOW` на каждой сделке и направленное завышение
    `resultProfit` (cross-ccy-слагаемое отсекается конъюнктом «категория
    экономическая»). Отдельным пунктом, а не расширением п. 2: тот
