...
- **Закрывает STRUCT-Q2 по построению.** Разделяемого ряда больше нет:
  ...
  Это снимает не только схемный краевой случай, но и **латентную торговую
  некорректность**: при шаринге структура, посчитанная по «чужому»
  ER/ATR-входу, отдавалась второму потребителю; теперь результат всегда
  соответствует входам своей настройки (инвариант «вход настройки → её
  результат», торговый фокус ТВ-4 пост-D DOCS_CHECK).
