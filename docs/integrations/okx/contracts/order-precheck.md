...
Каков контракт OKX-операции order precheck (серверная пре-оценка
влияния ордера на счёт до постановки): endpoint, поля, применимость.
...
Не используется. Форвард-кандидат **В-2** (шаг 5, преконтроль) — с
существенным ограничением применимости (ниже).
