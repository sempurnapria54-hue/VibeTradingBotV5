...
- `type` (опц.) — тип bill-записи (актуальный список — справочник
  OKX). **Рантайм (2026-06-19, demo, контур source-api):**
...
Bill-записи добывает команда рефреша движений: пагинация
«7 дней → 3 месяца» проходится **внутри команды**, результат наполняет
доменный носитель разбивки.

**Валюта в запрос не идёт** — это требование к запросу, а не деталь
потребителя: фильтр по валюте убивал бы контроль чужой валюты —
нарушающая запись не пришла бы в ответе вовсе.

Окно запроса, предикат привязки записи к сделке, резолв категории,
ветка чужой валюты и состав сверки здесь не пересказываются — их дома:
`docs/models/domain/other/DealCashFlow.md`,
`docs/models/mapping/DealCashFlow.md`, `docs/rules/pnl-reconciliation.md`,
`docs/spec/cash-flow-linkage.json`, `docs/spec/pnl-reconciliation.json`.

Пагинация применима к окнам ≤ 3 месяцев; глубже — архив ниже.
...
инициирует архив (заявка → `50026`, `GET` → `51604`). Зелёный кейс
...
**Мощность справочника — 32 верхних типа и 179 пар** `type`/`subType`
(прогон `AG6.1` 2026-08-30, провенанс `рантайм`).

### Семейство принудительного закрытия — строки справочника

Записаны здесь, потому что они — операнд предусловия `CODE` п. 10 (след
автоделевериджа), гейтящий статус которого снят решением держателя
2026-08-30 в пользу заменителей
(`.claude/decisions/unorderable-fact-substitutes.md`). Форма события взята
не из офдоковой прозы, а **из собственного справочника источника**
(прогон 2026-08-30, провенанс `рантайм`):

| `type` | `typeDesc` | Подтипы принудительного закрытия |
|---|---|---|
| `9` | ADL | `125` ADL close long, `126` ADL close short, `127` ADL buy, `128` ADL sell |
| `5` | Forced liquidation | `100` ADL close long, `101` ADL close short, `102`/`103` Partial liquidation buy/sell, `104`-`107` Liquidation, `116` Balance deducted for liquidation |

**Два следствия, и оба несущие.**

1. **След ADL живёт под двумя верхними типами** — выделенным `9` и
   `5` (подтипы `100`/`101`). Отображение, ключом которого взят только
   тип `9`, ADL под типом `5` не увидит.
2. **Тип `5` гетерогенен:** под ним лежат и движения **закрытия**
   (`100`-`107`), и движение **штрафа** (`116`). Резолв «тип без подтипа»
   для типа `5` склеил бы их в одну категорию, то есть завысил бы
   слагаемое штрафа на объём закрытия. Поэтому типового умолчания у `5`
   не заводится: непокрытая точной парой строка садится в `OTHER` с
   отчётом `UNCLASSIFIED_CASH_FLOW` — громкое направление ошибки вместо
   тихого (`docs/models/mapping/DealCashFlow.md`).

**Чего эти строки НЕ устанавливают.** Что биржа фактически эмитит при
эпизоде ADL и с какими суммами — справочник не говорит: он перечисляет
возможные виды движений, а не наблюдённые. Наблюдение назначено на
запуск (`.claude/work/backlog.md`).
