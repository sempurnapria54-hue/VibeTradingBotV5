...
1. Определить окно сделки — СОБСТВЕННЫЕ ПОЛЯ Deal:
   - begin = Deal.billsWindowBegin; при его пустоте —
            СУРРОГАТ Deal.externalCreatedAt;
   - end   = СЕРВЕРНОЕ ВРЕМЯ ИСТОЧНИКА прохода. Подвижная
            граница ЗАПРОСА; линковку ограничивает
            нетерминальность Deal.
...
3. Линковка к сделке (bills НЕ несут dealId) — предикат и его разбор
   ...
   с домом. Выход матчинга закрепляется как
...
4. Сохранить как DealCashFlow (категорийная разбивка); резолв категории
   ...
   - ccy != расчётная валюта инструмента -> персист + линковка (deal_id
     проставляется так же) + ПЕРЕСЧЁТ СРАЗУ + AnomalyReport (нарушение инварианта «комиссии только
...
5. Сверка (при финализации) — ЧЕТЫРЬМЯ ПАРАМИ ПО КАТЕГОРИЯМ; суммарной
   сверки sum(amount) по всей разбивке БОЛЬШЕ НЕТ:
   ...
   ОБЛАСТЬ СВЕРКИ ЗАДАНА СПИСКОМ ИСКЛЮЧЕНИЙ ПО БИРЖЕ:
   ...
   НЕПУСТОЙ СПИСОК — ПРЕДУСЛОВИЕ CODE: с пустым списком
   ...
   Расхождение — ОБЩЕЕ ПО СДЕЛКЕ → AnomalyReport (аудит-аномалия, НЕ блок
...
6. Итоговое число — в settle-ccy (USDT):
   Deal.resultProfit = net из positions-history
                     + sum(amount × appliedRate) по строкам
                       ...
                       (не OTHER) — конъюнкция целиком.
   ПРЕДИКАТ — ПРИЗНАК СОСТОЯНИЯ КУРСА, НЕ «чужая ccy»:
