...
5. Сверка (при финализации):
   ...
   positions-history (realizedPnl). Строки, пересчитываемые в settle-ccy,
   в эту сумму не складываются (разные валюты), они входят в число
   слагаемым — см. п.6.
   ОБЛАСТЬ СВЕРКИ ЗАДАНА СПИСКОМ ИСКЛЮЧЕНИЙ ПО БИРЖЕ (H14 DOCS_CHECK_10):
   из суммы вычитаются типы, не принадлежащие экономике сделки (переводы,
   ...
   Расхождение сверх epsilon (двухчастный: min(композиционный по обороту,
   омиссионный по ожидаемой комиссии) — H15 DOCS_CHECK_10) → AnomalyReport
   (аудит-аномалия, НЕ блок финализации; см.
   pnl-finalization-mechanics.md реш.5).

6. Итоговое число — в settle-ccy (USDT):
   Deal.resultProfit = net из positions-history
                     + sum(amount × appliedRate) по строкам rateStatus=APPLIED.
...
по строкам сделки **в расчётной валюте** и **вне списка исключений биржи**
(H14 `DOCS_CHECK_10`), включая `FUNDING` (net из
...
из него исключён (holding-cost, не realized pnl). Пересчитанные строки — своя,
четвёртая область (H5, `GAPS_CLOSE_6`): в валютные суммы они не складываются,
а входят слагаемым в число по зафиксированному `appliedRate` — предикат
области **`rateStatus = APPLIED`** (H9 `DOCS_CHECK_11`), не `ccy`. Области —
