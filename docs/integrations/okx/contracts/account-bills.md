...
1. Определить окно сделки — СОБСТВЕННЫЕ ПОЛЯ Deal (узел 1 DOCS_CHECK_8;
   ...
   - end   = СЕРВЕРНОЕ ВРЕМЯ ИСТОЧНИКА прохода (GET /public/time,
            ...
            граница ЗАПРОСА (Р7 GAPS_CLOSE_16); линковку ограничивает
            нетерминальность Deal (C1 DOCS_CHECK_19).
            Неподвижная Deal.billsWindowEnd (uTime записи закрытия
            позднейшего эпизода, пишет вторая нога
            REFRESH_POSITION_COMMAND, монотонно вперёд) — граница
            контроля полноты и порог покрытия, НЕ запроса и НЕ линковки.
   ...
   (факт закрытия не добыт) -> ДОБЫЧА идёт и ПРИВЯЗКА идёт, не
   завершается ЗВЕНО; суррогатом окно не закрывается.
...
3. Линковка к сделке (bills НЕ несут dealId) — предикат и его разбор
   ...
   DealCashFlow.deal_id при сохранении; валюта критерием матчинга НЕ
   является — она проверяемый атрибут (п.4).
...
4. Сохранить как DealCashFlow (категорийная разбивка); резолв категории
   ...
   - ccy == расчётная валюта инструмента -> штатно, rateStatus=NOT_REQUIRED;
   - ccy != расчётная валюта инструмента -> персист + линковка (deal_id
     проставляется так же) + ПЕРЕСЧЁТ СРАЗУ (курс — ИЗ СВЕЧИ НА МОМЕНТ
     ОПЕРАЦИИ, секундной при доступности, с деградацией к более грубой;
     кладётся в DealCashFlow.appliedRate, rateStatus=APPLIED — H25
     DOCS_CHECK_11) + AnomalyReport (нарушение инварианта «комиссии только
     в settle-ccy», docs/rules/trading-constraints.md).
     Курс НЕ берётся тикером на момент обработки — эта редакция снята:
     appliedRate фиксируется однократно и навсегда входит в resultProfit,
     а тикер невоспроизводим и неустойчив к одиночному выбросу.
     Котировка не получена -> rateStatus=RATE_UNAVAILABLE (долг догона);
     расчётная валюта не резолвится -> rateStatus=SETTLE_CURRENCY_UNAVAILABLE.
     Запись НЕ отбрасывается и НЕ теряется.

5. Сверка (при финализации) — ЧЕТЫРЬМЯ ПАРАМИ ПО КАТЕГОРИЯМ; суммарной
   сверки sum(amount) по всей разбивке БОЛЬШЕ НЕТ (H7 GAPS_CLOSE_13;
   рецепт выровнен с прозой H3 DOCS_CHECK_14):
   ...
   Каждая сумма — по строкам расчётной валюты вне списка исключений
   биржи. Строки, пересчитываемые в settle-ccy, в пары не складываются
   (разные валюты), они входят в число слагаемым — см. п.6.
   ОБЛАСТЬ СВЕРКИ ЗАДАНА СПИСКОМ ИСКЛЮЧЕНИЙ ПО БИРЖЕ (H14 DOCS_CHECK_10):
   из сумм вычитаются типы, не принадлежащие экономике сделки (переводы,
   добавление/снятие маржи и т. п.). Список — ОТ ПРОТИВНОГО: новый type
   источника по умолчанию входит в расчёт, и если он посторонний,
   обнаруживается сам через непустую корзину OTHER, а не выпадает молча.
   ...
   H15 DOCS_CHECK_10) → AnomalyReport (аудит-аномалия, НЕ блок
   финализации; см. pnl-finalization-mechanics.md реш.5).

6. Итоговое число — в settle-ccy (USDT):
   Deal.resultProfit = net из positions-history
                     + sum(amount × appliedRate) по строкам
                       cross-ccy-области: rateStatus=APPLIED И тип вне
                       списка исключений И категория экономическая
                       (не OTHER) — конъюнкция целиком (H11
                       GAPS_CLOSE_13; рецепт выровнен H2 DOCS_CHECK_14).
   ПРЕДИКАТ — ПРИЗНАК СОСТОЯНИЯ КУРСА, НЕ «чужая ccy» (H9 DOCS_CHECK_11):
   вторая формулировка затягивает в произведение строки с пустым appliedRate
   (RATE_UNAVAILABLE / SETTLE_CURRENCY_UNAVAILABLE) и даёт NULL всей суммы.
