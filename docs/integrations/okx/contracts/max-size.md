...
Каков контракт OKX-операций оценки максимального размера ордера
(`max-size`) и доступного баланса/эквити под сделку
(`max-avail-size`).
...
Не используется. Релевантность — преконтроль размера (шаг 5):
