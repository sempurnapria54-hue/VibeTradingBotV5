...
| Bills 7d | GET `/account/bills` | есть-док | 🟢 в коде | офдок | `account-bills.md`, `OkxAccountBillResponse`; **`GAPS_CLOSE_2`:** команда **`REFRESH_BILLS_COMMAND`** → `DealCashFlow` (разбивка P&L; `DealCashFlow.md`), линковка по **окну + `instId`** (`ccy` снят из критериев матчинга — `GAPS_CLOSE_6`, H5; манифест выровнен — H27 `GAPS_CLOSE_7`), дедуп по паре (`exchangeId`, `billId`) — H24 `GAPS_CLOSE_13`; метода клиента нет |
...
| Ticker | GET `/market/ticker` | есть-док | 🟢 в коде | офдок | `market-price-data.md`, `OkxTickerResponse`; `getTicker`. Носителем курса cross-ccy **не является** — H25 `DOCS_CHECK_11` выбрал свечу на момент операции: единичный тик наименее робастен, а `appliedRate` фиксируется однократно и навсегда входит в число |
