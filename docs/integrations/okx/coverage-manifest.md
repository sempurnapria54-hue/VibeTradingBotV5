...
| Bills 7d | GET `/account/bills` | есть-док | 🟢 в коде | офдок | `account-bills.md`, `OkxAccountBillResponse`; **`GAPS_CLOSE_2`:** команда **`REFRESH_BILLS_COMMAND`** → `DealCashFlow` (разбивка P&L; `DealCashFlow.md`), линковка по **окну + `instId`** (`ccy` снят из критериев матчинга — `GAPS_CLOSE_6`, H5; манифест выровнен — H27 `GAPS_CLOSE_7`), дедуп по `billId`; метода клиента нет |
