...
**Ветка „радиус локализован“: бросок радиусной реакции не поднимается,
но бюджет расходуется** (H3/H4 `DOCS_CHECK_15`, решение пользователя;
прежняя редакция «`attemptCount` не растёт» **снята**; дискриминатор
расширен до дизъюнкции — A3 `DOCS_CHECK_20`).
...
- **Оба дизъюнкта ратифицированы, и ни один не покрывает другой.**
  Первый — карв-аут аварийной тропы (H3/H4 `DOCS_CHECK_15`): live risk
  снят самой тропой, осталось довести сделку до терминала. Второй — C5
  `DOCS_CHECK_19`: на **штатной** тропе отказ добычи **разбивки** при
  доказанно закрытой позиции платил полным холдом инструмента, то есть
  в контуре фазы 1 остановкой контура из-за недоступности книжного
  эндпоинта. Радиусный довод там истинен дословно, но не применялся.
- **Почему дизъюнкция, а не замена** (A3 `DOCS_CHECK_20`). Одного
...
- **Пустая коллекция `positions` — не «живого риска нет».** Конъюнкция
  ...
  умолчанию, что запрещено `docs/rules/absent-value-semantics.md`
  §«Благоприятное значение по умолчанию запрещено». Аварийная тропа
...
- **Дискриминатор — дизъюнкция, разобранная выше** (C5
  `DOCS_CHECK_19` + A3 `DOCS_CHECK_20`). Оба операнда durable и собраны
  из существующих полей.
  - **Форма реакции этим не трогается.** Ратифицированное «форма
    реакции на исчерпание бюджета одна: полная»
    (`docs/rules/instrument-hold.md`) отвечает на вопрос «что делать,
    когда бросок случился»; здесь решается **бросается ли исключение
    вообще** — слой, который этот док уже отделил и в котором
    дискриминатор узаконен.
  - **Что остаётся вместо холда:** `AnomalyReport` и ошибочный терминал
    — ровно форма, ратифицированная решением П11 для локализованного
    радиуса (`docs/components/ExitPendingHandler.md` §«Выходные
    проверки»).
  - **Цена названа:** сигнал «не смогли добыть разбивку» перестаёт быть
    громким. Он остаётся счётным — журнальный отчёт на сделку плюс
    `reconciliationStatus`/`breakdownIncomplete`, — и именно эта ось
    (частота возможностей против отчётности) взвешена в пользу
    возможностей [Tharp гл.6 с.130-133,156-157; гл.11 с.279: годовой
    профит часто делает одна сделка, её нельзя пропустить].
