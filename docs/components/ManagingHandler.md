...
`FAIL_SAFE`. Команды: `REFRESH_BALANCE`, `AMEND_ALGO_ORDER`,
`CREATE_ALGO_ORDER`, `SUBMIT_ALGO_ORDER`, `CANCEL_ALGO_ORDER`,
`CREATE_ORDER`, `SUBMIT_ORDER`, `AMEND_ORDER`, `CANCEL_ORDER`,
`CLOSE_POSITION`, `REFRESH_POSITION`, `REFRESH_PENDING_ORDERS`,
`REFRESH_ALGO_ORDERS`, `REFRESH_FILLS`, `MARK_DEAL_ERROR`,
`EXECUTE_KILL_SWITCH`.
