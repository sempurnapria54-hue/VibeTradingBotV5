...
§`CreateOrderCommandPayload` (шесть операндов тождества + седьмое поле
`liquidationDistanceRatio`). Тем же путём, каким уже едут цена и
размер: строит payload та же сторона, что вызывает `RiskValidator`, и в
том же проходе. **Перечень здесь не дублируется** — прежняя редакция
называла четыре поля и отстала от состава (B4 `DOCS_CHECK_18`).
