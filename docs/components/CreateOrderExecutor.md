...
**Писатель — тот же per-leg executor** (H5 `DOCS_CHECK_12`, решение
пользователя): `CreateOrderExecutor` пишет операнды на `Order`,
`CreateAlgoOrderExecutor` — на `AlgoOrder`, каждый в той же транзакции, что
создаёт свою сущность. Это прямое следствие «дом — по тому, кто фактически
исполнился», и оно **не ломает** канон «CREATE-команда создаёт сущность и
пишет её поля одной транзакцией» (альтернатива «один писатель на границе
риск-преконтроля» его ломала бы). От исхода `RISK-Q4` назначение писателя
**не зависит**: `Deal` отвергнут при любом исходе, вопрос лишь в том, какая
из двух сущностей несёт колонки.
