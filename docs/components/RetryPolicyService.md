...
исполнения действия** (β, `docs/rules/command-lifecycle.md`), без обнуления при продвижении стадии. У многозвенного системного
...
**Расхождение доков с кодом.** Механизм конфигурации построен (`ServiceCommandRetryProperties`:
...
отсутствующего default'а (`.claude/work/backlog.md` 7).
...
**Чем `FAILED` доезжает до детектора**. Перевод в
...
`docs/components/ServiceCommandExecutor.md`. Пока
