...
- **Предикат ветки — «операций по сделке не было»**, составной по тропам
  (H2 `DOCS_CHECK_11`): на двух `PRECHECK`-тропах операнд — **статус
  сделки** (отправки не было по построению), на тропе из
  `ENTRY_SUBMITTED` — **факт движения средств на бирже**
  (`docs/models/domain/aggregate/Deal.md` §«Рамка R-выборки»). Ноль здесь
  — **исход проверки**, а не молчаливый fallback.
...
- **Почему валюта может не резолвиться именно здесь.** На тропе
  ...
  расчётная валюта может быть не заполнена вовсе (колонка новая, до
  ближайшего тика синка пуста).
