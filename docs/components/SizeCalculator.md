...
действия**: таблица
ниже описывает **reduce-only выход**; защитное действие идёт по ниже.
...
`docs/rules/live-risk-protection.md`) считается **не по
этой таблице**:
...
упирается в `minSz` по построению ,
поэтому всякая позиция меньше `N × minSz` делает лестницу из `N` ступеней
невыразимой.

**Отмена точечная.** Решение «размер объявленной доли не
...
- **Пол `minSz` остаётся у входного сайзинга**  — там он не меняет намерение, а лишь упирает размер в
...
(`docs/rules/risk-policy.md`). Убыток на стопе для линейного
...
шага 7** — G6, `docs/rules/risk-policy.md`).

**Считается закрытой формой, не подбором** — комиссия
...
`docs/rules/pnl-reconciliation.md` реш.4). **Дом ставки** — не
...
`docs/components/InstrumentExternalRulesDataService.md`. Для калькулятора seam тот же — поверхность
чтения не изменилась. Аксессор отдаёт **издержку** (знак биржевой конвенции
снят при маппинге, `docs/models/domain/other/TradeFeeRate.md`),
...
- **сделочные лимиты калькулятор не считает** — оба проверяются
  ...
  `docs/rules/risk-policy.md`): подбирать размер под остаток бюджета значило бы
...
- запас на проскок за стоп в фазе 1 не закладывается (убыток по цене
  ...
  пересмотра — `docs/rules/risk-policy.md`.
...
плеча нет (`docs/rules/risk-policy.md`).
