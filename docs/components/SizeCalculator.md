...
- **потолок сделки калькулятор не считает** — он проверяется
  `RiskValidator`'ом над готовым размером (`RISK_PER_DEAL_EXCEEDED`,
  `docs/decisions/per-trade-risk-policy.md` §«Лимит риска
  двухуровневый»): подбирать размер под остаток бюджета значило бы
