...
временных домена, и смешивать их запрещено»). Цена реджекта на **одном**
тике — пропуск входа: риск ещё не взят, отказ безопасен и симметричен
прочим gatekeeper-отказам скана.

**Названное ограничение: устойчивая недоступность останавливает все
входы контура, и событие не счётно** (N5 `DOCS_CHECK_21`). Прежняя
редакция называла цену **только на тик**, тогда как отказ не обязан быть
одиночным: пока `public/time` недоступен, ни одна сделка не создаётся —
ни по одному инструменту, ни по одной стратегии. Это **новый канал**
остановки контура, у которого нет ни носителя состояния (как
`Exchange.HOLD`), ни следа: сделка не заводится, строки нет, счётчика
пропусков нет.
...
- **Класс тот же, что у `PNL-Q1` п.2** («цена реакции неизмерима у
  ...
  вопроса расширен этим каналом
  (`.claude/work/questions/open-questions.md` §PNL-Q1), решение —
  вместе с ним и тем же механизмом (счётчик пропусков скана в разрезе
  scope + причина).
- **Почему не чинится здесь и сейчас.** Любая починка — либо счётчик
  пропусков (общий механизм, у него свой владелец и своя развилка в
  `PNL-Q1`), либо подстановка часов (запрещена классом), либо кэш
  последнего биржевого времени (число, выглядящее фактом, — тот же
  запрет). Отдельного решения канал не заслуживает: он одна из трёх
  веток одного вопроса.
