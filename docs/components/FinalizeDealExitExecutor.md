...
- **Признаки отбора для отчёта пишет он же** (узел F `DOCS_CHECK_12`), той
  ...
  - **Ветки «иначе `NORMAL_EXIT`» больше нет** (H2 `GAPS_CLOSE_13`).
    ...
    из легенды типа.
  - **Недостача обязательного операнда — отказ источника.** Если запись
    positions-history добыта, а `type` в ней пуст либо вне перечня,
    финализатор сверх отчёта зовёт `HoldService` с **биржевым** радиусом
    (`docs/rules/exchange-hold.md`): источник вернул неполный ответ по
    полю, которое обязан отдавать. Пустота из-за отсутствия самой записи
    (позиция не заводилась) холда не вызывает.
