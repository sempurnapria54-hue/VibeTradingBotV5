...
только из пустоты: вверх база автоматически не ходит, а вниз её двигают
терминальные исполнители сделки. Дом политики базы —
...
Довод — невозрастаемость базы: записанный ноль автоматически уже не
поднимется, и счёт остался бы в отказе до явного действия держателя,
хотя средств могло не быть лишь в момент снимка. Отказ при этом тот же,
что у пустой базы, и он громкий: risk-creating действие отвергается кодом
`BALANCE_INVALID` (`docs/components/RiskValidator.md`).
