...
**Пустую `ExchangeAccount.riskBase` заполняет этот исполнитель — той же
транзакцией, что приземляет снимок.** Обновив account-level поля и
заменив список `Balance`, исполнитель смотрит на строку торгового состояния счёта, которой
принадлежит снимок: база пуста — он записывает в неё доступный остаток
расчётной валюты и той же записью проставляет `ExchangeAccount.riskBaseCurrency`;
