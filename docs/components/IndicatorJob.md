...
с уникальностью `UNIQUE(instrument_id, config_id, candle_timestamp)` (ключ
по идентичности считаемого через реестр конфигураций — см.
`docs/decisions/market-data-result-identity-keying.md`), при повторном
запуске дублей не создаёт. Для `DELETED`-стратегий новые значения не
считает.
...
последней закрытой свечи в группе (инструмент + `timeframe`). На тике job
берёт `max` → досчитывает от него по закрытым свечам. Отдельная
persisted checkpoint-модель не заводится (дубль данных, рассинхрон на
рестарте в середине пачки).
