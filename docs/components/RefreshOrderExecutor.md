...
**`Order.positionId` этот executor не пишет** (A9 `DOCS_CHECK_24`).
Прежняя редакция назначала его писателем «при наблюдении **первого** филла
ноги» — момент, в который на открывающем входе живой строки `Position` ещё
не существует, то есть писатель был недостижим на основной популяции.
Писатель — `RefreshPositionExecutor`; разбор, довод и пустые ветки — место
истины `docs/models/domain/core/Order.md` §«`positionId` — ось эпизода»,
