...
- **Нога 2 идёт при not-found ноги 1 — всегда, а не только при наличии
  ...
  - **Нижняя граница окна — `Order.externalCreatedAt` первой отправленной
    ...
    `Deal.billsWindowBegin`/`billsWindowEnd`, а обе на create-тропе пишет
    **ровно эта же нога 2**, то есть на момент формирования запроса оба поля
    пусты по построению и операнда запроса не существует. Это тот же класс
  ...
  - **Позиция, впервые увиденная уже закрытой, заводится и финализирует
    ...
    `externalId` и `direction` **из самой записи**) и заполняет **обе**
    границы окна: `billsWindowBegin` из `cTime` записи, `billsWindowEnd` из
    её `uTime`. Ждать наблюдения «живой» незачем — его уже не будет. Это
    единственная тропа, где `Position` создаётся ногой 2; прочие доки
