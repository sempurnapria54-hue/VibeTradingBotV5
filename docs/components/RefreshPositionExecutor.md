...
`docs/models/domain/core/Order.md` §«`positionId` — ось эпизода», здесь не
пересказываются.

**Почему писатель здесь** (A9 `DOCS_CHECK_24`). Связь «нога ↔ эпизод»
наблюдаема только в момент, когда она возникает, и на **открывающем** входе
этот момент — не филл ноги, а материализация строки `Position`: до неё
локального эпизода не существует (`docs/lifecycles/Position.md` §«Легитимное
окно появления позиции»). Прежний писатель (`RefreshOrderExecutor`, «первый
филл») на этой тропе не имел достижимого момента вызова, и колонка
оставалась пустой навсегда у ровно той ноги, ради которой заводится.
