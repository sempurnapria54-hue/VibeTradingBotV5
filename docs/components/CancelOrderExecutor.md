...
нога, закрытая с `closeReason = REPLACED_BY_STRATEGY`, выходит из
**заявленной** суммы (`Deal.plannedRiskAmount` — предикат «последнее звено
цепочки замещений», `docs/models/domain/aggregate/Deal.md` §«Предикат
отбора слагаемых»), а её исполненная доля остаётся во **взятой**
(`Deal.incurredRiskAmount`). Обе суммы пересчитываются **целиком** той же
транзакцией, в которой проставлен терминальный `closeReason` ноги входа.
Для не-входных ног суммы не трогаются.
