...
учётом round-trip комиссии**, а не цена входа. Из условия
`(стоп − якорь) − ставка × (якорь + стоп) = 0`:
...
Форма **точная**, не линейная: приближение `якорь × (1 ± 2 × ставка)`
оставляет остаток `±2 × ставка² × якорь`, знак которого зависит от
стороны (у длинной — остаточный убыток, у короткой — остаточная прибыль),
а шаг называет себя безубытком. Дом формы и её проверка —
...
убыток ровно в размере round-trip комиссии, и эта ошибка — **прежней
практики**, где «безубытком» звалась цена входа, — направлена всегда в
одну сторону. Величина её равна уже вычисляемой величине, поэтому новых
чисел назначение не вводит. Клейм об односторонности относится именно к
той практике: у **точной** формулы выше остатка нет вовсе, а у линейного
приближения он есть и знак его зависит от стороны — потому приближение и
не берётся.
