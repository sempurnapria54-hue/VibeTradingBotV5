...
- **Операнд на этих двух тропах — статус сделки** (H2 `DOCS_CHECK_11`,
  ...
  `Deal.md` §«Рамка R-выборки».
- **Пишет не handler.** Ноль и валюту пишет `MARK_DEAL_CLOSED_COMMAND`
  второй веткой, той же транзакцией, что и терминал (H1 `DOCS_CHECK_11`;
  `docs/components/MarkDealClosedExecutor.md` §«Вторая ветка»). Канон
  «статусные рёбра, являющиеся исходом системного действия, пишет звено,
  а не handler» держится.
