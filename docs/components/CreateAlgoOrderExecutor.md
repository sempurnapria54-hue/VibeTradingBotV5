...
- **Что пересчитывается:** **все четыре числа целиком** — по общему
  правилу «кто меняет любой операнд, пересчитывает всю четвёрку»
  (B3 `DOCS_CHECK_19`). Меняемый здесь операнд один — действующая
...
- **Форма вычитаемого — закрытая, дом формулы —
  `docs/models/domain/aggregate/Deal.md` §«Форма вычитаемого `protectionRelievedRiskAmount`»**: та же
...
- **Защита ещё не подтверждена биржей — и это учтено:** действующей
  ...
  (P1 `DOCS_CHECK_24`; `docs/models/domain/aggregate/Deal.md`
