...
- **Сверка bills ↔ net гоняется на ветви (a) при выполненном предикате
  ...
  **Ветки «операнд допуска не резолвился» нет** (решение держателя, позиция
  ...
  - **Полноту разбивки аварийный терминал считает тем же сравнением**
    ...
    и обе достижимы на аварийной тропе: **границы окна не добыты** (на
    ветке (b) `billsWindowEnd` пуст по построению) и **движения не
    добывались** (`Deal.billsFetchedThrough` пуст — сделка ушла в
    `ERROR` до первого исполнения bills-звена). Вторая ветвь — тот же
...
PNL_RECONCILIATION_MISMATCH, exchangeId))` **той же транзакцией**, что
применяет терминал. Разбор, почему актор — терминальное звено, а не
финализатор: `docs/components/MarkDealClosedExecutor.md` §«Реакция на
расхождение сверки».
