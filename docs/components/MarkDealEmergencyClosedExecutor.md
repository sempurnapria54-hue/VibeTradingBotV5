...
- **Сверка bills ↔ net гоняется на ветви (a) и не гоняется на (b)** (H21
  ...
  **операнд допуска не резолвился — `OPERAND_MISSING`** (H6
  `DOCS_CHECK_16`, решение пользователя). Прежде этот исход выразить было
  нечем: терминал здесь `EMERGENCY_CLOSED` в **обоих** случаях, `NOT_RUN`
  запрещён (сверка была обязана), `MATCHED`/`MISMATCHED` невычислимы —
  различитель, назначенный H2 `GAPS_CLOSE_15`, внутри аварийного терминала
  вырожден. Журнальный `AnomalyReport` `RECONCILIATION_OPERAND_MISSING`
  ставится тем же ходом.
