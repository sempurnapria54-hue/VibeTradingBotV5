...
- **Отказ канала добычи приравнивается к «пусто»** на этой тропе — не к
  ...
  - **Контролируемое исключение под приравнивание не подпадает** (H4
    ...
    (`ControlledExchangeException`) даёт **полный биржевой холд** и на
...
- **Операнды на аварийной тропе есть настолько, насколько добыты до неё.**
  ...
  сверка на ветви (a) гоняется тогда и только тогда, когда строки были
  залинкованы, пока сделка шла выходной тропой; иначе признак остаётся
  `NOT_RUN` + журнальный `AnomalyReport`. `appliedRate` зафиксирован **на записи**
...
- **Сверка bills ↔ net гоняется на ветви (a) и не гоняется на (b)** (H21
  ...
  **Ветки «операнд допуска не резолвился» нет** (решение держателя, позиция
  ...
  - **Полноту разбивки аварийный терминал считает тем же сравнением**
    ...
    (`max(t_добычи, billsWindowEnd) − billsWindowBegin`)
