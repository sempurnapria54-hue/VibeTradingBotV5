...
- **Сверка bills ↔ net гоняется на ветви (a) и не гоняется на (b)** (H21
  ...
  **Операнд допуска не резолвился** — сверка не выполняется, признак
  остаётся `NOT_RUN`, а факт несёт журнальный `AnomalyReport`
  `RECONCILIATION_OPERAND_MISSING`. Отдельного значения признака под этот
  исход **не заводится** (H6 `DOCS_CHECK_16` отклонён решением держателя,
  `GAPS_CLOSE_16`). Цена названа там же: терминал здесь
  `EMERGENCY_CLOSED` в **обоих** случаях, поэтому внутри него «не были
  обязаны» и «были обязаны, операнда не нашлось» различает только наличие
  журнального отчёта.
