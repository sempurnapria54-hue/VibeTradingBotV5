...
**Команда, не действие-оркестрация** (CMD-Q6,
`docs/decisions/action-orchestration-vs-command.md`): аварийный тормоз
доводит свой teardown **сам**, синхронным fire-all, и **не зависит** от
того, жива ли петля — в отличие от REPLACE (действие-оркестрация петлёй по
фактам). Поэтому `KILL_SWITCH` — одна команда с внутренними шагами, а не
секвенс, ведомый `DealStateMachine`. Жёсткое правило порядка (ниже): защиту
снимать **последней** и только после подтверждённого закрытия позиции —
никогда не оголять живую позицию.
