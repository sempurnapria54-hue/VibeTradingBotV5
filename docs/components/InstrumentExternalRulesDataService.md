...
3. **`FinalizeDealExitExecutor`** (шаг 7, H7 `DOCS_CHECK_11`) — **только
   ...
   считается **поногово** и извлекается из `Order.plannedRiskAmount`
   каждой ноги входа вычитанием её ценовой части, слагаемые суммируются
   (H6 `DOCS_CHECK_15`;
...
`RiskValidator`). **У финализатора пустой `ctVal` при обязанной сверке
означает ошибочную тропу** (H2 `DOCS_CHECK_15`, решение пользователя):
сверка была обязана и не выполнилась ⇒ сделка уходит к
`EMERGENCY_CLOSED` + журнальный `AnomalyReport`
`RECONCILIATION_OPERAND_MISSING`; биржевого холда это не поднимает —
недостача **наша**, радиус ущерба локализован сделкой. Обязанность не
наступила ⇒ `NOT_RUN` и успешный терминал. Прежняя редакция (H25
`DOCS_CHECK_12`: «`NOT_RUN` + терминал не блокируется» безусловно)
**снята** — она давала успешный терминал сделке, чья обязанная сверка не
выполнена (`docs/components/FinalizeDealExitExecutor.md` §epsilon).
