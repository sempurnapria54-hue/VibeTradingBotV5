...
- **Сверка была обязана выполниться и не выполнилась → тоже ошибочный
  терминал** (H10 `DOCS_CHECK_14`, решение пользователя). Обязанность
  ...
  движений закрыто (`Deal.billsWindowEnd` непуст). Пустой при этом правый
  операнд любой из четырёх пар сверки — недостача обязательного поля
  добытой записи, то есть **нарушение контракта интеграции**: журнальный
  `AnomalyReport` `RECONCILIATION_OPERAND_MISSING` + **полный биржевой
  холд**, а сама сделка уходит **ошибочной тропой** к `EMERGENCY_CLOSED`.
  Успешного `CLOSED` с непосчитанной парой не существует.
...
`DOCS_CHECK_14`): пустой правый операнд пары уводит сделку ошибочной тропой
