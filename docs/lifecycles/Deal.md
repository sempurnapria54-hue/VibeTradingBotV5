...
- **Сверка была обязана выполниться и не выполнилась → тоже ошибочный
  ...
  **независимо от того, чей операнд недостаёт**.
  - **Живой случай после переноса валидации — операнд допуска**
    (уровень стопа, не резолвимый предикатом селекции защиты;
    `docs/components/FinalizeDealExitExecutor.md` §epsilon. Дизъюнкт
    «`ctVal` навеса» снят H5 `DOCS_CHECK_16` — значение персистится
    write-once на ноге и резолвится всегда).
