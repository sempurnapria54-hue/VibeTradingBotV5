...
- **Сверка была обязана выполниться и не выполнилась → тоже ошибочный
  ...
  **независимо от того, чей операнд недостаёт**.
  - **Живого случая под эту ветку не осталось** (Р3 `GAPS_CLOSE_16`): оба
    операнда допуска — `ctVal` (H5 `DOCS_CHECK_16`) и уровень стопа (Р3) —
    персистятся write-once на ноге и резолвятся всегда
    (`docs/components/FinalizeDealExitExecutor.md` §epsilon). Ветка описана
    как недостижимая.
