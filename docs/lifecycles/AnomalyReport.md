...
инструмента, ступень 2 биржевой лестницы — обе ступени биржевой лестницы
разведены так, что реактивного `NON_CRITICAL`-отчёта на биржевом радиусе
не бывает: ступень 1 ставится вручную и отчёта не производит,
...
через `HoldService` (`docs/components/HoldService.md`). Ведущий
`STATE`-ветки **внутри `NON_CRITICAL`** у lifecycle один — синк; других
не требуется — формулировка говорит о ветке журнальной тропы, **не** о
виде `kind = STATE` поперёк троп (`ANOM-Q5`, разночтение разведено явно):
процессные `STATE`-отчёты ведут **три** актора — синк (журнальная),
...
- **`kind = STATE`** (факт держится во времени и сопровождается холдом
  без kill-switch; производитель один — несвежесть ставки / ключа группы
  у синка):
  1. Создание (`CREATED`) со слепками на момент обнаружения.
