...
**Почему `STATE` не завершается сразу.** Анкер идемпотентности охраняет
именно множество **незакрытых** отчётов. Если бы `STATE`-отчёт
завершался сразу, множество в установившемся режиме было бы пусто, анкер
не срабатывал бы **никогда**, и синк заводил бы копию каждый час — у того
самого производителя, ради которого анкер и вводился. Открытый отчёт при
живом холде — это и есть его «расследуется».
