...
`CRITICAL`-тропа остаётся за реакциями **с kill-switch** (полная реакция
инструмента, ступень 2 биржевой лестницы).

**Ступень 1 биржевой лестницы отчёт производит, и он `NON_CRITICAL`**
(A10 `DOCS_CHECK_20`; клауза «ставится вручную и отчёта не производит»
**снята** — она описывала редакцию до `GAPS_CLOSE_19`, давшего ступени 1
первый автоматический триггер). Писатель — `HoldService` в ветке
`FREEZE`, отчёт `kind = STATE` на журнальной тропе
(`docs/rules/exchange-hold.md`, `docs/components/HoldService.md`
§«Что делает», §Жизненный цикл ниже). Ручная постановка ступени 1
остаётся доступной и отчёта по-прежнему не производит — производит его
...
- `docs/components/MarkDealClosedExecutor.md` — нерезолвимая расчётная
  валюта на тропах закрытия **без входа** (терминал ставится с пустой
  `resultProfitCurrency`, `docs/lifecycles/Deal.md` §«Смягчение по
  валюте на тропах без входа»);
- `docs/components/SizeCalculator.md` (через исполнителя действия) —
  округление доли частичного выхода до полного и доля ниже `minSz`
  (`PARTIAL_EXIT_ROUNDED_TO_FULL`, `PARTIAL_EXIT_BELOW_MIN_SIZE`);
- `docs/components/InstrumentExternalRulesSyncJob.md` — несвежесть ставки
  или ключа комиссионной группы (мягкий холд).

**Перечень сверен с реестром кодов поимённо** (A10 `DOCS_CHECK_21`):
производителей журнальной тропы **шесть**, и все шесть названы. Прежняя
редакция перечисляла четырёх при шести в реестре
(`docs/models/domain/other/AnomalyReport.md` §«Реестр кодов шага 7») —
тот же класс расхождения, который этот же перечень уже фиксировал
строчкой ниже про `RefreshBillsExecutor`. Отсюда способ проверки:
перечень производителей сверяется **колонкой «Производитель» реестра**,
а не пополняется по памяти правки.
...
через `HoldService` (`docs/components/HoldService.md`). Ведущих
`STATE`-ветки **внутри `NON_CRITICAL`** у lifecycle двое — синк
(несвежесть ставки) и `HoldService` в ветке `FREEZE` (биржевая ступень 1,
`GAPS_CLOSE_19`); других не требуется — формулировка говорит о ветке
журнальной тропы, **не** о виде `kind = STATE` поперёк троп (`ANOM-Q5`,
разночтение разведено явно): процессные `STATE`-отчёты ведут **четыре**
актора — синк (журнальная), `HoldService` (`FREEZE`, биржевая ступень 1),
`SafetyHoldCoordinator` (реактивная, `CRITICAL`), `AnomalyJob`
(проактивная, шаг 8), — и поиск незавершённого применяется ко всем,
включая критические (`docs/models/domain/other/AnomalyReport.md`
...
- **`kind = STATE`** (факт держится во времени и сопровождается холдом
  без kill-switch; производителей два — несвежесть ставки / ключа группы
  у синка и биржевая ступень 1 у `HoldService`,
  ...
  3. `COMPLETED` — при **ручном снятии холда**
     ...
     `docs/rules/exchange-hold.md` §«Снятие — вручную, в `ACTIVE`»).
