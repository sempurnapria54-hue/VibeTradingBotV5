...
инструмента, ступень 2 биржевой лестницы); отчёт **биржевой заморозки**
(`Exchange.HOLD`, ступень 1, `docs/rules/exchange-hold.md`) идёт
`NON_CRITICAL`-тропой: `kind = STATE`, kill-switch-шагов нет,
закрывается ручным снятием холда.
...
- `docs/components/HoldService.md` — биржевая заморозка
  (`Exchange.HOLD`, ступень 1, `docs/rules/exchange-hold.md`): отчёт
  `kind = STATE`, ставится вместе с холдом напрямую, без координатора;
  завершается ручным снятием холда (вводится на `CODE`).
...
`STATE`-ветки **внутри `NON_CRITICAL`** у lifecycle два — синк и
`HoldService` (заморозка), других не требуется — формулировка говорит о
ветке журнальной тропы, **не** о виде
`kind = STATE` поперёк троп (`ANOM-Q5`, разночтение разведено явно):
процессные `STATE`-отчёты ведут **четыре** актора — синк (журнальная),
`HoldService` (заморозка, ступень 1), `SafetyHoldCoordinator`
(реактивная, `CRITICAL`), `AnomalyJob`
...
- **`kind = STATE`** (факт держится во времени и сопровождается холдом
  ...
  у синка и **биржевая заморозка** ступени 1 у `HoldService`,
