...
| `SYNC` | нет | Синхронизация спецификации инструмента с биржей (идентичность из `InstrumentExternalSnapshot`). |
...
- `CREATED → SYNC`: инструмент взят в онбординг; `ClientService`
  тянет спецификацию (идентичность → domain, справочные поля
  транзиентно; `docs/models/mapping/Instrument.md`).
