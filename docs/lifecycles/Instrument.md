...
реактивный/safety-контур **и ручной вызов держателя**
(`docs/rules/manual-halt.md`) — множества входа при этом те же; снимаются
они **вручную**, по умолчанию в `ACTIVE` (исключение — ступень, затёршая
незавершённый онбординг, см. ниже). **Множества входа у них разные**:
...
их затереть может: авария приоритетнее онбординга.

**Куда возвращает снятие в этом случае — закрыто (2026-08-29).** Снятие
жёсткой блокировки с инструмента, у которого **онбординг не был
завершён**, ведёт в онбординговый `HOLD`, а не в `ACTIVE`. Довод: снятие
safety-ступени не выдаёт объекту права торговать, которого у него не было
до её подъёма; безусловный `ACTIVE` вернул бы в выборку входа инструмент
с несинхронизированной спецификацией или недогруженными свечами — это
разрешающая ошибка (`docs/concept.md`, П1 следствие 3). Из онбордингового
`HOLD` инструмент двигает онбординг, а не поверхность снятия. Форма
операции и исполнимая проверка — `docs/rules/manual-halt.md`
§«Снятие: что во что переходит и при каком предусловии»,
`docs/spec/manual-halt.json` (`clearanceTarget`).
