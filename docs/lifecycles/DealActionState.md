...
**`RETRY_PENDING → SKIPPED` допущен** (A14 `DOCS_CHECK_21`). Тропы
**перехвата** уводят сделку прямой записью в любой момент
(`docs/components/DealOrchestratorJob.md` §«Перехват реакции» —
исключение исполнителя, и §«Enforcement холда» — жёсткий холд scope;
адрес поправлен A2 `DOCS_CHECK_22`, прежний `§«Перехваты»` не
соответствовал ни одному заголовку целевого файла), поэтому
исполнение может стать неактуальным, пока ждёт `nextRetryAt`. Без этого
ребра единственным выходом из ожидания оставался бы `FAILED`, а он
притягивает холд инструмента (`docs/rules/instrument-hold.md` §«Серия
неудач») — реакцию, которую решение П11 для этой популяции прямо
исключает: сделка ушла с тропы штатно, отказа не было. Сцеплено с
§«Писатель `SKIPPED`»: закрывает ждущую строку та же точка входа.
...
- `* → SKIPPED` — действие стало неактуальным (например, добыча при уже
  ...
  **Пишет `SystemActionExecutor`** — при выводе стадии (A4
  `DOCS_CHECK_19`) **либо ревизией живых строк на проходе** (A3
  `DOCS_CHECK_21`, §«Писатель `SKIPPED`» ниже).
...
**Актуальность исполнения выводится там же, где стадия** (A4
`DOCS_CHECK_19`). `SystemActionExecutor` на каждом проходе читает строку
...
**Этого одного входа недостаточно, и вторая точка входа заведена** (A3
`DOCS_CHECK_21`). Вывод стадии происходит внутри `next(type,
...
- **Почему не handler.** Единственный командный уровень handler'а —
  cleanup (`docs/decisions/fsm-execution-layering.md`); права двигать
  строки исполнений у него нет.
- **Почему не звено соседнего действия.** `MarkDealErrorExecutor` —
  звено `FINALIZE_DEAL_ERROR_ACTION`; закрывать чужую строку значило бы
  завести знание о соседнем действии внутри звена, прецедента чему в
  доках нет.
- **Цена «не закрывать вовсе» названа и отвергнута:** живая строка на
  терминализованной сделке поднимается §Recovery по статусу, а частичный
