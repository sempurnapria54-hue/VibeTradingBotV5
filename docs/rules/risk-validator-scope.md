...
- создание защитного algo-order, не обеспечивающего требуемый контроль
  риска — операционное содержание этой формулировки — **предикат
  покрытия** (`docs/rules/risk-creating-entry-protection.md` §«Предикат
  покрытия и точки его проверки», C1 `DOCS_CHECK_23`).

**Набор проверок для ветки risk-weakening — не тот же, что для
risk-creating** (C3 `DOCS_CHECK_23`): поактное неравенство на ней не
применяется, «риск акта» = 0, `liveRiskNow` берётся пост-действенным.
Состав — `docs/components/RiskValidator.md` §«Конкретные проверки
(фаза 1)»; дом определения — `docs/decisions/per-trade-risk-policy.md`
§«„Риск акта“ определён по классу действия, и на ослаблении защиты форма
другая».

**Алго-тропа валидируется не как вход** (`RISK-Q4`): включение
`CREATE_ALGO_ORDER_COMMAND` в это множество относится к ветке
risk-weakening (последний пункт), а не к входной ветке — входной тропы
алго-ордером не существует (`docs/models/domain/core/AlgoOrder.md`
§Назначение). Вывод «валидируется ⇒ вход алго-ордером достижим» из этого
перечня не следует.
