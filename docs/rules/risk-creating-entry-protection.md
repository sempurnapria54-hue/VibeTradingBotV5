...
названо: ветка «обязанная сверка с нерезолвимым операндом» остаётся
достижимой — реакция на неё описана
(`docs/components/FinalizeDealExitExecutor.md` §epsilon: ошибочная тропа +
журнальный `AnomalyReport`, отдельного значения признака под неё не
заводится).
