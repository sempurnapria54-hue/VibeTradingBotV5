...
- **Недостача обязательного поля в добытой записи positions-history**
  (H7/H10 `DOCS_CHECK_14`) — тот же класс «нарушение контракта
  интеграции», обнаруживаемый **финализаторами**: запись закрытия добыта,
  а `type` в ней пуст/вне перечня (`CLOSE_OUTCOME_UNDETERMINED`) либо
  пуст правый операнд пары сверки при обязанной сверке
  (`RECONCILIATION_OPERAND_MISSING`). Детекторы —
  `FinalizeDealExitExecutor` / `MarkDealEmergencyClosedExecutor`
  (внесены в реестр `docs/components/HoldService.md` §«Кто зовёт»);
  фабрика — `exchange(code, exchangeId)`, `ReactionClass.FULL`. Пустота
  из-за отсутствия самой записи триггером не является.
...
ущерба (у триггера финализаторов он мал — отчётный признак одной закрытой
сделки), а неизвестностью радиуса поражения; такой случай требует ручного
разбора человеком (`docs/rules/error-handling-policy.md` §«Нарушение
контракта интеграции — радиус по неизвестности поражения»). Поэтому
блок-сет ниже — тотальный по бирже.
