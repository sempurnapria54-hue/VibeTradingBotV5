...
- **Отмена, не объявленная нашим намерением, причину всё равно получает.**
  ...
  инициированную биржей, непроходимой. Различение ручной отмены и биржевой
  остаётся: у первой намерение непусто (`MANUAL_CANCEL`), у обеих сырой
  статус источника сохранён на сущности. Форма — `closeReasonCandidate` в
  спеке.
