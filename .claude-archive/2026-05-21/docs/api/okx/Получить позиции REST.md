...
* `posId` — id позиции. Можно перечислить до **20** `posId`. **Важно:** после полного закрытия позиции `posId` живёт
  ограниченное время (в доке: 30 дней), потом позиция/posId «очищаются».
...
* Если режим **long/short** → вернёт отдельные записи `posSide=long` и/или `posSide=short` (а `pos` обычно
  положительный).
...
* Auth headers (REST private): те же, что и для balanceExternalSnapshot (OK-ACCESS-*)
...
* `pos` — размер позиции. В net-режиме для деривативов знак может означать long/short; в long/short-режиме обычно
  положительное число.
