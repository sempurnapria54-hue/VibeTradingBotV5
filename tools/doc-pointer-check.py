...
Javadoc и комментарии живых деревьев кода (`services/**`, `web/**`)
...
ОБЛАСТЬ. Живые деревья кода `services/**` и `web/**`. Дерево монолита
фазы 1 из области выпало вместе с самим монолитом (2026-09-16,
...
# В форме САМОГО указателя `libs/` оставлен намеренно — так отставший
# указатель на снятое дерево опознаётся битым, а не пропускается молча.
ROOTS = ('services', 'web')
POINTER = re.compile(
...
def main():
    ...
    print('файлов обойдено: %d; указателей проверено: %d; битых: %d '
          '(область: %s)'
          % (files, total, len(broken), ', '.join(ROOTS)))
