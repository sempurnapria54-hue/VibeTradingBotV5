...
ЧТО ВЫВОДИТСЯ ИЗ ЧЕГО — и где граница вывода, названная, а не умолчанная:
  ...
  * `идёт:` — ВСЯКАЯ команда оболочки, чей результат не пришёл за пять
    ...
    (`> файл`, `>> файл`, `| tee файл`); время — его последней записи. У
    обёртки реактора лог — её постоянный `target/reactor-test.log`, в тексте
    команды его нет. Лога нет (вывод не перенаправлен, путь из переменной
    `$LOG`, файл не создан) — хвост просто `(N мин)`. Отличает прогон от зависания: у живого прогона
...
# Обёртка реактора пишет лог изнутри, по ПОСТОЯННОМУ пути от корня
# репозитория (шапка `tools/reactor-test.sh`, раздел ЛОГ): в тексте команды его
# нет, и лента берёт его отсюда.
REACTOR_RUN = re.compile(r"tools/reactor-test\.sh")
REACTOR_LOG = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                           "target", "reactor-test.log")
...
class Feed:
    ...
    def log_file(self, first, base_dir):
        ...
        когда его нет или путь не читается из текста (`$LOG`). У обёртки
        реактора — её постоянный лог, даже при перенаправлении: вывод самой
        обёртки молчит всю сборку, и его давность читалась бы зависанием."""
        if REACTOR_RUN.search(first):
            return REACTOR_LOG
