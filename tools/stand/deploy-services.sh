...
#
# УБОРКА ХОСТА — ВТОРАЯ ПОЛОВИНА ТОЙ ЖЕ ПОЛИТИКИ, а не отдельный механизм.
# Прежде уборка доходила до узла и на нём останавливалась, хотя `docker
# build` кладёт образ СНАЧАЛА на хост: после `kind load` содержимое уже в
# узле, и хостовая копия своё отслужила. Не убранная, она копится тем же
# линейным ходом, что и копия в узле, — по образу на выкладку.
#
# Замер, на котором записано (2026-09-09): на хосте лежали шесть образов
# `registry.invalid/vibetrading/*` от одного дня выкладок плюс 2.72 ГБ
# кэша сборки — 4.2 ГБ, при том что узел был чист (по одной ссылке на
# сервис). Убирал их держатель руками.
#
# ЦЕНА НАЗВАНА: базовый образ ВЫКАЧИВАЕТСЯ ЗАНОВО НА КАЖДУЮ ВЫКЛАДКУ.
# BuildKit держит слои `FROM` в кэше сборки, а не в хранилище образов;
# снятие образа сервиса делает этот кэш нессылаемым, и `builder prune`
# уносит вместе с ним базу. Измерено: пересборка после уборки — 25 с и
# ~157 МиБ сети. Против реактора, идущего минутами, это шум, а место
# уборка держит ограниченным вместо линейного роста.
#
...
for svc in "${SERVICES[@]}"; do
  ...
  # Содержимое уже в узле — хостовая копия своё отслужила. Отказ снятия
  # выкладку не роняет, как и отказ уборки узла: копия уйдёт следующей.
  docker image rm "registry.invalid/vibetrading/$svc:$TAG" >/dev/null 2>&1 || echo "  хостовая копия не снята, уйдёт следующей выкладкой: $svc:$TAG"
...
echo "=== уборка кэша сборки на хосте"
