...
# `▶`/`■` на границах сессии, действия (чтения пачками, правки, прогоны),
# строку `идёт:` раз в пять минут у всякой команды дольше пяти минут — с
# давностью её лога, когда вывод перенаправлен в файл, — и пульс `…` при
# тишине. Источник — поток событий самой сессии
...
# его не дублирует. Фоновых задач у сессии цикла нет вовсе (ниже), и снятый
# потолок остаётся страховкой на случай, если держатель фон вернёт.
export CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS="${CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS:-0}"

# ФОНОВЫХ ЗАДАЧ У СЕССИИ ЦИКЛА НЕТ ВОВСЕ. В `-p` со схемой конца хода ждёт
# конверт: сессия, закончившая ход ради ожидания фонового субагента, получает
# `[structured-output-enforce]` сразу, а не по его возвращении, — и выдаёт
# конверт посреди единицы, без реактора и гейта. Субагент доходит уже после
# конверта, CLI будит сессию вторым ходом, и цикл читает последний конверт —
# красный. Фон у субагента при этом включён УМОЛЧАНИЕМ CLI, а не выбором
# модели, поэтому запрет в промпте класс не закрывает. Переменная убирает
# `run_in_background` у Agent и Bash и автоперевод прогона в фон: субагент
# становится обычным вызовом, чей результат приходит в том же ходе, а
...
echo "фоновые задачи: CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=${CLAUDE_CODE_DISABLE_BACKGROUND_TASKS:-<пусто — фон включён>}"
