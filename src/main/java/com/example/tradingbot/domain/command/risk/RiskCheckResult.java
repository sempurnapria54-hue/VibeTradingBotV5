...
public class RiskCheckResult {
    ...
    public enum RiskCheckCode {
        ...
        /**
         * Прогнозная ставка комиссии не резолвится: у инструмента нет
         * ключа комиссионной группы либо группа ещё не наблюдалась.
         * ВРЕМЕННЫЙ: ставку приносит синк справочника, и следующий проход
         * может её застать (docs/models/domain/other/TradeFeeRate.md).
         */
        FEE_RATE_UNAVAILABLE(false),

        /**
         * Число риск-аппетита не задано конфигурацией — сверять не с чем
         * (docs/rules/risk-policy.md §«Числа назначает держатель; пустое
         * место — отказ»). ВРЕМЕННЫЙ: задаётся конфигурацией, не правкой
         * стратегии.
         */
        RISK_APPETITE_NOT_CONFIGURED(false),

        /**
         * Порог остановки по серии убытков не задан — энфорсера остановки
         * не существует (docs/rules/loss-streak-halt.md). ВРЕМЕННЫЙ по той
         * же причине.
         */
        LOSS_LIMIT_NOT_CONFIGURED(false),

        /**
         * Граф сделки предъявлен контекстом не целиком — операнды
         * занижены (docs/spec/deal-context-load.json, graphComplete).
         * ВРЕМЕННЫЙ: следующий проход может предъявить граф полностью.
         * Вердиктом риск-политики НЕ является: реакция на него — тропа
         * ошибки сделки на ЛЮБОЙ стадии, а не терминал транша
