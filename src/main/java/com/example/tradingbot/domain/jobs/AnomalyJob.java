 ...
 *
 * <p><b>Названное ограничение — прочие детекторы перечня
 * (docs/components/AnomalyJob.md §«Что ищет») здесь не построены:</b>
 * живая заявка без локальной сущности, терминальная сделка с живой
 * позицией, локально закрытая сущность, живая на бирже, переоценка
 * инварианта ликвидации, биржа-широкая orphan-сверка. Их предмет —
 * проактивная детекция шага 8 (`.claude/work/backlog.md` §«Шаг 8
 * (safety / AnomalyJob)»): у каждого свой такт, свой гистерезис и своя
 * классификация реакции, и заводить их «по одному признаку» значило бы
 * назначить реакцию, которой корпус ещё не выбрал.
 *
