...
public class ManagingHandler implements FsmHandler {
    ...
    public DealTransition handle(DealContext dealContext) {
        ...
        if (isFalse(support.positionLiveRisk(deal))) {
            // Позиция закрыта/отсутствует (SL/TP/внешнее закрытие) — дочистка в EXIT_PENDING.
            return DealTransition.transition(Deal.Status.EXIT_PENDING);
        }
        List<StrategyStep> steps = support.stepsOfType(support.stepsFor(dealContext, Deal.Status.MANAGING),
    ...
    private DealTransition continueInProgress(List<StrategyStep> steps, DealContext dealContext) {
        for (StrategyStep step : steps) {
            for (StrategyAction action : nullSafe(step)) {
                ...
                if (isExitSubmitted(step, status)) {
                    return DealTransition.transition(Deal.Status.EXIT_PENDING);
                }
