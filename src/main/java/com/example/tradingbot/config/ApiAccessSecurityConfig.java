 ...
 * вызове подделывать нечего. <b>Названное ограничение:</b> браузер кэширует
 * Basic-креды по источнику и переотправляет их сам, поэтому с появлением
 * браузерного фронта (шаг 11) решение перечитывается вместе с ним — сегодня
 * браузерной тропы к поверхности не существует.
 *
 * <p><b>Цена stateless: проверка хэша идёт на каждом вызове.</b> BCrypt
 * намеренно медленный, и это плата за отсутствие второго носителя факта «кто
 * вызывает». Для контура одного оператора она принята; порог, при котором
 * вопрос возвращается, — не число запросов, а появление тропы с их потоком
 * (фронт, второй субъект).
 */
@Configuration
@EnableWebSecurity
